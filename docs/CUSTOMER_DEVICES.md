...
### 2.2.23 顧客タブ初期表示と未使用センサー非表示（2026-09-23 追記）

既存 2.2 / 2.2.1〜2.2.22 は残す。上書きしない。

| 項目 | 値 |
|------|------|
| 顧客タブ | `/customer` は「お知らせ / 家のようす / 履歴」。初期はお知らせ |
| 通知ボタン | エリア別通知のミュート表示は「通知オフ」（モード値 `silent` は維持） |
| 顧客に出すセンサー | 外周ビーム（母屋・遠）と建物至近ビーム（母屋・近）のみ |
| はなれ | 道路側・通路側は顧客の通知カードから外す。API と社内設定は残す |
| SW | `tisly-pwa-v2564-customer-alert-tab` |

### 2.2.22 至近単独ではフラッシュ（DO3）を出さない（OTA 1.2.14 / 2026-09-23 追記）

既存 2.2 / 2.2.1〜2.2.21 は残す。上書きしない。

| 項目 | 値 |
|------|------|
| 判定 | 外周（DI1）が先に反応した `perimeterTimeoutSec` 以内の至近（DI2）だけ段階侵入 |
| 至近が先 / 単独 | DO1+DO2 のみ。DO3 フラッシュは起動しない |
| 段階侵入 | DO1+DO2 に加え DO3 を点灯 |
| 手動威嚇 | `patlite_test` / `flash_test` は従来どおり DO3 を駆動 |
| OTA | `FIRMWARE_LOGIC_VERSION=1.2.14` · `OTA_VERSION=1.2.14` |
...
| 日付 | 内容 |
|------|------|
| 2026-09-23 | 顧客タブ初期を「お知らせ」に変更。エリア別通知は母屋ビーム2つのみ、「サイレント」表示を「通知オフ」。既存センサー定義は非破壊 |
| 2026-09-23 | 豊島邸 至近が外周より先または単独のときは DO1+DO2 のみ。DO3 は段階侵入のときだけ。OTA 1.2.14。既存 2.2 系・はなれ・板橋は非破壊 |
