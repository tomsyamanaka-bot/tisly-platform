...
**本番デプロイ:** `f7bfd36a`（`commitShort`: `f7bfd36`）  
**確認URL:** https://tisly.jp/api/health  
**ステータス:** 完全コンプリート／本日業務クローズ済み
...
点と線分の最短距離計算により
許容10px以内の最短1本だけを
`splice` で削除する Hit Testing を実装した。
...
| `027db9fa` | forceNukeTouchBlockerEl で透明遮断幕を物理消去 |
| **`f7bfd36a`** | **最短距離 Hit Testing＋AI 閾値極小化（本番最終）** |
...
- 単一パス削除は最短距離 Hit Testing
  （`ERASER_HIT_TOLERANCE_PX = 10`）
- AI 輪郭抽出は閾値極小（`minArea=1` 等）
...
- `hidden` クラスだけでオーバーレイ解除を済ませる
  （透明タッチブロックが残る）
- 消しゴムで複数パスを一括削除する実装に戻す
- AI 作図の過剰フィルタ／高い minArea を復活させる
...
## 8. 本日最終到達（f7bfd36）開発手順

### 8.1 消しゴム Hit Testing（単一パス削除）
...
- 点とパス各線分の最短距離を計算
- 許容は `ERASER_HIT_TOLERANCE_PX = 10`
- 許容内で最も近いパスを1本だけ採用
- `cleaned.splice(hitIndex, 1)` で物理削除
- マスク消しゴムと併用し方眼紙は保護
...
- `minArea=1` 等へ閾値を極限まで下げる
- open 処理など過剰フィルタを無効化
- クライアント／サーバ双方で閾値を統一
- 関連: `survey-sketch-auto-draw-v1.js`
- 関連: `survey-sketch-line-detect-v1.ts`
...
- 関連テスト 22 件全パス
- 本番デプロイ `f7bfd36`
- https://tisly.jp/api/health の
  `commitShort` が `f7bfd36` であること

**本日の図面アプリ（survey-drawing-v1）に関する
UI改修・バグ修復・AI連携タスクは
これをもって完全終了（クローズ）する。**
