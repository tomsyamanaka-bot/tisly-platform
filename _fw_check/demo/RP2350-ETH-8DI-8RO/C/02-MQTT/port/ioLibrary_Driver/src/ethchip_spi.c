...
void ethchip_reset()
{
    ...
    bi_decl(bi_1pin_with_name(PIN_RST, "NETCHIP RESET"));
}
...
{
#ifdef USE_PIO
    spi_handle = net_spi_pio_open(&g_spi_config);
    (*spi_handle)->set_active(spi_handle);
#else
    // this example will use SPI0 at 5MHz
    spi_init(SPI_PORT, _NETCHIP_SPI_SCLK_SPEED * 1000 * 1000);
...
void ethchip_initialize(void)
{
    ...
    reg_ethchip_spi_cbfunc((*spi_handle)->read_byte, (*spi_handle)->write_byte);
    reg_ethchip_spiburst_cbfunc((*spi_handle)->read_buffer, (*spi_handle)->write_buffer);
    reg_ethchip_cs_cbfunc((*spi_handle)->frame_start, (*spi_handle)->frame_end);
#else
    ...
    do
    {
        if (ctlethchip(CW_GET_PHYLINK, (void *)&temp) == -1)
        {
