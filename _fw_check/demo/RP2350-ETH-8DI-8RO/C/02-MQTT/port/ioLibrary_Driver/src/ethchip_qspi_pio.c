...
#include "ethchip_qspi_pio.h"

#include "ethchip_qspi_pio.pio.h"
...
#if (_NETCHIP_QSPI_MODE_ == QSPI_SINGLE_MODE)
#define PIO_PROGRAM_NAME ethchip_pio_spi_single_write_read
#elif (_NETCHIP_QSPI_MODE_ == QSPI_DUAL_MODE)
#define PIO_PROGRAM_NAME ethchip_pio_spi_dual_write_read
#elif (_NETCHIP_QSPI_MODE_ == QSPI_QUAD_MODE)
#define PIO_PROGRAM_NAME ethchip_pio_spi_quad_write_read
#endif
...
#else
    #define PIO_PROGRAM_NAME net_spi_write_read
    #define PIO_PROGRAM_FUNC __CONCAT(PIO_PROGRAM_NAME, _program)
    #define PIO_PROGRAM_GET_DEFAULT_CONFIG_FUNC __CONCAT(PIO_PROGRAM_NAME, _program_get_default_config)
    #define PIO_OFFSET_WRITE_BITS __CONCAT(PIO_PROGRAM_NAME, _offset_write_bits)
    ...
    // All net spi operations must start with writing a 3 byte header
...
typedef struct spi_pio_state {
    net_spi_funcs_t *funcs;
    const net_spi_config_t *spi_config;
    pio_hw_t *pio;
...
static void net_spi_pio_close(net_spi_handle_t funcs);
static net_spi_funcs_t *get_net_spi_pio_impl(void);
...
net_spi_handle_t net_spi_pio_open(const net_spi_config_t *spi_config) {
...
spi_pio_state_t *state;
    ...
    state->funcs = get_net_spi_pio_impl();
...
static void net_spi_pio_close(net_spi_handle_t handle) {
...
static void net_spi_pio_frame_start(void) {
    assert(active_state);
    #if   (_NETCHIP_ == W6300)
        #if (_NETCHIP_QSPI_MODE_ == QSPI_SINGLE_MODE)
...
static void net_spi_pio_frame_end(void) {
    assert(active_state);
...
void net_spi_pio_read_byte(uint8_t op_code, uint16_t AddrSel, uint8_t *rx, uint16_t rx_length)  
{
...
void net_spi_pio_write_byte(uint8_t op_code, uint16_t AddrSel, uint8_t *tx, uint16_t tx_length)  
{
    uint8_t command_buf[8] = {0,};
    uint16_t command_len = mk_cmd_buf(command_buf, op_code, AddrSel);
    uint32_t loop_cnt = 0;
...
static uint8_t net_spi_pio_read_byte(void) {
    assert(active_state);    
...
static void net_spi_pio_write_byte(uint8_t wb) {
    panic_unsupported(); // shouldn't be used
}

// To read a buffer we must first have been asked to write a 3 byte spi header
static void net_spi_pio_read_buffer(uint8_t* pBuf, uint16_t len) {
...
static void net_spi_pio_write_buffer(uint8_t* pBuf, uint16_t len) {
    assert(active_state);
...
static void net_spi_pio_set_active(net_spi_handle_t handle) {
    active_state = (spi_pio_state_t *)handle;
}

static void net_spi_pio_set_inactive(void) {
    active_state = NULL;
}

static void ethchip_spi_pio_reset(net_spi_handle_t handle) {
...
static net_spi_funcs_t *get_net_spi_pio_impl(void) {
    static net_spi_funcs_t funcs = {
        .close = net_spi_pio_close,
        .set_active = net_spi_pio_set_active,
        .set_inactive = net_spi_pio_set_inactive,
        .frame_start = net_spi_pio_frame_start,
        .frame_end = net_spi_pio_frame_end,
        .read_byte = net_spi_pio_read_byte,
        .write_byte = net_spi_pio_write_byte,
        #if   (_NETCHIP_ == W5500)
        .read_buffer = net_spi_pio_read_buffer,
        .write_buffer = net_spi_pio_write_buffer,
        #endif
        .reset = ethchip_spi_pio_reset,
    };
    return &funcs;
}