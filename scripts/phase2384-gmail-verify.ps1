...
Write-Host "=== Phase 2384 Gmail real-send verify ==="
...
try {
  ...
  $pc = Invoke-RestMethod -Uri "$BaseUrl/api/deploy/production-check-2383" -Method Get
}
$pc | Select-Object phase, adminPasswordStatus, operationalReady, gmailMode, smtpConfigured, productionRatePercent, ready | ConvertTo-Json -Compress

Write-Host "`n=== Phase 2384 Gmail verify: SUCCESS ==="
Write-Host "受信メール確認: NOTIFICATION_TEST_TO 宛に [TiSLY] Gmail 通知テスト が届いているか確認してください"
