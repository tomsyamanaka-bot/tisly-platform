...
function renderInfoPage(ctx: PracticalCompletionReportContext): string {
  const rows = [
    ["螳帛錐", ctx.addressee],
    ["譯井ｻｶ逡ｪ蜿ｷ", ctx.projectNo],
    ["迴ｾ蝣ｴ蜷・, ctx.siteName],
    ["蟾･莠句ｴ謇", ctx.workLocation],
    ["菴懈･ｭ譌･", ctx.workDate],
    ["諡・ｽ楢・, ctx.staffName],
    ...(ctx.notes?.trim() ? [["蛯呵・, ctx.notes.trim()]] : []),
  ]
    .map(
      ([label, value]) =>
...
function renderNoPhotosPage(): string {
  ...
  <div class="cr-no-photos">蜀咏悄譛ｪ逋ｻ骭ｲ</div>
</div>`;
}
...
<title>螳御ｺ・ｱ蜻頑嶌 ${escapeHtml(ctx.projectNo)}</title>
