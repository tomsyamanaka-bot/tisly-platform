...
import { buildTomsFormatPreviewV1, createEstimateFromSurveyV1, createInvoiceFromEstimateV1, duplicateEstimateV1, finalizeEstimateV1, getEstimatePdfContextV1, getEstimateProjectV1Detail, listEstimateProjectsV1, listPendingSurveysV1, renderCompletionReportHtmlV1, renderSpecificationHtmlV1, updateEstimateHeaderV1, updateEstimateItemsV1, } from "../../estimate/estimate-v1-store.js";
import { businessUploadsDir } from "../../business/business-store.js";
import { getEstimatePdfOrPlaceholder, getInvoicePdfOrPlaceholder, } from "../../business/services/pdfService.js";
import { getBusinessProject, getEstimate, getInvoice } from "../../business/business-store.js";
import { buildMaterialCandidatesForSurvey, listAllMaterialCandidatePresets, } from "../../estimate/material-candidates.js";
export const estimateV1Router = Router();
const estimateV1Auth = [requireAuth("surveyor")];
function assertEstimateV1Role(req, res) {
    const role = req.admin?.role ?? "viewer";
    if (!roleMeetsRequirement(role, "surveyor") &&
...
function parseIncludePhotos(query) {
    const raw = query.includePhotos ?? query.photos;
    if (raw === "1" || raw === "true" || raw === "yes")
        return true;
    return false;
}
estimateV1Router.get("/material-candidates/:surveyProjectId", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const groups = buildMaterialCandidatesForSurvey(String(req.params.surveyProjectId));
    res.json({ groups });
});
estimateV1Router.get("/material-candidates", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    res.json({ groups: listAllMaterialCandidatePresets() });
});
estimateV1Router.get("/pending-surveys", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const customerCode = req.query.customerCode ?? req.admin?.customerCode;
    res.json({ surveys: listPendingSurveysV1({ customerCode }) });
});
estimateV1Router.get("/projects", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const customerCode = req.query.customerCode ?? req.admin?.customerCode;
    res.json({ projects: listEstimateProjectsV1({ customerCode }) });
});
estimateV1Router.post("/from-survey/:surveyProjectId", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    try {
        const detail = createEstimateFromSurveyV1(String(req.params.surveyProjectId), req.admin?.username);
        res.status(201).json(detail);
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : "create failed";
        const status = msg === "survey project not found"
            ? 404
...
estimateV1Router.get("/projects/:id", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
...
estimateV1Router.patch("/projects/:id/header", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const body = req.body;
    try {
...
estimateV1Router.patch("/projects/:id/items", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const body = req.body;
...
estimateV1Router.post("/projects/:id/finalize", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const body = (req.body ?? {});
    try {
...
estimateV1Router.get("/projects/:id/pdf", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    ...
    const includePhotos = parseIncludePhotos(req.query);
...
estimateV1Router.post("/projects/:id/invoice", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    try {
...
estimateV1Router.get("/projects/:id/invoice/pdf", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const project = getBusinessProject(String(req.params.id));
...
estimateV1Router.get("/projects/:id/specification/pdf", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const project = getBusinessProject(String(req.params.id));
    if (!project) {
...
estimateV1Router.get("/projects/:id/completion-report/pdf", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    const project = getBusinessProject(String(req.params.id));
    if (!project) {
...
estimateV1Router.post("/projects/:id/duplicate", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    try {
...
estimateV1Router.get("/projects/:id/toms-format", ...estimateV1Auth, (req, res) => {
    if (!assertEstimateV1Role(req, res))
        return;
    try {
        const includePhotos = parseIncludePhotos(req.query);
