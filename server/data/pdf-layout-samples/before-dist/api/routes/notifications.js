...
import { getGmailSmtpStatus, sendGmailTestEmail, } from "../../notification/smtp-gmail.js";
...
function markRead(id, res) {
    const db = getDatabase();
    const r = db
        .prepare(`UPDATE notification_logs SET read_at = datetime('now') WHERE id = ?`)
        .run(id);
    if (r.changes === 0) {
...
notificationsRouter.post("/read-all", (_req, res) => {
    const db = getDatabase();
    const r = db
        .prepare(`UPDATE notification_logs SET read_at = datetime('now') WHERE read_at IS NULL`)
        .run();
...
notificationsRouter.post("/test", async (req, res) => {
    const channel = (req.body?.channel ?? "web_push");
