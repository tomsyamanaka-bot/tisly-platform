...
import { SURVEY_MATERIAL_CATEGORIES, SURVEY_WORKFLOW_STATUSES, } from "../../survey/survey-v1-types.js";
import { addSurveyMaterialV1, addSurveyPhotoMemoV1, copySurveyProjectV1, createSurveyProjectV1, deleteSurveyPhotoV1, deleteSurveyProjectV1, getSurveyProjectV1Detail, listSurveyProjectsV1, markEstimatePendingV1, moveSurveyPhotoV1, updateSurveyPhotoV1, updateSurveyProjectV1, } from "../../survey/survey-v1-store.js";
export const surveyV1Router = Router();
const surveyV1Auth = [requireAuth("surveyor")];
...
function parseWorkflowStatus(raw) {
    if (typeof raw !== "string")
        return null;
    return SURVEY_WORKFLOW_STATUSES.includes(raw)
        ? raw
        : null;
}
surveyV1Router.get("/projects", ...surveyV1Auth, (req, res) => {
...
surveyV1Router.post("/projects", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    const body = req.body;
...
surveyV1Router.get("/projects/:id", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
...
surveyV1Router.patch("/projects/:id", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    const body = req.body;
...
surveyV1Router.post("/projects/:id/copy", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    try {
...
surveyV1Router.delete("/projects/:id", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
...
surveyV1Router.post("/projects/:id/photos/:photoId/move", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    const body = req.body;
    ...
    const photos = moveSurveyPhotoV1(String(req.params.id), String(req.params.photoId), body.direction);
    if (!photos) {
        res.status(404).json({ error: "Not found" });
        return;
    }
...
surveyV1Router.delete("/projects/:id/photos/:photoId", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
...
surveyV1Router.patch("/projects/:id/photos/:photoId", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    const body = req.body;
...
surveyV1Router.post("/projects/:id/photos", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    const body = req.body;
...
surveyV1Router.post("/projects/:id/materials", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    const body = req.body;
    if (!body.category || !SURVEY_MATERIAL_CATEGORIES.includes(body.category)) {
...
surveyV1Router.post("/projects/:id/estimate-pending", ...surveyV1Auth, (req, res) => {
    if (!assertSurveyRole(req, res))
        return;
    try {
