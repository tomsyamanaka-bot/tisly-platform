...
import { getAutomationSettings, saveAutomationSettings, } from "../../security-automation/security-automation-store.js";
import { collectSecurityNotificationCandidates } from "../../security-automation/security-notifications.js";
import { confirmPendingArmCheck, createSecurityEventLog, evaluatePresenceOnlyChange, evaluateSecurityArmGate, getAutomationRules, getLastSecurityEventLog, getSecurityState, listSecurityEventLogs, setSecurityMode, startPendingArmCheck, updateAutomationRule, } from "../../services/securityAutomationService.js";
import { getPresenceSummary, getRegisteredDevices, registerPresenceDevice, updateDevicePresence, } from "../../services/securityPresenceService.js";
import { getSwitchBotBridgeWorkerState, handleSwitchBotLocked, handleSwitchBotUnlocked, pollSwitchBotAndBridge, } from "../../services/switchBotSecurityBridge.js";
import { getLockProvider, resolveLockProviderId } from "../../providers/lock/index.js";
import { generateMockLockEvent, getLockProviderDashboardAsync, listFaceLockEvents, listLockEvents, } from "../../services/lockEventService.js";
import { getFamilyPresenceOverview, listChildArrivalNotifications, listPresenceUsers, } from "../../services/familyPresenceService.js";
import { isRealUnlockGuarded, verifySwitchBotDryRunConnection, } from "../../services/switchbotService.js";
...
securityAutomationRouter.post("/state/arm", (req, res) => {
...
securityAutomationRouter.post("/state/disarm", (req, res) => {
...
securityAutomationRouter.patch("/automation/rules/:id", (req, res) => {
    ...
    if (req.body?.delaySeconds !== undefined || req.body?.unknownDevicePolicy !== undefined) {
        saveAutomationSettings({
            ...
            unknownDevicePolicy: req.body.unknownDevicePolicy ?? getAutomationSettings().unknownDevicePolicy,
            autoArmEnabled: req.body.autoArmEnabled ?? config.switchbot.autoArmEnabled
                ? true
                : getAutomationSettings().autoArmEnabled,
            autoDisarmEnabled: req.body.autoDisarmEnabled ?? getAutomationSettings().autoDisarmEnabled,
            switchbotIntegrationEnabled: req.body.switchbotIntegrationEnabled ?? getAutomationSettings().switchbotIntegrationEnabled,
        });
    }
    res.json(updated);
});
...
securityAutomationRouter.post("/operations/real-confirm", (req, res) => {
    const body = req.body ?? {};
...
securityAutomationRouter.post("/operations/real-revoke", (req, res) => {
