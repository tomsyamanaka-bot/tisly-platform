...
import { countPushSubscriptions, sendWebPush, } from "../../notification/channels/web-push.js";
import { notifyChStateChanges } from "../../remote-test/remote-test-ch-notify.js";
import { processSecurityInputChanges, notifySecurityModeChange, } from "../../remote-test/security-demo-notify.js";
import { getSecurityDemoStatus, setSecurityMode, } from "../../remote-test/security-demo-state.js";
import { loadSecurityDemoConfig } from "../../remote-test/security-demo-config.js";
import { CHANNEL_COUNT, applySimulatedInputChange, consumePendingCommand, getDeviceStatus, getRemoteTestDebugInfo, getRemoteTestStatus, markPushResult, normalizeDeviceChStates, normalizeDeviceInputStates, queueChCommand, recordDeviceHeartbeat, recordHeartbeatDebug, recordWebAccess, } from "../../remote-test/remote-test-state.js";
...
export function requireRemoteTestToken(req, res, next) {
...
function trackWebAccess(req) {
    recordWebAccess(clientIp(req));
}
function safeSubscriptionCount() {
    try {
...
remoteTestRouter.post("/arm", async (req, res) => {
    ...
    if (changed)
        await notifySecurityModeChange(mode);
...
remoteTestRouter.post("/demo/intrusion-simulation", async (req, res) => {
    trackWebAccess(req);
    const change = { input: 1, from: "off", to: "on" };
...
function logHeartbeatRequest(req) {
...
function extractHeartbeatFirmware(req) {
    const fromQuery = typeof req.query.firmware === "string" ? req.query.firmware.trim() : "";
    if (fromQuery)
        return fromQuery;
    const body = req.body;
    if (body && typeof body === "object" && typeof body.firmware === "string") {
        return body.firmware.trim();
    }
...
function extractHeartbeatChStates(req) {
...
function extractHeartbeatInputStates(req) {
...
async function handleDeviceHeartbeat(req, res) {
    ...
    const { chChanges, inputChanges } = recordDeviceHeartbeat(firmware || undefined, chStates ?? undefined, inputStates ?? undefined);
