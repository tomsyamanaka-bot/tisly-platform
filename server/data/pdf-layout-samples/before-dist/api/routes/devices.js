...
function formatDevice(row) {
    ...
    return {
        ...
        label: row.label,
        tenantId: meta.tenant_id ?? config.defaultTenantId,
        siteId: meta.site_id ?? "default",
...
function findDeviceRow(param) {
    const db = getDatabase();
    const byUuid = db.prepare("SELECT * FROM devices WHERE id = ?").get(param);
    if (byUuid)
        return byUuid;
    return db.prepare("SELECT * FROM devices WHERE device_id = ?").get(param);
}
...
devicesRouter.post("/register", requireIngestOrDeviceAuth, (req, res) => {
    const { deviceId, deviceType, platform, label, userId, tenantId, siteId, metadata, sourceType, } = req.body;
    ...
    if (existing) {
        ...
        db.prepare(`UPDATE devices SET device_type = COALESCE(?, device_type), platform = COALESCE(?, platform),
       label = COALESCE(?, label), user_id = COALESCE(?, user_id), metadata_json = ?,
       updated_at = datetime('now') WHERE device_id = ?`).run(deviceType, platform, label, userId, JSON.stringify(merged), deviceId);
        const updated = findDeviceRow(deviceId);
        res.json({ ...formatDevice(updated), updated: true });
        return;
    }
    const id = uuid();
    db.prepare(`INSERT INTO devices (id, user_id, device_type, platform, device_id, label, metadata_json, heartbeat_status)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'unknown')`).run(id, userId ?? null, deviceType ?? "gateway", platform ?? "unknown", deviceId, label ?? deviceId, JSON.stringify(meta));
...
devicesRouter.patch("/:id", (req, res) => {
    ...
    const { deviceType, platform, label, siteId, tenantId, metadata, heartbeatStatus } = req.body;
    const meta = { ...parseMetadata(row.metadata_json) };
    if (siteId !== undefined)
        meta.site_id = siteId;
    ...
    db.prepare(`UPDATE devices SET
     ...
     WHERE id = ?`).run(deviceType, platform, label, heartbeatStatus, JSON.stringify(meta), row.id);
...
devicesRouter.post("/:id/test", async (req, res) => {
    ...
    const unified = normalizeUnifiedInput({
        ...
        source_type: meta.source_type ?? row.platform ?? "system",
...
devicesRouter.post("/:id/restart-request", (req, res) => {
    ...
    const db = getDatabase();
    db.prepare(`UPDATE devices SET metadata_json = ?, updated_at = datetime('now') WHERE id = ?`).run(JSON.stringify(meta), row.id);
    broadcast({
        type: "event",
        payload: {
            command: "restart",
...
devicesRouter.post("/:id/rotate-secret", requireAdminAuth, (req, res) => {
    const deviceId = String(req.params.id);
    const db = getDatabase();
    const cred = db
        .prepare(`SELECT site_id, tenant_id FROM device_credentials
    ...
    db.prepare(`UPDATE device_credentials SET secret_hash = ?, rotated_at = datetime('now')
     WHERE device_id = ?`).run(hashSecret(newSecret), deviceId);
    logAudit({
