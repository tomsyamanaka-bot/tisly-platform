...
function recordExport(req, meta) {
    const exportId = uuid();
    const generatedBy = req.admin?.username ?? req.admin?.userId ?? "system";
    const generatedAt = new Date().toISOString();
    getDatabase()
        .prepare(`INSERT INTO report_exports (id, tenant_id, site_id, format, generated_by, generated_at, metadata_json)
       VALUES (?, ?, ?, ?, ?, ?, ?)`)
        .run(exportId, meta.tenantId ?? config.defaultTenantId, meta.siteId ?? null, meta.format, generatedBy, generatedAt, JSON.stringify({ exportId, generatedBy, generatedAt }));
    logAudit({
...
reportsRouter.get("/operations", (req, res) => {
    ...
    const tenantId = req.query.tenantId ?? config.defaultTenantId;
    ...
    if (format === "csv") {
        const lines = [
            ...
            ...report.sites.map((s) => `${s.id},${s.name},${s.tenantId},${s.status}`),
        ];
    ...
    if (format === "pdf") {
        ...
        res.setHeader("Content-Disposition", `attachment; filename="tisly-operations-${exportId.slice(0, 8)}.pdf"`);
...
reportsRouter.get("/sales", (req, res) => {
    ...
    const recent = db
        .prepare(`SELECT event_type, severity, message, site_id, created_at FROM events ORDER BY created_at DESC LIMIT 20`)
        .all();
    res.json({
