...
import { createWebhook, deleteWebhook, getWebhook, listWebhooks, sendWebhookTest, } from "../../notification/channels/webhook.js";
import { requireAuth } from "../../auth/auth-middleware.js";
import { requireTenantMatch } from "../../auth/tenant-guard.js";
import { canAccessCustomer } from "../../auth/customer-auth.js";
import { logAudit } from "../../provisioning/audit-log.js";
import { listDeliveriesForCustomer, retryDeliveryById, } from "../../notification/webhook-retry-queue.js";
...
customerWebhooksRouter.post("/:customerCode/webhooks", requireAuth("admin"), requireTenantMatch("customerCode"), (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
    }
    if (!requireNotificationChannel(customer.plan, "webhook", res))
        return;
    ...
    if (!requireActiveContract(customer, res))
        return;
...
customerWebhooksRouter.get("/:customerCode/webhooks", requireAuth("manager"), requireTenantMatch("customerCode"), (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
...
customerWebhooksRouter.post("/:customerCode/webhooks/:id/test", requireAuth("admin"), requireTenantMatch("customerCode"), (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
...
customerWebhooksRouter.delete("/:customerCode/webhooks/:id", requireAuth("admin"), requireTenantMatch("customerCode"), (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
...
customerWebhooksRouter.get("/:customerCode/webhooks/deliveries", requireAuth("manager"), requireTenantMatch("customerCode"), (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
    ...
    if (!requireNotificationChannel(customer.plan, "webhook", res))
        return;
    const deliveries = listDeliveriesForCustomer(customer.customer_id, Number(req.query.limit ?? 50));
    res.json({ deliveries });
});
customerWebhooksRouter.post("/:customerCode/webhooks/deliveries/:id/retry", requireAuth("admin"), requireTenantMatch("customerCode"), (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
