...
customerReportsRouter.get("/:customerCode/reports/monthly", ...auth, (req, res) => {
    ...
    if (!requirePlanFeature(customer.plan, "sales_report", res))
        return;
    const report = buildCustomerMonthlyReport(customer.customer_code, req.admin?.username ?? "system");
    if (!report) {
...
customerReportsRouter.get("/:customerCode/reports/weekly", ...auth, (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
    ...
    if (!requirePlanFeature(customer.plan, "sales_report", res))
        return;
    const report = buildCustomerWeeklyReport(customer.customer_code, req.admin?.username ?? "system");
    if (!report) {
        res.status(404).json({ error: "Not found" });
        return;
    }
...
customerReportsRouter.post("/:customerCode/reports/export", requireAuth("manager"), requireTenantMatch("customerCode"), (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
...
customerReportsRouter.post("/:customerCode/reports/send-email", requireAuth("manager"), requireTenantMatch("customerCode"), async (req, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    if (!customer) {
        res.status(403).json({ error: "Denied" });
        return;
    ...
    const record = recordReportExport(report);
    const pdf = await renderReportPdf(report.html, `${customer.customer_name} レポート`, {}, { exportId: record.export_id, customerId: customer.customer_id, reportType });
    const to = req.body.to ?? process.env.ADMIN_EMAIL ?? "admin@tisly.jp";
