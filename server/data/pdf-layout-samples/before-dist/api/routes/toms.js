...
import { getTomsWorkflowState, listWorkflowHistory, transitionTomsWorkflow, businessStatusToToms, } from "../../toms/workflow-engine.js";
import { unifiedSearch } from "../../toms/unified-search.js";
import { listCustomerMaster, getCustomerMaster, upsertCustomerMaster, } from "../../toms/customer-master.js";
import { createAsset, getAsset, listAssets, listProjectAssets } from "../../toms/asset-master.js";
import { generateAssetQrPng, resolveAssetFromQr, recordQrScan, } from "../../toms/qr-management.js";
...
import { listProjectNotifications, acknowledgeProjectNotification, } from "../../toms/project-notifications.js";
import { listProjectMaintenance, createProjectMaintenance, closeProjectMaintenance, } from "../../toms/maintenance-flow.js";
import { compareDrawingVersions } from "../../toms/drawing-diff.js";
import { buildProjectFloorStack } from "../../toms/floor-stack-project.js";
import { listIntegrationRetryQueue, retryIntegrationQueueItem, cancelIntegrationRetry, getIntegrationRetryLog, } from "../../business/integration-retry-queue.js";
...
import { isLiveOpsMockPushEnabled, getMqttBridgeCertStatus, isMqttMockMode, listMqttBridgeLogs, } from "../../toms/mqtt-live-push-bridge.js";
...
import { aggregateAiFeedbackLearning, buildAiLearningCandidateHints, } from "../../toms/ai-feedback-learning.js";
...
tomsRouter.get("/customer-master/:customerId/kpi/csv", (req, res) => {
    ...
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="kpi-${req.params.customerId}.csv"`);
    res.send(csv);
});
tomsRouter.get("/hub/operations", (req, res) => {
...
tomsRouter.get("/hub/snapshot", (req, res) => {
    const code = (req.admin?.customerCode ?? "TOMS001").toUpperCase();
    res.json(buildHubOfflineSnapshot(code));
});
tomsRouter.post("/hub/snapshot/sync", (req, res) => {
    const code = (req.admin?.customerCode ?? "TOMS001").toUpperCase();
...
tomsRouter.post("/projects/:projectId/notifications/:notificationId/ack", (req, res) => {
    const n = acknowledgeProjectNotification(String(req.params.projectId), String(req.params.notificationId), req.admin?.username ?? "user");
    if (!n) {
...
tomsRouter.post("/projects/:projectId/maintenance/:caseId/close", (req, res) => {
    const c = closeProjectMaintenance(String(req.params.projectId), String(req.params.caseId), req.admin?.username);
    if (!c) {
...
tomsRouter.post("/projects/:projectId/timeline", (req, res) => {
...
tomsRouter.post("/projects/:projectId/workflow/transition", (req, res) => {
    try {
