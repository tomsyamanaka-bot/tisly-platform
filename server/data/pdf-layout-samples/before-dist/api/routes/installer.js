...
import { getCustomerInstallChecklist, completeChecklistItem, } from "../../installer/install-checklist.js";
import { runDeviceConnectivityTest, getMqttDiagnostic, runMqttRttTest, } from "../../installer/device-connectivity-test.js";
import { buildInstallCompletionReportHtml, buildInstallCompletionReportPdf, buildCompletionReportMeta, } from "../../installer/completion-report.js";
import { getDeviceLabelData } from "../../installer/device-label.js";
import { buildDevicesLabelsCsv, buildDeviceLabelSvg, buildTepraLabelsCsv, buildBrotherLabelsCsv, buildDeviceQrSvg, } from "../../installer/device-label-export.js";
...
import { processOfflineSync } from "../../installer/offline-sync.js";
import { startInstallSession, completeInstallSession, listInstallSessions, } from "../../installer/install-session.js";
import { isDryRunRequest, logDryRun } from "../../installer/dry-run.js";
import { issueDeviceCertificatePlaceholder, applyTrustToDeviceRow, } from "../../provisioning/device-certificates.js";
import { archiveFloorplanToQnap } from "../../qnap/floorplan-archive.js";
import { getFloorById } from "../../site-builder/floor-store.js";
import { registerDeviceCsr, issueDeviceCertFromCsr, revokeDeviceCert, getDeviceCertStatus, } from "../../provisioning/device-csr.js";
import { saveInstallPhoto, listInstallPhotos, deleteInstallPhoto, isAllowedInstallPhotoFile, getInstallPhotoUrl, } from "../../installer/install-photos.js";
import { getInstallDashboard } from "../../installer/install-dashboard.js";
import { evaluateFieldChecklist, updateFieldChecklistItem, getInstallerHomeCards, } from "../../installer/installer-field-checklist.js";
...
const installAuth = [requireAuth("installer"), requireTenantMatch("customerCode")];
const portalViewAuth = [requireAuth("viewer"), requireTenantMatch("customerCode")];
...
customerInstallerRouter.post("/:customerCode/devices/qr/create", ...installAuth, provisionRateLimit, (req, res) => {
    ...
    if (!requirePlanFeature(customer.plan, "customer_portal", res))
        return;
    const { deviceId, deviceType, serialNumber, ttlMinutes } = req.body;
...
customerInstallerRouter.post("/:customerCode/devices/qr/claim", ...installAuth, provisionRateLimit, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
    ...
    if (isDryRunRequest(req)) {
        logDryRun(customer.customer_code, "installer.qr.claim", body);
        logAudit({
            ...auditContextFromRequest(req),
            tenantId: customer.tenant_id ?? customer.customer_id,
...
customerInstallerRouter.post("/:customerCode/devices/nfc/claim", ...installAuth, provisionRateLimit, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
    ...
    if (!requirePlanFeature(customer.plan, "customer_portal", res))
        return;
    const { nfcUid, deviceId, deviceType, serialNumber, siteId, floorId, zoneId } = req.body;
    if (!nfcUid) {
...
customerInstallerRouter.get("/:customerCode/install/checklist", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/install/checklist/:item/complete", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
    ...
    try {
        const completed = completeChecklistItem(customer.customer_id, deviceId, item, req.admin?.username);
        logAudit({
            ...auditContextFromRequest(req),
            tenantId: customer.tenant_id ?? customer.customer_id,
            action: "installer.checklist.complete",
...
customerInstallerRouter.post("/:customerCode/devices/:id/test/heartbeat", ...installAuth, deviceTestRoute("heartbeat"));
customerInstallerRouter.post("/:customerCode/devices/:id/test/event", ...installAuth, deviceTestRoute("event"));
customerInstallerRouter.post("/:customerCode/devices/:id/test/relay", ...installAuth, deviceTestRoute("relay"));
customerInstallerRouter.post("/:customerCode/devices/:id/test/notification", ...installAuth, deviceTestRoute("notification"));
customerInstallerRouter.get("/:customerCode/install/mqtt/:deviceId", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/floorplans/:id/archive", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/install/photos/upload", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
    }
    const { deviceId, siteId, photoType, imageBase64, fileName } = req.body;
    if (!imageBase64) {
...
customerInstallerRouter.get("/:customerCode/install/photos", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.delete("/:customerCode/install/photos/:id", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/install/dashboard", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/install/home-cards", ...portalViewAuth, async (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/install/field-checklist", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.put("/:customerCode/install/field-checklist/:itemId", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
    ...
    try {
        const item = updateFieldChecklistItem(customer.customer_code, String(req.params.itemId), status);
        res.json(item);
    }
    catch (e) {
        res.status(400).json({ error: String(e) });
...
customerInstallerRouter.get("/:customerCode/devices/:id/label", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/install/completion-report", ...portalViewAuth, async (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/install/sync", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
    ...
    const report = processOfflineSync(customer.customer_id, entries ?? [], req.admin?.username);
    logAudit({
        ...auditContextFromRequest(req),
        tenantId: customer.tenant_id ?? customer.customer_id,
...
customerInstallerRouter.post("/:customerCode/install/session/start", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/install/session/complete", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/install/sessions", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/labels.csv", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/:id/label.svg", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/devices/:id/test/mqtt-rtt", ...installAuth, provisionRateLimit, async (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/devices/:id/csr", ...installAuth, provisionRateLimit, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/devices/:id/cert/issue", ...installAuth, provisionRateLimit, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/devices/:id/cert/revoke", ...installAuth, provisionRateLimit, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/:id/cert/status", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/:id/label.json", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/:id/cert-placeholder", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/install/field-live-status", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/:id/firmware-config", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.post("/:customerCode/devices/:id/test/live-mqtt", ...installAuth, provisionRateLimit, async (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/labels/tepra.csv", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/labels/brother.csv", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/devices/:id/qr.svg", ...portalViewAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(req.admin ? 403 : 404).json({ error: "Not found" });
        return;
...
customerInstallerRouter.get("/:customerCode/install/billing-check", ...portalViewAuth, (req, res) => {
