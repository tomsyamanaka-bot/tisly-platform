...
import { getTvFocusState, setTvFocusState, clearTvFocusState, } from "../../tv/tv-focus-state.js";
...
function formatTvDevice(row) {
    return {
        ...
        hasActivePairingCode: !!(row.pairing_code &&
...
function findTvRow(param) {
    const db = getDatabase();
    const byId = db.prepare("SELECT * FROM tv_devices WHERE id = ?").get(param);
    if (byId)
        return byId;
    return db.prepare("SELECT * FROM tv_devices WHERE device_id = ?").get(param);
}
function generatePairingCode() {
    return String(Math.floor(100000 + Math.random() * 900000));
}
function normalizeCertFingerprint(input) {
    if (!input?.trim())
        return null;
...
function expireStalePairingCodes() {
    getDatabase().prepare(`UPDATE tv_devices SET
       pairing_code = NULL,
       pairing_expires_at = NULL,
       status = CASE WHEN paired_at IS NULL THEN 'expired' ELSE status END,
       updated_at = datetime('now')
     WHERE pairing_expires_at IS NOT NULL AND pairing_expires_at < datetime('now')`).run();
}
function getPairingLock(deviceId, ip) {
    const db = getDatabase();
    const row = db
        .prepare(`SELECT attempt_count, locked_until FROM tv_pairing_attempts
...
function recordPairingFailure(deviceId, ip) {
    const db = getDatabase();
    const row = db
        .prepare(`SELECT attempt_count FROM tv_pairing_attempts WHERE device_id = ? AND ip_address = ?`)
        .get(deviceId, ip);
    const next = (row?.attempt_count ?? 0) + 1;
    const lockedUntil = next >= MAX_PAIRING_ATTEMPTS
        ? new Date(Date.now() + PAIRING_LOCK_MS).toISOString()
        : null;
    db.prepare(`INSERT INTO tv_pairing_attempts (device_id, ip_address, attempt_count, locked_until, updated_at)
     VALUES (?, ?, ?, ?, datetime('now'))
     ON CONFLICT(device_id, ip_address) DO UPDATE SET
       attempt_count = excluded.attempt_count,
       locked_until = excluded.locked_until,
       updated_at = datetime('now')`).run(deviceId, ip, next, lockedUntil);
    return next;
}
function clearPairingAttempts(deviceId, ip) {
    getDatabase()
...
tvRouter.post("/pairing/start", pairingStartLimiter, (req, res) => {
    ...
    const { deviceId, tvDeviceId, tenantId, displayName, } = req.body;
    const logicalId = (typeof tvDeviceId === "string" && tvDeviceId) ||
    ...
    if (existing) {
        db.prepare(`UPDATE tv_devices SET
         pairing_code = ?,
         pairing_expires_at = ?,
         ...
         status = 'pairing',
         updated_at = datetime('now')
       WHERE device_id = ?`).run(code, expiresAt, tenant, displayName ?? null, logicalId);
    ...
    const id = uuid();
    db.prepare(`INSERT INTO tv_devices (
       id, tenant_id, site_id, device_id, display_name,
       pairing_code, pairing_expires_at, status, settings_json
     ) VALUES (?, ?, NULL, ?, ?, ?, ?, 'pairing', ?)`).run(id, tenant, logicalId, displayName ?? logicalId, code, expiresAt, JSON.stringify({ display_mode: "dashboard", camera_mode: "placeholder" }));
...
tvRouter.post("/pairing/confirm", pairingConfirmLimiter, async (req, res) => {
    const { pairingCode, code, siteId, site_id, tvDeviceId, deviceId, displayName, tenantId, certificateFingerprint, deviceCertificate, } = req.body;
    ...
    const db = getDatabase();
    const row = db
        .prepare(`SELECT * FROM tv_devices
       WHERE pairing_code = ? AND pairing_expires_at IS NOT NULL
       ORDER BY updated_at DESC LIMIT 1`)
        .get(entered);
    if (!row) {
        const attempts = recordPairingFailure(tvDeviceId ?? deviceId ?? "unknown", (typeof req.ip === "string" ? req.ip : req.ip?.[0]) ?? "unknown");
        res.status(404).json({
            error: "無効なペアリングコードです",
    ...
    if (row.pairing_expires_at && new Date(row.pairing_expires_at).getTime() < Date.now()) {
        db.prepare(`UPDATE tv_devices SET pairing_code = NULL, pairing_expires_at = NULL, status = 'expired'
       WHERE id = ?`).run(row.id);
    ...
    const finalDeviceId = (typeof tvDeviceId === "string" && tvDeviceId) ||
        (typeof deviceId === "string" && deviceId) ||
        row.device_id;
    ...
    const certPlaceholder = deviceCertificate?.trim() ||
    ...
    db.prepare(`UPDATE tv_devices SET
       ...
       updated_at = datetime('now')
     WHERE id = ?`).run(finalDeviceId, site, tenant, displayName ?? null, pairedAt, pairedAt, certFp, certPlaceholder, row.id);
...
tvRouter.get("/config/:deviceId", (req, res) => {
    ...
    res.json({
        ...
        displayMode: settings.display_mode ?? "dashboard",
        cameraMode: settings.camera_mode ?? "placeholder",
        settings,
    });
});
...
tvRouter.patch("/devices/:id", (req, res) => {
    ...
    const { displayName, siteId, status, settings, lastSeen } = req.body;
    ...
    db.prepare(`UPDATE tv_devices SET
       ...
       updated_at = datetime('now')
     WHERE id = ?`).run(displayName, siteId, status, JSON.stringify(mergedSettings), lastSeen ? 1 : 0, row.id);
    res.json(formatTvDevice(findTvRow(row.id)));
});
tvRouter.post("/devices/:id/revoke", (req, res) => {
...
tvRouter.post("/devices/:id/test-alert", async (req, res) => {
    ...
    const db = getDatabase();
    db.prepare(`UPDATE tv_devices SET last_seen_at = datetime('now'), updated_at = datetime('now') WHERE id = ?`).run(row.id);
