...
import { buildDeviceProvisioningReportData, buildDeviceProvisioningReportHtml, buildDeviceProvisioningReportPdf, } from "../../installer/device-provisioning-report.js";
...
deviceCommissioningRouter.get("/:customerCode/devices/onboard/state", ...installAuth, (req, res) => {
...
deviceCommissioningRouter.post("/:customerCode/devices/onboard/create", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
    }
    const { deviceId, deviceType, serialNumber, siteId } = req.body;
    ...
    const existing = db
        ...
        .get(deviceId, customer.customer_id);
    const id = existing?.id ?? uuid();
    if (existing) {
        db.prepare(`UPDATE devices SET device_type = ?, serial_number = COALESCE(?, serial_number),
          device_status = 'COMMISSIONING', commissioning_status = 'draft', updated_at = datetime('now')
         WHERE id = ?`).run(deviceType, serialNumber ?? deviceId, id);
    }
    else {
        db.prepare(`INSERT INTO devices (id, customer_id, site_id, device_id, device_type, serial_number,
          commissioning_status, device_status, label)
         VALUES (?, ?, ?, ?, ?, ?, 'draft', 'COMMISSIONING', ?)`).run(id, customer.customer_id, siteId ?? null, deviceId, deviceType, serialNumber ?? deviceId, deviceId);
    }
...
deviceCommissioningRouter.post("/:customerCode/devices/onboard/qr", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
    }
    const { deviceId, deviceType, serialNumber } = req.body;
...
deviceCommissioningRouter.post("/:customerCode/devices/onboard/claim", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
...
deviceCommissioningRouter.get("/:customerCode/devices/:deviceId/onboard/firmware", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
...
deviceCommissioningRouter.post("/:customerCode/devices/:deviceId/onboard/heartbeat-check", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
    ...
    const db = getDatabase();
    const row = db
        .prepare(`SELECT last_heartbeat_at, device_status FROM devices WHERE customer_id = ? AND device_id = ?`)
        .get(customer.customer_id, deviceId);
    if (!row) {
        res.status(404).json({ error: "Device not found" });
        return;
    }
    const ok = row.device_status === "ONLINE" ||
...
deviceCommissioningRouter.post("/:customerCode/devices/onboard/complete", ...installAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
    ...
    const db = getDatabase();
    db.prepare(`UPDATE devices SET commissioning_status = 'completed', commissioned_at = datetime('now'),
        commissioned_by = ?, device_status = COALESCE(device_status, 'UNKNOWN')
       WHERE customer_id = ? AND device_id = ?`).run(req.admin?.username ?? null, customer.customer_id, deviceId);
    res.json({ ok: true, deviceId });
});
deviceCommissioningRouter.get("/:customerCode/devices/timeline", ...portalAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
...
deviceCommissioningRouter.get("/:customerCode/map/live", ...portalAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
...
deviceCommissioningRouter.get("/:customerCode/health", ...portalAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
    ...
    const tvCount = db
        .prepare(`SELECT COUNT(*) as c FROM devices WHERE customer_id = ? AND UPPER(device_type) = 'TV'`)
        .get(customer.customer_id).c;
    const certPending = db
        .prepare(`SELECT COUNT(*) as c FROM devices WHERE customer_id = ? AND COALESCE(cert_status, 'none') != 'issued'`)
        .get(customer.customer_id).c;
    res.json({
...
deviceCommissioningRouter.post("/:customerCode/simulator/event", ...portalAuth, (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
...
deviceCommissioningRouter.post("/:customerCode/notifications/test-all", ...portalAuth, async (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
...
deviceCommissioningRouter.get("/:customerCode/devices/:deviceId/provisioning-report", ...installAuth, async (req, res) => {
    const customer = resolveCustomer(req, String(req.params.customerCode));
    if (!customer) {
        res.status(404).json({ error: "Not found" });
        return;
    ...
    const data = buildDeviceProvisioningReportData(customer.customer_code, String(req.params.deviceId), req.admin?.username);
    if (format === "pdf") {
        const buf = await buildDeviceProvisioningReportPdf(data);
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename="provisioning-${data.deviceId}.pdf"`);
        res.send(buf);
        return;
    }
...
deviceCommissioningRouter.post("/:customerCode/heartbeat", ...portalAuth, (req, res) => {
