...
import { buildHubCards, buildPracticalHubCards, canAccessPwa, PWA_APP_CATALOG, showOpsPanelsForRole, } from "../../pwa/pwa-hub.js";
...
pwaHubRouter.get("/hub", requireAuth("viewer"), (req, res) => {
    const role = req.admin?.role ?? "viewer";
    const customerCode = (req.admin?.customerCode ?? "TOMS001").toUpperCase();
    const installerSurveyOptional = process.env.TISLY_INSTALLER_SURVEY_OPTIONAL === "true";
...
pwaHubRouter.get("/access/:pwaId", requireAuth("viewer"), (req, res) => {
    const pwaId = String(req.params.pwaId);
