...
async function buildFullHealthResponse() {
    ...
    const tvCount = db.prepare("SELECT COUNT(*) as c FROM tv_devices").get().c;
    const pairedTv = db
        .prepare("SELECT COUNT(*) as c FROM tv_devices WHERE paired_at IS NOT NULL AND status = 'paired'")
        .get().c;
    const tvPairing = db.prepare("SELECT COUNT(*) as c FROM tv_devices WHERE status = 'pairing'").get().c;
    ...
    try {
        const pendingNotifications = db
            .prepare(`SELECT COUNT(*) as c FROM notification_deliveries WHERE status = 'pending'`)
            .get().c;
        notificationQueue = {
    ...
    const mqttStatus = process.env.MQTT_SUBSCRIBER_ENABLED === "true"
        ? "ok"
