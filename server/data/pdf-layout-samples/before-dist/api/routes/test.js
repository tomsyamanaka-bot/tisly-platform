...
testRouter.post("/event", async (req, res) => {
    const unified = normalizeUnifiedInput(baseBody({
        event_id: req.body?.event_id ?? req.body?.eventId ?? `test-evt-${uuid()}`,
...
testRouter.post("/alarm", async (req, res) => {
    const unified = normalizeUnifiedInput(baseBody({
        event_id: req.body?.event_id ?? `test-alarm-${uuid()}`,
...
testRouter.post("/heartbeat", (req, res) => {
    const deviceId = req.body?.deviceId ?? "TEST-ESP-001";
...
testRouter.post("/recovery", async (req, res) => {
    const deviceId = req.body?.deviceId ?? "TEST-RP2350-001";
...
testRouter.post("/tv-alert", async (req, res) => {
    const unified = normalizeUnifiedInput(baseBody({
        event_id: req.body?.event_id ?? `test-tv-${uuid()}`,
        event_type: "tv_alert",
        severity: req.body?.severity ?? "alarm",
