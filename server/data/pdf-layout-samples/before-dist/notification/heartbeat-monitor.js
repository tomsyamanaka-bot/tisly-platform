...
const deviceLastState = new Map();
export function recordHeartbeat(deviceId, platform) {
    recordDeviceHeartbeat(deviceId, platform);
    deviceLastState.set(deviceId, "ok");
}
export function startHeartbeatMonitor(service) {
    const hb = getPlatformSetting("heartbeat");
    const warnSec = hb?.warnSec ?? config.heartbeat.warnSec;
    const alarmSec = hb?.alarmSec ?? config.heartbeat.alarmSec;
    setInterval(() => {
        const db = getDatabase();
        const devices = db
            .prepare(`SELECT device_id, last_heartbeat_at, heartbeat_status FROM devices WHERE last_heartbeat_at IS NOT NULL`)
            .all();
        const now = Date.now();
        for (const d of devices) {
            ...
            if (eventType && deviceLastState.get(d.device_id) !== nextStatus) {
                ...
                db.prepare("UPDATE devices SET heartbeat_status = ? WHERE device_id = ?").run(nextStatus, d.device_id);
                void service.processEvent({
                    deviceId: d.device_id,
                    eventType,
                    title: eventType === "heartbeat_alarm"
                        ? `通信断 — ${d.device_id}`
                        : `通信遅延 — ${d.device_id}`,
                ...
                db.prepare("UPDATE devices SET heartbeat_status = 'ok' WHERE device_id = ?").run(d.device_id);
            }
        }
    }, 10_000);
}
