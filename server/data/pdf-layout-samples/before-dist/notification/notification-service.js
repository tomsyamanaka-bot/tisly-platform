...
import { applyAiPriorityToEvent, handleEventRecovery, } from "../recovery/recovery-engine.js";
...
export class NotificationService {
    ...
    connectMqtt() {
        ...
        const subEnabled = process.env.MQTT_SUBSCRIBER_ENABLED === "true" || process.env.MQTT_MODE === "real";
        if (subEnabled) {
        ...
        this.mqttClient.on("connect", () => {
            ...
            this.mqttClient?.subscribe(config.mqtt.topicPrefix, (err) => {
                if (err)
                    ...
                    console.log("[MQTT] Subscribed:", config.mqtt.topicPrefix);
            });
        });
        this.mqttClient.on("message", (topic, buf) => {
            ...
            if (parsed.eventType === "heartbeat") {
                recordHeartbeat(parsed.deviceId, parsed.payload?.platform);
                return;
            }
    ...
    async processEvent(event, channels) {
        ...
        const priorityLabel = analytics.priority === "critical"
            ? "【重大】"
        ...
        if (flags.discord)
            results.push(await sendDiscord(payload));
        if (flags.email)
    ...
    getEnabledChannels() {
        const push = getPlatformSetting("push");
        const discord = getPlatformSetting("discord");
        const email = getPlatformSetting("email");
        return {
    ...
    logDelivery(event, payload, result) {
        const logId = uuid();
        const db = getDatabase();
        db.prepare(`INSERT INTO notification_logs (id, device_id, event_type, channel, title, body, payload_json, status, sent_at, error_message)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(logId, event.deviceId, event.eventType, result.channel, payload.title, payload.body, JSON.stringify(payload), result.success ? "sent" : "failed", result.success ? new Date().toISOString() : null, result.error ?? null);
        if (!result.success) {
