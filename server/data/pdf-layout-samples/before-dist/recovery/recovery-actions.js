...
export async function executeRecoveryAction(input) {
    const actionId = uuid();
    const reason = input.reason ?? "operator_manual";
    switch (input.action) {
        case "restart_device": {
            if (!input.deviceId)
                throw new Error("deviceId required for restart_device");
            ...
            db.prepare(`UPDATE devices SET metadata_json = ?, updated_at = datetime('now') WHERE device_id = ?`).run(JSON.stringify(meta), input.deviceId);
            broadcast({
                type: "event",
                payload: { action: "restart_device", deviceId: input.deviceId, actionId },
                at: new Date().toISOString(),
            });
            const incidentId = ensureIncident(input.deviceId, input.siteId ?? meta.site_id);
            appendTimeline(incidentId, "respond", "手動: デバイス再起動要求", actionId, input.deviceId);
            logAudit({
                siteId: input.siteId ?? meta.site_id,
        ...
        case "escalate": {
            if (!input.deviceId)
                throw new Error("deviceId required for escalate");
            const result = await runDeviceRecovery(input.deviceId, "heartbeat_lost");
            appendTimeline(ensureIncident(input.deviceId, input.siteId), "notify", `手動エスカレーション: ${reason}`, actionId, input.deviceId);
            logAudit({
