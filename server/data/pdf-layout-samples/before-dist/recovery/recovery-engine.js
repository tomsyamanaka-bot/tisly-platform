...
export function startRecoveryEngine() {
    if (recoveryEngineStarted)
        return;
    recoveryEngineStarted = true;
    console.log("[TiSLY Recovery] Engine started — rules:", DEVICE_RECOVERY_RULES.length);
}
export async function handleEventRecovery(event, meta) {
    const incidentId = ensureIncident(event.deviceId, event.siteId);
    const riskNote = meta?.riskScore != null
        ? `risk=${meta.riskScore} priority=${meta.priority ?? event.severity}`
        : event.severity ?? "info";
    appendTimeline(incidentId, event.eventType === "recovery" ? "recover" : "anomaly", event.title, riskNote, event.deviceId, event.siteId);
    if (event.eventType === "heartbeat_alarm" ||
...
export function applyAiPriorityToEvent(event, priority) {
    return { ...event, severity: priority };
}
export function getRecoveryOverview() {
    const db = getDatabase();
    const recentRuns = db
        .prepare(`SELECT id, rule_id as ruleId, device_id as deviceId, status, started_at as startedAt, completed_at as completedAt
