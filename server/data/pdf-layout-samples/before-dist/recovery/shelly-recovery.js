...
export async function executeShellyReboot(input) {
    const db = getDatabase();
    const device = db
        .prepare(`SELECT d.device_id, d.device_type, d.customer_id, c.customer_code
       FROM devices d
    ...
    db.prepare(`UPDATE recovery_runs SET steps_json = ? WHERE id = ?`).run(JSON.stringify(steps), record.actionId);
    logAudit({
...
export function listShellyRecoveryHistory(customerCode, limit = 50) {
    const db = getDatabase();
