...
const DEVICE_CREDENTIAL_COLUMNS = [
...
const DEVICE_COMMISSIONING_COLUMNS = [
    {
...
const INCIDENT_LOCATION_COLUMNS = [
...
function addColumnsIfMissing(database, table, columns) {
    const existing = new Set(database.prepare(`PRAGMA table_info(${table})`).all().map((r) => r.name));
...
export function runMigrations(database) {
    const existing = new Set(database.prepare("PRAGMA table_info(events)").all().map((r) => r.name));
    for (const col of EVENT_COLUMNS) {
        if (!existing.has(col.name)) {
            database.exec(col.ddl);
        }
    }
    const tvCols = new Set(database.prepare("PRAGMA table_info(tv_devices)").all().map((r) => r.name));
    for (const col of TV_DEVICE_COLUMNS) {
        if (!tvCols.has(col.name)) {
            database.exec(col.ddl);
        }
    }
    database.exec("CREATE INDEX IF NOT EXISTS idx_tv_devices_site ON tv_devices(site_id)");
    database.exec("CREATE INDEX IF NOT EXISTS idx_tv_devices_pairing ON tv_devices(pairing_code)");
    const auditCols = new Set(database.prepare("PRAGMA table_info(audit_logs)").all().map((r) => r.name));
    for (const col of AUDIT_COLUMNS) {
        if (!auditCols.has(col.name)) {
            database.exec(col.ddl);
        }
    }
    const credCols = new Set(database.prepare("PRAGMA table_info(device_credentials)").all().map((r) => r.name));
...
const BUSINESS_ESTIMATE_COLUMNS = [
...
/** 日程調整 PWA v1 — 現場不可日 */
function migrateSchedulePlannerV1(database) {
    database.exec(`
  ...
  `);
    const marker = database
        ...
        .get("migration:schedule_planner_v1");
    if (marker)
        return;
    database
...
function migratePracticalPwaV2(database) {
    database.exec(`
  ...
  `);
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:practical_pwa_v2");
    if (marker)
        return;
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
...
function migrateTomsEstimateStandardFormat(database) {
    ...
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:toms_estimate_standard_format_v1");
    if (marker)
        return;
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:toms_estimate_standard_format_v1", JSON.stringify({ at: new Date().toISOString() }));
}
/** 現調PWA — 写真並び順 */
function migrateSurveyPhotoSortOrder(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:survey_photo_sort_order");
    if (marker)
        return;
    ...
    const projects = database
        ...
        .all();
    const listStmt = database.prepare(`SELECT id FROM survey_photos WHERE project_id = ? ORDER BY created_at ASC, id ASC`);
    const updateStmt = database.prepare(`UPDATE survey_photos SET sort_order = ? WHERE id = ?`);
    for (const { project_id } of projects) {
        const rows = listStmt.all(project_id);
...
function migrateSurveyCustomerSiteV2(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:survey_customer_site_v2");
    if (marker)
        return;
...
const SURVEY_PROJECT_V1_COLUMNS = [
...
function migrateFieldSurveyPwaV1(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:field_survey_pwa_v1");
    if (marker)
        return;
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_survey_materials_project ON survey_materials(project_id)");
    database.exec(`
...
function migrateSurveyMaterialAntennaCategory(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:survey_material_antenna");
    if (marker)
        return;
...
function migrateFieldEstimatePwaV1(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:field_estimate_pwa_v1");
    if (marker)
        return;
    database.exec(`
...
function migratePhase2201(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase2201_real_data");
    if (marker)
        return;
    database.exec(`
...
function migratePhase261(database) {
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_customer_webhooks_customer ON customer_webhooks(customer_id)");
    database.exec("CREATE INDEX IF NOT EXISTS idx_customer_report_exports_customer ON customer_report_exports(customer_id)");
    migrateCustomerUsersInviteStatus(database);
    migratePhase281(database);
    migratePhase301(database);
    migratePhase321(database);
}
function migratePhase321(database) {
    addColumnsIfMissing(database, "devices", DEVICE_COLUMNS);
    ...
    for (const c of customers) {
        ...
        if (exists)
            continue;
        database
            .prepare(`INSERT INTO customer_recovery_rules (id, customer_id, name, condition_type, condition_device_type, action_type, action_target, enabled, priority)
         VALUES (?, ?, ?, ?, ?, ?, ?, 1, 10)`)
            .run(`seed-esp-offline-${c.customer_id}`, c.customer_id, "ESP Offline → Shelly reboot", "device_offline", "ESP", "shelly_reboot", "auto");
    }
    migratePhase341(database);
}
function migratePhase341(database) {
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_qr_tokens_customer ON qr_provisioning_tokens(customer_id)");
    database.exec("CREATE INDEX IF NOT EXISTS idx_qr_tokens_hash ON qr_provisioning_tokens(token_hash)");
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_install_photos_customer ON install_photos(customer_id)");
    migrateCustomerUsersInstallerRole(database);
    seedInstallerDemoUsers(database);
    migratePhase361(database);
}
function migratePhase361(database) {
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_install_sessions_customer ON install_sessions(customer_id)");
    migratePhase421(database);
}
const DEVICE_PHASE421_COLUMNS = [
    {
        name: "device_status",
...
function migratePhase421(database) {
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_device_timeline_customer ON device_timeline(customer_id, created_at)");
    database.exec("CREATE INDEX IF NOT EXISTS idx_device_timeline_device ON device_timeline(device_id, created_at)");
...
function migratePhase781(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase781_production_reliability");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase781_production_reliability", JSON.stringify({ at: new Date().toISOString() }));
    migratePhase1001(database);
    migratePhase1041(database);
    migratePhase1121(database);
    migratePhase1161(database);
...
function migratePhase1121(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase1121_field_deployment_rc1");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase1121_field_deployment_rc1", JSON.stringify({ at: new Date().toISOString(), phase: "1121-1160" }));
}
function migratePhase1321(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase1321_security_automation");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase1321_security_automation", JSON.stringify({ at: new Date().toISOString(), phase: "1321-1340" }));
}
function migratePhase1361(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase1361_lock_provider");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase1361_lock_provider", JSON.stringify({ at: new Date().toISOString(), phase: "1361-1380" }));
}
function migratePhase1621(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase1621_field_operations");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase1621_field_operations", JSON.stringify({ at: new Date().toISOString(), phase: "1621-1680" }));
}
function migratePhase1161(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase1161_field_deployment_rc2");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase1161_field_deployment_rc2", JSON.stringify({ at: new Date().toISOString(), phase: "1161-1200" }));
}
function migratePhase1041(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase1041_installer_field_checklist");
    if (marker)
        return;
    const cols = database.prepare(`PRAGMA table_info(deployment_checklist)`).all();
    if (!cols.some((c) => c.name === "installer_items_json")) {
        database.exec(`ALTER TABLE deployment_checklist ADD COLUMN installer_items_json TEXT NOT NULL DEFAULT '{}'`);
    }
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase1041_installer_field_checklist", JSON.stringify({ at: new Date().toISOString() }));
}
function migratePhase1001(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase1001_deployment_kit");
    if (marker)
        return;
    database.exec(`
...
function migratePhase621(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase621_toms_unified_workflow");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase621_toms_unified_workflow", JSON.stringify({ at: new Date().toISOString() }));
}
function migratePhase661(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase661_command_center");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    try {
        database.exec(`ALTER TABLE business_drawing_versions ADD COLUMN devices_json TEXT NOT NULL DEFAULT '[]'`);
    }
    catch {
        /* column may exist */
    }
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase661_command_center", JSON.stringify({ at: new Date().toISOString() }));
}
function migratePhase741(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase741_real_connection");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase741_real_connection", JSON.stringify({ at: new Date().toISOString() }));
}
function migratePhase701(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase701_live_operations");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase701_live_operations", JSON.stringify({ at: new Date().toISOString() }));
}
function migratePhase481(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase481_survey_maintenance_floor");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_survey_projects_customer ON survey_projects(customer_code)");
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_survey_photos_project ON survey_photos(project_id)");
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_maintenance_cases_customer ON maintenance_cases(customer_code)");
    database.exec(`
  ...
  `);
    database.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_pro_floor_layers_site_tier ON pro_floor_layers(site_id, tier)");
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_pro_map_pins_layer ON pro_map_pins(layer_id)");
    database
        .prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`)
        .run("migration:phase481_survey_maintenance_floor", JSON.stringify({ at: new Date().toISOString() }));
}
function migratePhase521(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase521_toms_business_pwa");
    if (marker)
        return;
    database.exec(`
...
function migratePhase601(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase601_drawing_pwa");
    if (marker)
        return;
    database.exec(`
...
function migratePhase561(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase561_business_production");
    if (marker)
        return;
    database.exec(`
...
function migratePhase541(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase541_business_workflow");
    if (marker)
        return;
    database.exec(`
...
function migratePhase501(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:phase501_survey_ai_sync");
    if (marker)
        return;
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_survey_ai_intakes_project ON survey_ai_intakes(project_id)");
    database.exec(`
...
function migrateCustomerUsersPwaRoles461(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:customer_users_pwa_roles_461");
    if (marker)
        return;
    try {
        database.exec(`
    ...
    `);
        const cols = new Set(database.prepare("PRAGMA table_info(customer_users)").all().map((r) => r.name));
        database.exec(`
    ...
    `);
        ...
        database.exec("CREATE INDEX IF NOT EXISTS idx_customer_users_customer ON customer_users(customer_id)");
        database.prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`).run("migration:customer_users_pwa_roles_461", JSON.stringify({ at: new Date().toISOString() }));
    }
    catch {
        /* CHECK may already include surveyor/maintenance */
    }
}
function seedPwaRoleDemoUsers(database) {
    const customers = database
    ...
    for (const c of customers) {
        ...
        if (!owner)
            continue;
        for (const role of ["surveyor", "maintenance"]) {
            const userId = `cu-${c.customer_code}-${role}`;
            const username = `${c.customer_code.toLowerCase()}.${role}`;
            database
                .prepare(`INSERT INTO customer_users (id, customer_id, username, password_hash, role, status)
           VALUES (?, ?, ?, ?, ?, 'active')
...
function seedInstallerDemoUsers(database) {
    const customers = database
        .prepare(`SELECT customer_id, customer_code FROM customers WHERE customer_code IN ('TOMS001', 'HOTEL001', 'PLANT001')`)
        .all();
    for (const c of customers) {
...
function migrateCustomerUsersInstallerRole(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:customer_users_installer_role");
    if (marker)
        return;
    try {
        database.exec(`
    ...
    `);
        ...
        database.exec("CREATE INDEX IF NOT EXISTS idx_customer_users_customer ON customer_users(customer_id)");
        database.prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`).run("migration:customer_users_installer_role", JSON.stringify({ at: new Date().toISOString() }));
    }
    catch {
        /* role CHECK may already include installer */
    }
}
function migratePhase301(database) {
    const customerCols = [
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_report_email_queue_pending ON report_email_queue(status, next_retry_at)");
...
function migratePhase281(database) {
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_customer_notification_rules_customer ON customer_notification_rules(customer_id)");
    database.exec(`
  ...
  `);
    database.exec("CREATE INDEX IF NOT EXISTS idx_webhook_delivery_pending ON webhook_delivery_logs(status, next_retry_at)");
}
function migrateCustomerUsersInviteStatus(database) {
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:customer_users_invited_status");
    if (marker)
        return;
    try {
        database.exec(`
    ...
    `);
        ...
        database.exec("CREATE INDEX IF NOT EXISTS idx_customer_users_customer ON customer_users(customer_id)");
        database.prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`).run("migration:customer_users_invited_status", JSON.stringify({ at: new Date().toISOString() }));
    }
    catch {
        /* already migrated */
    }
}
const CUSTOMER_USER_COLUMNS = [
...
function migrateCustomerUsersPhase241(database) {
    addColumnsIfMissing(database, "customer_users", CUSTOMER_USER_COLUMNS);
    const marker = database
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("migration:customer_users_owner_role");
    if (marker)
        return;
    try {
        database.exec(`
    ...
    `);
        ...
        database.exec("CREATE INDEX IF NOT EXISTS idx_customer_users_customer ON customer_users(customer_id)");
        database.prepare(`INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`).run("migration:customer_users_owner_role", JSON.stringify({ at: new Date().toISOString() }));
    }
    catch {
