...
import { createEstimate, createInvoiceFromEstimate, getBusinessProject, updateBusinessProject, } from "../business/business-store.js";
...
const SKIP = { skipTransitionCheck: true };
function lineItem(name, quantity, unit, unitPrice, costPrice) {
    return {
        id: uuid(),
        category: "other",
        name,
...
function paidDateForMonth(offset) {
    const d = new Date();
...
function insertDemoProject(input) {
    ...
    getDatabase()
        .prepare(`INSERT OR IGNORE INTO business_projects (
        ...
        .run(input.id, input.projectNo, input.customerId, input.customerName, input.title, input.address, input.status, input.paidDate ?? null, qnapBasePath);
}
function bootstrapPaidProject(projectId, paidDate) {
...
export function seedDemoKpiProjects() {
    const db = getDatabase();
    let created = 0;
    for (let i = 0; i < DEMO_PACK_CUSTOMERS.length; i++) {
        const c = DEMO_PACK_CUSTOMERS[i];
        const bcId = `BCU-DEMO-${c.customerCode}`;
        db.prepare(`INSERT OR IGNORE INTO business_customers (
        id, name, type, contact_name, phone, email, address, pricing_tier_id,
        payment_terms, invoice_closing_day, created_at, updated_at
      ) VALUES (?, ?, 'corporate', ?, '03-0000-0000', ?, ?, NULL, '月末締め翌月末払い', 31, datetime('now'), datetime('now'))`).run(bcId, c.customerName, c.customerName, `demo@${c.customerCode.toLowerCase()}.local`, c.address);
...
export function clearDemoKpiProjects() {
    const db = getDatabase();
    const rows = db
    ...
    for (const r of rows) {
        if (r.estimate_id)
            db.prepare(`DELETE FROM business_estimates WHERE id = ?`).run(r.estimate_id);
        if (r.invoice_id)
            db.prepare(`DELETE FROM business_invoices WHERE id = ?`).run(r.invoice_id);
