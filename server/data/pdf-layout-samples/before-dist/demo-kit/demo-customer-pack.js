...
import { ensureDemoDevice, ensureDemoSite, getCustomerByCode, upsertCustomer, } from "../customer/customer-store.js";
import { appendDeviceTimeline } from "../device/device-timeline.js";
import { v4 as uuid } from "uuid";
import fs from "fs";
import path from "path";
import { createSurveyProject, saveSurveyPhoto, saveSurveyDrawing, } from "../survey/survey-store.js";
/** 1x1 PNG — デモ写真プレースホルダ */
...
export const DEMO_PACK_CUSTOMERS = [
    {
...
function surveyProjectId(code) {
    return `SVY-DEMO-${code}`;
}
export function seedDemoCustomerAccounts() {
    ...
    for (const c of DEMO_PACK_CUSTOMERS) {
        for (const role of [
            ...
            db.prepare(`INSERT INTO customer_users (id, customer_id, username, password_hash, role, status)
         VALUES (?, ?, ?, ?, ?, 'active')
         ON CONFLICT(customer_id, username) DO NOTHING`).run(`cu-${c.customerCode}-${role}`, c.customerId, `${c.customerCode.toLowerCase()}.${role}`, hash, role);
        }
    }
}
function seedDevicesForCustomer(c) {
    const proDevices = [
        {
    ...
    for (const d of extra) {
       ...
       ON CONFLICT(id) DO UPDATE SET customer_id=excluded.customer_id, site_id=excluded.site_id,
         device_status='ONLINE', last_seen=excluded.last_seen, metadata_json=excluded.metadata_json`).run(d.id, d.deviceType, d.deviceId, d.label, c.customerId, c.siteId, `SN-${d.deviceId}`, now, now, JSON.stringify({ demo_kit: true, customer_code: c.customerCode, site_id: c.siteId }));
    }
    for (const deviceId of [`${c.customerCode}-ESP-01`, `${c.customerCode}-ESP-02`, `${c.customerCode}-GW-01`]) {
        db.prepare(`UPDATE devices SET device_status = 'ONLINE', metadata_json = ? WHERE device_id = ?`).run(JSON.stringify({ demo_kit: true, customer_code: c.customerCode, site_id: c.siteId }), deviceId);
    }
    const camId = `cam-${c.customerCode}-01`;
    db.prepare(`INSERT OR IGNORE INTO camera_devices (id, customer_id, site_id, device_id, channel, camera_name, camera_group)
     VALUES (?, ?, ?, ?, 1, ?, 'demo')`).run(camId, c.customerId, c.siteId, `${c.customerCode}-CAM-01`, `${c.customerName} カメラ`);
}
function ensureSurveyProjectId(c) {
    const db = getDatabase();
    const row = db
...
function seedSurveyAssets(c) {
    ...
    const drawingCount = getDatabase()
...
function seedNotificationHistory(c) {
    ...
    for (const s of samples) {
        db.prepare(`INSERT INTO notification_logs (id, device_id, event_type, channel, title, body, payload_json, status, sent_at)
       VALUES (?, ?, ?, 'web_push', ?, ?, ?, 'sent', datetime('now', '-' || abs(random() % 72) || ' hours'))`).run(uuid(), `${c.customerCode}-ESP-01`, s.type, s.title, s.body, JSON.stringify({ demo_kit: true, customerCode: c.customerCode }));
    }
...
export function ensureDemoCustomerPack() {
    const seeded = [];
    for (const c of DEMO_PACK_CUSTOMERS) {
        upsertCustomer({
            customerId: c.customerId,
...
export function getDemoPackStatus() {
    const db = getDatabase();
    return DEMO_PACK_CUSTOMERS.map((c) => {
        ...
        const siteCount = db.prepare(`SELECT COUNT(*) as c FROM sites WHERE customer_id = ?`).get(customer.customer_id).c;
        const deviceCount = db.prepare(`SELECT COUNT(*) as c FROM devices WHERE customer_id = ?`).get(customer.customer_id).c;
        const svy = db
            .prepare(`SELECT project_id FROM survey_projects WHERE customer_code = ? LIMIT 1`)
            .get(c.customerCode);
        const photoCount = svy
            ? db.prepare(`SELECT COUNT(*) as c FROM survey_photos WHERE project_id = ?`).get(svy.project_id).c
            : 0;
...
export function clearDemoPackSurveyUploads() {
    for (const c of DEMO_PACK_CUSTOMERS) {
