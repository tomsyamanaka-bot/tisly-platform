...
export function hasDemoTimelineSeed() {
    const row = getDatabase()
        .prepare(`SELECT COUNT(*) as c FROM events WHERE source_type = 'demo-kit' AND payload_json LIKE ?`)
        .get(`%${DEMO_TIMELINE_MARKER}%`);
    return row.c >= 8;
}
...
export function seedDemoTimeline30Days() {
    ...
    for (const c of DEMO_PACK_CUSTOMERS) {
        ...
        for (const e of dayEvents) {
            ...
            db.prepare(`INSERT INTO events (id, event_id, tenant_id, site_id, device_id, source_type, event_type, severity, message, title, payload_json, created_at)
         VALUES (?, ?, ?, ?, ?, 'demo-kit', ?, ?, ?, ?, ?, ?)`).run(uuid(), `DEMO-TL-${c.customerCode}-${e.day}-${e.eventType}`, tenantId, siteId, e.deviceId, e.eventType, e.severity, e.message, e.title, JSON.stringify({ demo_kit: true, demo_timeline: DEMO_TIMELINE_MARKER, customerCode: c.customerCode }), createdAt);
            events += 1;
        ...
        if (bizProject) {
            ...
            for (const be of bizEvents) {
                ...
                db.prepare(`UPDATE business_project_timeline SET created_at = ? WHERE id = ?`).run(daysAgoIso(be.day), entry.id);
                projectEntries += 1;
            }
        }
    }
...
export function clearDemoTimeline() {
    const db = getDatabase();
    db.prepare(`DELETE FROM events WHERE source_type = 'demo-kit'`).run();
    for (const c of DEMO_PACK_CUSTOMERS) {
        db.prepare(`DELETE FROM device_timeline WHERE actor = 'demo-kit' AND customer_id = ?`).run(c.customerId);
        db.prepare(`DELETE FROM business_project_timeline WHERE actor = 'demo-kit' AND project_id LIKE ?`).run(`BIZ-DEMO-${c.customerCode}%`);
    }
}
