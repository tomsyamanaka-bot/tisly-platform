import { createAiEstimatePlaceholder, listSurveyPhotos, } from "../survey/survey-store.js";
import { createEstimate, getBusinessProject, updateBusinessProject, } from "../business/business-store.js";
import { getCustomerByCode } from "../customer/customer-store.js";
import { getDatabase } from "../db/database.js";
import { DEMO_KPI_PREFIX } from "./demo-kpi-seed.js";
import { v4 as uuid } from "uuid";
const SKIP = { skipTransitionCheck: true };
export function getDemoSurveyProjectId(customerCode) {
    const row = getDatabase()
        .prepare(`SELECT project_id FROM survey_projects WHERE customer_code = ? ORDER BY created_at LIMIT 1`)
        .get(customerCode.toUpperCase());
...
export function runDemoAiEstimateFlow(customerCode = "TOMS001") {
    ...
    if (!surveyProjectId)
        throw new Error("Survey project not found — run demo reset first");
    const photos = listSurveyPhotos(surveyProjectId);
    const photoIds = photos.slice(0, 3).map((p) => p.id);
    if (photoIds.length === 0)
        throw new Error("No survey photos — run demo reset first");
    ...
    if (project) {
        ...
        if (!project.estimateId) {
            ...
            estimateId = est.id;
            updateBusinessProject(businessProjectId, { estimateId: est.id, status: "estimate_created" }, SKIP);
        }
        else {
