...
export function ensureDemoFloorMapsForAllCustomers() {
    ...
    for (const c of DEMO_PACK_CUSTOMERS) {
        ...
        for (const t of PIN_LAYOUT) {
            ...
            if (!floorRow) {
                ...
                db.prepare(`INSERT OR IGNORE INTO floors (id, site_id, name, order_no, created_at, updated_at)
           VALUES (?, ?, ?, ?, datetime('now'), datetime('now'))`).run(fid, site.id, t.name, t.order);
                floorRow = { id: fid };
            }
            const layerId = `layer-${c.customerCode}-${t.tier}`;
            db.prepare(`INSERT OR IGNORE INTO pro_floor_layers (id, customer_id, site_id, tier, display_name, sort_order, floor_id, image_kind)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'svg')`).run(layerId, c.customerId, site.id, t.tier, t.name, t.order, floorRow.id);
            db.prepare(`UPDATE pro_floor_layers SET image_path = COALESCE(image_path, ?), image_kind = 'svg'
         WHERE id = ?`).run(`/assets/demo-floor/${t.tier}.svg`, layerId);
            layers += 1;
            const existingPins = db.prepare(`SELECT COUNT(*) as c FROM pro_map_pins WHERE layer_id = ?`).get(layerId).c;
...
export function clearDemoFloorMaps() {
    const db = getDatabase();
    for (const c of DEMO_PACK_CUSTOMERS) {
...
export function getDemoFloorMapStatus() {
    const db = getDatabase();
    return DEMO_PACK_CUSTOMERS.map((c) => ({
        customerCode: c.customerCode,
        tiers: ["perimeter", "1f", "2f"].map((tier) => {
            const layerId = `layer-${c.customerCode}-${tier}`;
            const pinCount = db.prepare(`SELECT COUNT(*) as c FROM pro_map_pins WHERE layer_id = ?`).get(layerId).c;
