...
export function buildDemoEstimateHtml(type) {
    const m = ESTIMATE_META[type];
    const rows = m.lines
        .map((l) => `<tr><td>${l.name}</td><td class="num">${l.qty}</td><td>${l.unit}</td><td class="num">${fmtYen(l.price)}</td><td class="num">${fmtYen(l.qty * l.price)}</td></tr>`)
        .join("");
    return `<!DOCTYPE html>
<html lang="ja">
<head>
...
export function getDemoEstimateMeta(type) {
    const m = ESTIMATE_META[type];
    return {
        type,
        title: m.title,
...
export function listDemoEstimateTypes() {
