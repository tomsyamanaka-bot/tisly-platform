...
export function checkProductionEnv(source = process.env) {
    ...
    const get = (key, fallback = "") => (source[key] ?? fallback).trim();
...
export function hasBlockingEnvErrors(source = process.env) {
    return checkProductionEnv(source).some((i) => i.level === "error");
}
/** 起動前にコンソールへ warning/error を出力（NODE_ENV=test ではスキップ） */
export function logProductionEnvWarnings(source = process.env) {
