...
import { buildInputNotifyPayload, loadSecurityDemoConfig, } from "./security-demo-config.js";
import { markPushResult, recordSecurityNotification } from "./remote-test-state.js";
import { getSecurityMode, isArmed, recordInputSecurityEvent, } from "./security-demo-state.js";
const REMOTE_TEST_USER_ID = "remote-test";
function persistSecurityLog(eventType, title, body, payload, result) {
    const logId = uuid();
    ...
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(logId, REMOTE_TEST_USER_ID, cfg.deviceId, eventType, "web_push", title, body, JSON.stringify(payload), result.success ? "sent" : "failed", result.success ? new Date().toISOString() : null, result.error ?? null);
    }
    catch {
...
async function sendSecurityPush(payload) {
    try {
...
export async function processSecurityInputChanges(changes) {
    for (const change of changes) {
        const event = recordInputSecurityEvent(change);
        console.log(`[security-demo] input event DI${change.input} ${change.from}→${change.to} armed=${isArmed()} type=${event.type}`);
        if (!isArmed()) {
...
export async function notifySecurityModeChange(mode) {
    const cfg = loadSecurityDemoConfig();
...
export function getSecurityModeLabel() {
