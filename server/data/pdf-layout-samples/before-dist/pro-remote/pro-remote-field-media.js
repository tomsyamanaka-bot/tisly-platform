...
export function buildFieldMediaByTier(customerCode) {
    ...
    const surveyRow = db
        .prepare(`SELECT sp.project_id FROM survey_projects sp
    ...
    const bizRow = db
        .prepare(`SELECT id, construction_photos_json FROM business_projects
       WHERE customer_id = ? ORDER BY created_at DESC LIMIT 1`)
        .get(customerCode);
    if (bizRow?.construction_photos_json) {
        try {
            const photos = JSON.parse(bizRow.construction_photos_json);
