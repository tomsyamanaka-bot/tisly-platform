...
import { isTotpEnabled, verifyTotpCode, adminRequires2fa, } from "./totp.js";
export function isAuthConfigured() {
    return Boolean(config.auth.jwtSecret);
}
export function isAdminPasswordConfigured() {
    return Boolean(config.auth.jwtSecret && isValidScryptPasswordHash(config.auth.adminPasswordHash));
}
export function loginAdmin(username, password, meta) {
...
export function recordFailedLogin(username, meta) {
    const db = getDatabase();
    const id = `failed-login:${username}`;
    const row = db
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get(id);
    const prev = row ? JSON.parse(row.value_json) : { count: 0 };
    const next = { count: prev.count + 1, lastAt: new Date().toISOString(), ip: meta?.ip };
    db.prepare(`INSERT INTO platform_settings (key, value_json, updated_at)
     VALUES (?, ?, datetime('now'))
     ON CONFLICT(key) DO UPDATE SET value_json = excluded.value_json, updated_at = datetime('now')`).run(id, JSON.stringify(next));
}
...
export function getFailedLoginCount(username) {
    ...
    if (!row)
        return 0;
    return JSON.parse(row.value_json).count ?? 0;
}
export function recordIngestError(deviceId) {
    const db = getDatabase();
    ...
    db.prepare(`INSERT INTO platform_settings (key, value_json, updated_at)
     VALUES (?, ?, datetime('now'))
     ON CONFLICT(key) DO UPDATE SET value_json = excluded.value_json, updated_at = datetime('now')`).run(key, JSON.stringify({
        count: prev.count + 1,
        lastAt: new Date().toISOString(),
        lastDeviceId: deviceId ?? prev.lastDeviceId,
    }));
}
export function getIngestErrorCount() {
    const row = getDatabase()
        .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
        .get("security:ingest-errors");
    if (!row)
        return 0;
    return JSON.parse(row.value_json).count ?? 0;
}
export function logoutAdmin(userId, meta) {
