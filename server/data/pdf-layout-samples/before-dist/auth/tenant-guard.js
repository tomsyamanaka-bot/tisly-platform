 ...
 */
export function assertTenantScope(session, tenantOrCustomerId) {
    if (!tenantOrCustomerId)
        return true;
...
export function tenantQueryGuard(req, res, next) {
    if (!req.admin) {
        next();
        return;
    }
    ...
    const qTenant = (req.query.tenant_id ?? req.query.tenantId);
    const qCustomer = (req.query.customer_id ?? req.query.customerId);
