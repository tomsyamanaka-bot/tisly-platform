...
function resolveDeviceSecret(deviceId, headerSecret) {
    if (headerSecret && verifyDeviceSecret(deviceId, headerSecret)) {
        return headerSecret;
    }
    const row = getDatabase()
        .prepare(`SELECT secret_encrypted FROM device_credentials
...
export function verifyEventSignature(deviceId, timestamp, rawBody, signature, headerSecret) {
...
export function requireEventSignature(req, res, next) {
