...
export function buildDevicesLabelsCsv(customerId) {
    const customer = getCustomerById(customerId);
    const customerName = customer?.customer_name ?? customer?.customer_code ?? "";
    const rows = getDatabase()
        .prepare(`SELECT d.device_id, d.serial_number, d.label, d.device_type, d.site_id, d.zone_id,
       ...
       WHERE d.customer_id = ? ORDER BY d.device_id`)
        .all(customerId);
    const header = "device_id,serial,customer,site,zone,label,device_type,install_date,cert_status,trust_level,qr_payload";
...
export function getDeviceLabelJson(customerId, customerCode, deviceId) {
    const data = getDeviceLabelData(customerId, deviceId);
    const row = getDatabase()
        .prepare(`SELECT commissioned_at FROM devices WHERE device_id = ? AND customer_id = ?`)
...
export function buildDeviceLabelSvg(customerId, deviceId) {
    const data = getDeviceLabelData(customerId, deviceId);
...
export function buildTepraLabelsCsv(customerId) {
    const rows = getDatabase()
...
export function buildBrotherLabelsCsv(customerId) {
    const rows = getDatabase()
        .prepare(`SELECT device_id FROM devices WHERE customer_id = ? ORDER BY device_id`)
        .all(customerId);
...
export function buildDeviceQrSvg(customerId, deviceId) {
    const data = getDeviceLabelData(customerId, deviceId);
    return `<?xml version="1.0" encoding="UTF-8"?>
