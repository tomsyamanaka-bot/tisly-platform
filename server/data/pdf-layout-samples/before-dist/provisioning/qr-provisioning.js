...
export function createQrProvisioning(input) {
    ...
    const id = uuid();
    db.prepare(`INSERT INTO qr_provisioning_tokens (id, customer_id, device_id, device_type, serial_number, token_hash, expires_at, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, input.customerId, input.deviceId, input.deviceType, input.serialNumber, tokenHash, expiresAt, input.createdBy ?? null);
...
export function claimQrProvisioning(input) {
    const db = getDatabase();
    const tokenHash = hashSecret(input.provisioningToken);
    const row = db
        .prepare(`SELECT id, customer_id, device_id, device_type, serial_number, expires_at, used_at
    ...
    const existing = db
        ...
        .get(input.deviceId, input.customerId);
    let deviceRowId;
    if (existing) {
        ...
        db.prepare(`UPDATE devices SET
         ...
         updated_at = ?
       WHERE id = ?`).run(now, input.claimedBy ?? null, tokenHash, input.serialNumber, input.deviceType, input.siteId ?? null, input.floorId ?? null, input.zoneId ?? null, now, deviceRowId);
    }
    else {
        deviceRowId = uuid();
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'claimed', ?, ?, ?, ?, ?)`).run(deviceRowId, input.customerId, input.siteId ?? null, input.floorId ?? null, input.zoneId ?? null, input.deviceType, input.deviceId, input.deviceId, input.serialNumber, now, input.claimedBy ?? null, tokenHash, now, now);
    }
    logAudit({
        tenantId: input.customerId,
