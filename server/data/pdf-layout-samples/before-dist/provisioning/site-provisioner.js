...
import { getTemplate } from "./site-templates.js";
...
export function ensureTenant(tenantId, name) {
    const db = getDatabase();
    const existing = db.prepare("SELECT id FROM tenants WHERE id = ?").get(tenantId);
    if (!existing) {
        db.prepare(`INSERT INTO tenants (id, name, slug) VALUES (?, ?, ?)`).run(tenantId, name ?? tenantId, tenantId);
    }
}
export function createSite(input) {
    ...
    const db = getDatabase();
    db.prepare(`INSERT INTO sites (id, tenant_id, name, template_id, site_type, address, lat, lng, dashboard_json, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')`).run(siteId, tenantId, input.name, template?.id ?? null, template?.siteType ?? "custom", input.address ?? null, input.lat ?? null, input.lng ?? null, JSON.stringify(dashboard));
    const zoneMap = new Map();
    const zones = [];
    const zoneDefs = template?.zones ?? [{ name: "デフォルト", zoneType: "default" }];
    zoneDefs.forEach((z, i) => {
        const zoneId = `${siteId}-zone-${i}`;
        db.prepare(`INSERT INTO zones (id, site_id, name, zone_type, sort_order) VALUES (?, ?, ?, ?, ?)`).run(zoneId, siteId, z.name, z.zoneType, i);
    ...
    for (const d of deviceDefs) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, 'unknown')`).run(devUuid, d.kind, d.platform, deviceId, `${d.labelPrefix} — ${input.name}`, JSON.stringify(meta));
...
export function getSiteDetail(siteId) {
    ...
    return {
        ...
        status: site.status,
        dashboard: site.dashboard_json ? JSON.parse(site.dashboard_json) : {},
        zones,
        devices: siteDevices,
