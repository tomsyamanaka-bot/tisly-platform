...
import { customerUrls, getCustomerByCode, listDevicesForCustomer, } from "../customer/customer-store.js";
...
export function buildCustomerHandoverPackage(customerCode) {
    ...
    const bizProject = getDatabase()
        .prepare(`SELECT id FROM business_projects WHERE customer_id = ? ORDER BY created_at DESC LIMIT 1`)
        .get(customerCode);
