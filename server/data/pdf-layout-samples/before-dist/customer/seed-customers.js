...
import { ensureDemoDevice, ensureDemoSite, upsertCustomer, } from "./customer-store.js";
...
export function ensureDemo001Kit() {
    ...
    for (const z of zones) {
        db.prepare(`INSERT INTO zones (id, site_id, name) VALUES (?, ?, ?) ON CONFLICT(id) DO NOTHING`).run(z.id, siteId, z.name);
    }
    const kitDevices = [
    ...
    for (const d of kitDevices) {
        db.prepare(`INSERT INTO devices (id, customer_id, site_id, zone_id, device_id, device_type, platform, label,
        device_status, commissioning_status, metadata_json)
       VALUES (?, ?, ?, ?, ?, 'ESP32', 'demo-kit', ?, 'COMMISSIONING', 'claimed', ?)
       ON CONFLICT(id) DO NOTHING`).run(d.id, customerId, siteId, d.zone, d.deviceId, d.label, JSON.stringify({ demo_kit: true }));
    }
    for (const role of ["owner", "admin", "installer", "viewer"]) {
        db.prepare(`INSERT INTO customer_users (id, customer_id, username, password_hash, role, status)
       VALUES (?, ?, ?, ?, ?, 'active')
       ON CONFLICT(customer_id, username) DO NOTHING`).run(`cu-DEMO001-${role}`, customerId, `demo001.${role}`, hash, role);
    }
}
export function seedProRemoteCustomers() {
    const db = getDatabase();
    ...
    for (const c of DEMO_CUSTOMERS) {
        ...
        for (const role of [
            ...
            db.prepare(`INSERT INTO customer_users (id, customer_id, username, password_hash, role, status)
         VALUES (?, ?, ?, ?, ?, 'active')
         ON CONFLICT(customer_id, username) DO NOTHING`).run(userId, c.customerId, `${c.customerCode.toLowerCase()}.${role}`, hash, role);
        }
    }
    ensureDemo001Kit();
    ensureProFloorLayersSeed();
