...
export function addMaintenanceReplacementParts(input) {
    ...
    for (const p of input.parts) {
        ...
        db.prepare(`INSERT INTO maintenance_replacement_parts
       (part_id, report_id, customer_code, part_name, quantity, unit, notes, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(partId, input.reportId, input.customerCode.toUpperCase(), p.partName, p.quantity ?? 1, p.unit ?? "個", p.notes ?? null, now);
        created.push({
            partId,
...
export function listMaintenanceReplacementParts(reportId) {
    const rows = getDatabase()
        .prepare(`SELECT * FROM maintenance_replacement_parts WHERE report_id = ? ORDER BY created_at ASC`)
        .all(reportId);
    return rows.map((r) => ({
