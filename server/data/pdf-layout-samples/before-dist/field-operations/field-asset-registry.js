...
export const FIELD_ASSET_KINDS = ["ESP", "Shelly", "Camera", "SwitchBot", "Sensor"];
...
function normalizeKind(kind, label) {
    ...
    if (k.includes("esp") || l.includes("esp"))
        return "ESP";
    if (k.includes("shelly") || l.includes("shelly"))
        return "Shelly";
    if (k.includes("camera") || k.includes("cam") || l.includes("カメラ"))
        return "Camera";
    if (k.includes("switchbot") || l.includes("switchbot"))
        return "SwitchBot";
    if (k.includes("sensor") || l.includes("センサ"))
        return "Sensor";
    return kind;
}
export function listFieldAssets(filters) {
    ...
    params.push(limit);
    const qrRows = getDatabase().prepare(sql).all(...params);
    ...
    if (filters?.health) {
        ...
        result = result.filter((r) => r.healthCode === h ||
            r.health === h ||
...
export function summarizeFieldAssets(customerCode) {
    ...
    for (const a of assets) {
        if (a.healthCode === "normal")
            summary.normal++;
        else if (a.healthCode === "warning")
            summary.warning++;
        else
            summary.abnormal++;
    }
