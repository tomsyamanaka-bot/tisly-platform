...
function pipelineFromBusinessStatus(status, hasInvoice, hasPaid) {
    const p = {
        survey: "pending",
        estimate: "pending",
...
function pipelineFromSurveyStatus(status) {
    const p = {
        survey: "active",
        estimate: "pending",
        construction: "pending",
...
export function listProjectsV1(opts) {
    const limit = opts?.limit ?? 80;
    const items = [];
    const db = getDatabase();
    const bizRows = db
        .prepare(`SELECT id, project_no, title, customer_name, address, status, invoice_id, paid_date, updated_at
    ...
    const surveyRows = db
        .prepare(`SELECT project_id, project_no, site_name, customer_name, address, workflow_status, updated_at
...
export function getProjectDetailV1(id, source) {
    const db = getDatabase();
    if (source !== "survey") {
        const row = db
            .prepare(`SELECT id, project_no, title, customer_name, address, phone, status, invoice_id, paid_date, updated_at,
        ...
        if (row) {
            ...
            if (!timeline.length) {
                let surveyDate = null;
                let constructionDate = null;
                try {
                    const ss = JSON.parse(String(row.survey_schedule_json ?? "{}"));
                    surveyDate = ss.date ?? null;
                }
                catch { /* */ }
                try {
                    const cs = JSON.parse(String(row.construction_schedule_json ?? "{}"));
                    constructionDate = cs.date ?? null;
                }
                catch { /* */ }
                if (surveyDate)
                    timeline.push({ date: formatTimelineDate(surveyDate), label: "現調", detail: "" });
                if (constructionDate)
                    timeline.push({ date: formatTimelineDate(constructionDate), label: "工事", detail: "" });
                if (hasInvoice)
                    timeline.push({ date: "—", label: "請求", detail: "" });
    ...
    const survey = db
        .prepare(`SELECT project_id, project_no, site_name, customer_name, address, phone, assignee, workflow_status, survey_date, updated_at
