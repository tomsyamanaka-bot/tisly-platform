...
export function upsertProjectCaseChain(input) {
    ...
    if (input.surveyProjectId) {
        const existing = db
            ...
            .get(input.surveyProjectId);
        if (existing) {
            if (input.businessProjectId) {
                db.prepare(`UPDATE project_case_chain SET business_project_id = ?, updated_at = datetime('now') WHERE id = ?`).run(input.businessProjectId, existing.id);
            }
    ...
    if (input.businessProjectId) {
        const existing = db
            ...
            .get(input.businessProjectId);
        if (existing)
            return rowToChain(existing);
    }
    const id = uuid();
    const caseNo = input.caseNo ?? generateCaseNo();
    db.prepare(`INSERT INTO project_case_chain (id, case_no, survey_project_id, business_project_id, customer_code, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).run(id, caseNo, input.surveyProjectId ?? null, input.businessProjectId ?? null, input.customerCode ?? null);
    const row = db.prepare(`SELECT * FROM project_case_chain WHERE id = ?`).get(id);
    return rowToChain(row);
}
export function getCaseChainBySurveyId(surveyProjectId) {
    const row = getDatabase()
        .prepare(`SELECT * FROM project_case_chain WHERE survey_project_id = ?`)
        .get(surveyProjectId);
    return row ? rowToChain(row) : null;
}
export function getCaseChainByBusinessId(businessProjectId) {
    const row = getDatabase()
        .prepare(`SELECT * FROM project_case_chain WHERE business_project_id = ?`)
