...
import { evaluateSwitchBotLockedEvent, evaluateSwitchBotUnlockedEvent, getSecurityState, } from "./securityAutomationService.js";
import { getLockProvider } from "../providers/lock/index.js";
export async function handleSwitchBotLockStatusChanged(status) {
...
export function handleSwitchBotLocked(status) {
...
export function handleSwitchBotUnlocked(status) {
    const lockStatus = status ?? {
        deviceId: "event",
        lockState: "unlocked",
        mode: "mock",
...
function broadcastSecurityFocus(event) {
...
function recordLockStateTimeline(prev, next, status) {
    if (prev === next)
        return;
    const eventType = next === "locked"
...
let lastKnownLockState = null;
let lastLoggedLockState = null;
const workerState = {
    lastPollAt: null,
    lastLockState: null,
    lastUnlockAt: null,
...
export function getSwitchBotBridgeWorkerState() {
    return { ...workerState };
}
export async function pollSwitchBotAndBridge(deviceId) {
...
export function resetSwitchBotBridgeState() {
