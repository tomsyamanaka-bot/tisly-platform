 ...
 */
import { createFamilyNotification, listChildArrivalNotifications, listFamilyNotifications, listPresenceUsers, } from "../lock-provider/lock-provider-store.js";
...
function resolveUserRole(userName) {
    ...
    if (user)
        return user.role;
    if (userName === "不明" || userName === "Unknown")
        return "unknown";
    return "guest";
}
export function processFamilyUnlockEvent(event) {
    const role = resolveUserRole(event.userName);
    const method = event.eventType === "face_unlock"
        ? "SESAME Face"
...
async function dispatchFamilyNotification(kind, body) {
