 ...
 */
import { createSecurityEventLogEntry, getAutomationRules, getAutomationSettings, getSecurityState as loadSecurityState, listSecurityEventLogs, saveSecurityState, updateAutomationRule, } from "../security-automation/security-automation-store.js";
...
let pendingArmStartedAt = null;
export function getSecurityState() {
    return loadSecurityState();
}
export function setSecurityMode(mode, reason, source, changedBy = "system") {
...
export function createSecurityEventLog(event) {
    return createSecurityEventLogEntry(event);
}
export function clearPendingArmTimer() {
    if (pendingArmTimer) {
...
export function evaluateSwitchBotLockedEvent(lockStatus) {
    ...
    if (!isRealModeConfirmed()) {
        ...
        void dispatchSecurityEventNotification("real_command_rejected", "real モード — confirmed 未設定のため自動警戒ONを拒否");
        return before;
    }
    const lockedRule = getAutomationRules().find((r) => r.enabled && r.triggerType === "switchbot_locked");
    ...
    if (!presenceCheck.canArm || !gate.canArm) {
        const reason = presenceCheck.reason ?? gate.armReasons.join("; ") ?? "Auto arm blocked";
        ...
        if (gate.unknownDeviceDetected) {
            void dispatchSecurityEventNotification("unknown_device_blocked", "unknown 端末により自動警戒ONをブロック");
        }
        else {
    ...
    const pending = setSecurityMode("pending_arm", "Pending arm started — waiting for all-away confirmation", "switchbot", "automation");
    createSecurityEventLogEntry({
...
export function confirmPendingArmCheck() {
    const settings = getAutomationSettings();
    ...
    if (!presenceCheck.canArm || !gate.canArm) {
        ...
        void dispatchSecurityEventNotification("auto_arm_skipped", reason);
        return setSecurityMode("disarmed", reason, "presence", "automation");
    }
    createSecurityEventLogEntry({
    ...
    return setSecurityMode("armed", "Auto armed by SwitchBot locked + all away", "switchbot", "automation");
}
export function startPendingArmCheck() {
    const settings = getAutomationSettings();
    const lockedRule = getAutomationRules().find((r) => r.enabled && r.triggerType === "switchbot_locked");
    const delaySeconds = lockedRule?.delaySeconds ?? settings.delaySeconds;
    clearPendingArmTimer();
...
export function evaluateSwitchBotUnlockedEvent(lockStatus) {
    const settings = getAutomationSettings();
    const before = loadSecurityState();
    const gate = evaluateSecurityArmGate(lockStatus);
    createSecurityEventLogEntry({
    ...
    if (!isRealModeConfirmed()) {
        ...
        void dispatchSecurityEventNotification("real_command_rejected", "real モード — confirmed 未設定のため自動警戒OFFを拒否");
        return before;
    }
    if (!gate.canDisarm) {
        createSecurityEventLogEntry({
        ...
        void dispatchSecurityEventNotification("auto_disarm_skipped", gate.disarmReasons.join("; ") || "自動警戒OFFをスキップ");
        return before;
    }
    createSecurityEventLogEntry({
...
export function evaluatePresenceOnlyChange(_deviceId, _status) {
    return loadSecurityState();
}
export function getLastSecurityEventLog() {
