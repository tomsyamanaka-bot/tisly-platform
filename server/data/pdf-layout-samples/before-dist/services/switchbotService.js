...
export function getSwitchBotMode() {
    return config.switchbot.mode;
}
export function resetSwitchBotMockState(state = "unlocked") {
    mockLockState = state;
    if (state === "unlocked") {
        lastUnlockAt = new Date().toISOString();
    }
}
export function getSwitchBotLastUnlockAt() {
    return lastUnlockAt;
}
/** 同期参照用 — mock/dryRun(無認証) のインメモリ状態 */
export function getSwitchBotLockStateSync() {
...
export function hasSwitchBotCredentials() {
...
async function switchBotFetch(path, init) {
...
function normalizeLockValue(lock) {
    if (lock === "locked")
        return "locked";
    if (lock === "unlocked")
        return "unlocked";
...
async function fetchRealLockStatus(deviceId) {
    const mode = getSwitchBotMode();
...
export async function getSwitchBotLockStatus(deviceId) {
    const mode = getSwitchBotMode();
...
export async function verifySwitchBotDryRunConnection() {
    const mode = getSwitchBotMode();
    if (mode === "mock") {
        return {
            ok: true,
...
export async function sendSwitchBotLockCommand(deviceId, command, confirmed) {
    const mode = getSwitchBotMode();
...
export async function lockSwitchBot(deviceId, confirmed = false) {
    const id = deviceId || config.switchbot.lockDeviceId || "mock-lock-001";
    return sendSwitchBotLockCommand(id, "lock", confirmed);
}
export async function unlockSwitchBot(deviceId, confirmed = false) {
    const id = deviceId || config.switchbot.lockDeviceId || "mock-lock-001";
    return sendSwitchBotLockCommand(id, "unlock", confirmed);
}
/** Release Gate — real unlock が confirmed guard なしで動くか検証用 */
export function isRealUnlockGuarded() {
    return true;
}
