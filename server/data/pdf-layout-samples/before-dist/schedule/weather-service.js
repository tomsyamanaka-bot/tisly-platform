...
function mockSlots(date, location) {
...
function hourIndexForPeriod(period) {
    if (period === "morning")
        return 8;
    if (period === "afternoon")
        return 14;
...
async function fetchOpenMeteoSlots(date, lat, lon) {
    ...
    return defs.map((d) => {
        ...
        if (idx < 0)
            idx = Math.min(times.length - 1, targetHour);
...
export async function fetchDayWeather(date, opts) {
