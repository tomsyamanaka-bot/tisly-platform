...
function mockDurationMin(seed) {
    let h = 0;
    for (let i = 0; i < seed.length; i++)
        h = (h * 31 + seed.charCodeAt(i)) >>> 0;
    return 12 + (h % 25);
}
function parseJsonSchedule(raw) {
    if (!raw)
        return null;
    try {
...
function loadProjectStopsForDate(date) {
    const stops = [];
    const db = getDatabase();
    const businessRows = db
        .prepare(`SELECT id, title, address, customer_name, survey_schedule_json, construction_schedule_json
       FROM business_projects ORDER BY updated_at DESC LIMIT 100`)
        .all();
    for (const row of businessRows) {
        const survey = parseJsonSchedule(row.survey_schedule_json);
        const construction = parseJsonSchedule(row.construction_schedule_json);
    ...
    const surveyRows = db
        .prepare(`SELECT project_id, site_name, address, assignee, survey_date
...
function constructionStopsFromEvents(events) {
...
function projectStopsToRoute(stops) {
    return stops.map((s) => ({
        time: s.time,
        title: s.title,
        address: s.address,
...
export function buildDayDispatch(date, events) {
    const projectStops = loadProjectStopsForDate(date);
    let stops = projectStops.length
        ? projectStopsToRoute(projectStops)
        : constructionStopsFromEvents(events);
    if (!stops.length)
        return null;
    const driver = projectStops.find((s) => s.assignee)?.assignee ?? DEFAULT_DRIVER;
    const legs = [];
...
export function travelHintsForSite(date, siteTitle, dispatch) {
    if (!dispatch) {
        return {
            date,
            siteTitle,
