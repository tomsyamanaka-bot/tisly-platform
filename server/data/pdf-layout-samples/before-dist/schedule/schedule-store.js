...
import { hasCachedCalendarEvents, listCachedCalendarEvents, } from "./schedule-calendar-store.js";
...
export function calcAvailability(eventCount, unavailable) {
    if (unavailable) {
        return { stars: "現場不可", label: "現場不可", level: "unavailable" };
    }
    if (eventCount === 0)
        return { stars: "★★★★★", label: "空いています", level: "free" };
    if (eventCount <= 2)
        return { stars: "★★★★☆", label: "余裕あり", level: "light" };
    if (eventCount <= 4)
        return { stars: "★★☆☆☆", label: "やや混雑", level: "busy" };
    return { stars: "満車", label: "いっぱい", level: "full" };
}
function rowToUnavailable(r) {
    return {
        id: String(r.id),
...
export function listUnavailableDays(startDate, endDate) {
    const rows = getDatabase()
        .prepare(`SELECT id, unavailable_date, reason, created_at, updated_at
...
function buildDayCard(date, events, unavailableMap) {
...
function buildSummary(days) {
    ...
    for (const day of days) {
        if (day.unavailable)
            unavailableDays += 1;
        if (day.availability.level === "free")
            freeDays += 1;
        totalEvents += day.eventCount;
        for (const ev of day.events) {
            if (ev.category === "construction")
                constructionCount += 1;
            else if (ev.category === "office")
                officeCount += 1;
            else if (ev.category === "family")
                familyCount += 1;
        }
    }
    return {
...
export async function getScheduleWeekView(offsetRaw) {
...
export async function getScheduleThreeWeekView(offsetRaw) {
    const offset = parseOffset(offsetRaw);
    const startDate = weekStartFromOffset(offset);
...
export async function getScheduleDayDetail(dateRaw, opts) {
...
export async function getScheduleMonthView(yearRaw, monthRaw) {
...
export async function getScheduleSummary(range, offsetRaw) {
    if (range === "week") {
...
export function createUnavailableDay(input) {
    ...
    const existing = getDatabase()
        ...
        .get(date);
    if (existing)
        throw new Error("unavailable day already exists for this date");
    const id = uuid();
    const now = new Date().toISOString();
    getDatabase()
        .prepare(`INSERT INTO schedule_unavailable_days (id, unavailable_date, reason, created_at, updated_at)
...
export function updateUnavailableDay(id, patch) {
...
export function deleteUnavailableDay(id) {
