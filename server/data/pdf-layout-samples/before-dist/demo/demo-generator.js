...
import { DEMO_SITES, DEMO_ZONES, DEVICE_TEMPLATES, } from "./demo-sites.js";
...
export function buildVirtualDevices() {
    const list = [];
...
export function seedDemoDevices(db = getDatabase()) {
    const devices = buildVirtualDevices();
    const insert = db.prepare(`INSERT INTO devices (id, device_type, platform, device_id, label, last_heartbeat_at, heartbeat_status, metadata_json)
    ...
    for (const d of devices) {
        const id = d.deviceId;
        insert.run(id, d.kind, d.platform, d.deviceId, d.label, JSON.stringify({ site_id: d.siteId, site_name: d.siteName, zone: d.zone, demo: true }));
        count++;
    }
    return count;
}
...
export function createRandomUnifiedEvent() {
    const device = pickRandomDevice();
    const scenario = pickWeighted(EVENT_SCENARIOS);
    const sourceType = device.kind === "esp32"
        ? "esp32"
        : device.kind === "rp2350"
            ? "rp2350"
            : device.kind === "plc"
...
export async function emitDemoEvent(unified) {
    ...
    const hbStatus = u.severity === "critical" || u.severity === "alarm"
        ? "alarm"
        : u.severity === "warning"
            ? "warning"
            : "ok";
    db.prepare(`UPDATE devices SET last_heartbeat_at = datetime('now'), heartbeat_status = ? WHERE device_id = ?`).run(hbStatus, u.device_id);
    const wsType = u.severity === "alarm" || u.severity === "critical" ? "alarm" : "event";
    broadcast({
        type: wsType,
        payload: {
            ...u,
