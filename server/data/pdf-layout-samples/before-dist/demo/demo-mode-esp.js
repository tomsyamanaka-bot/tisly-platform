...
let demoTimer = null;
export function seedDemoModeVirtualEsp(customerId, siteId) {
    ...
    for (const z of zones) {
        ...
        db.prepare(`INSERT INTO devices (id, customer_id, site_id, device_id, device_type, platform, label,
       ...
       ON CONFLICT(id) DO UPDATE SET
         ...
         updated_at = datetime('now')`).run(id, customerId, siteId, z.deviceId, z.label, JSON.stringify({ zone: z.zone, demo_virtual: true }));
...
const SIM_EVENTS = [
    { eventType: "motion", title: "人感", severity: "warning" },
    { eventType: "window_open", title: "窓開", severity: "warning" },
    { eventType: "intrusion", title: "侵入", severity: "alarm" },
    { eventType: "recovery", title: "復旧", severity: "info" },
    { eventType: "power_loss", title: "停電", severity: "critical" },
    { eventType: "comm_loss", title: "通信断", severity: "alarm" },
];
export function startDemoModeVirtualEspRunner() {
    if (!config.demoMode || demoTimer)
        return;
    const db = getDatabase();
    const demo = db
    ...
    if (demo) {
        ...
        if (site)
            seedDemoModeVirtualEsp(demo.customer_id, site.id);
    }
...
export function stopDemoModeVirtualEspRunner() {
    if (demoTimer) {
        clearInterval(demoTimer);
        demoTimer = null;
    }
}
export function emitSimulatorEvent(customerCode, scenario, deviceId) {
    const map = {
