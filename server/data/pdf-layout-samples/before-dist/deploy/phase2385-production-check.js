...
import { GMAIL_TEST_ATTACHMENT_FILENAME, } from "../notification/smtp-gmail.js";
...
function analyzeGmailSmtpModule(source) {
    const pdfAttachmentEnabled = source.includes("buildGmailTestAttachmentPdf") &&
        source.includes("GMAIL_TEST_ATTACHMENT_FILENAME") &&
        source.includes("attachments:");
    const testEmailBodySafe = !source.includes("`SMTP: ${status.maskedCredentials}`") &&
...
export function buildPhase2385ProductionCheck(env = process.env) {
    ...
    const smtpGmailDist = readText(path.join(getServerRoot(), "dist/notification/smtp-gmail.js")) ?? "";
    ...
    const distRuntimeAligned = distPdfAttachmentEnabled &&
    ...
    return {
        ...
        operationalReady: criticalOk && base2384.adminPasswordStatus === "GREEN" && base2384.gmailSendVerified,
