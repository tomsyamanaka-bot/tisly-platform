...
export function buildDeployPreflight(source = process.env) {
    const getEnv = (key, fallback = "") => (source[key] ?? fallback).trim();
    const categories = [];
    ...
    const adminMissing = [];
    if (!adminUser)
        adminMissing.push("ADMIN_USERNAME");
    if (!adminHash)
        adminMissing.push("ADMIN_PASSWORD_HASH");
    categories.push({
        id: "ADMIN",
        label: "ADMIN",
        status: adminMissing.length === 0 ? "ok" : "missing",
        configured: [adminUser && "ADMIN_USERNAME", adminHash && "ADMIN_PASSWORD_HASH"].filter(Boolean),
    ...
    if (mqttMode === "real") {
        if (!getEnv("MQTT_URL"))
            mqttMissing.push("MQTT_URL");
        if (getEnv("MQTT_SUBSCRIBER_ENABLED") !== "true")
            mqttMissing.push("MQTT_SUBSCRIBER_ENABLED");
    }
    categories.push({
        id: "MQTT",
        label: "MQTT",
    ...
    const qnapMissing = [];
    if (qnapMode === "real" && !getEnv("QNAP_HOST"))
        qnapMissing.push("QNAP_HOST");
    if (qnapUpload === "real" && !getEnv("QNAP_WEBDAV_URL"))
        qnapMissing.push("QNAP_WEBDAV_URL");
    categories.push({
        id: "QNAP",
        label: "QNAP",
    ...
    if (gmailMode === "real" || (googleOAuth && gmailMode !== "mock")) {
        if (!googleOAuth)
            gmailMissing.push("GOOGLE_OAUTH_ENABLED");
        if (googleOAuth && !getEnv("GOOGLE_CLIENT_ID"))
            gmailMissing.push("GOOGLE_CLIENT_ID");
    }
    categories.push({
        id: "GMAIL",
        label: "GMAIL",
    ...
    categories.push({
        ...
        message: shellyMissing.length === 0
            ? `SHELLY_MODE=${shellyMode}`
            : `real 不足: ${shellyMissing.join(", ")}`,
    });
    const switchbotMode = getEnv("SWITCHBOT_MODE", "mock").toLowerCase();
    const switchbotMissing = [];
    if (switchbotMode === "real") {
        if (!getEnv("SWITCHBOT_TOKEN"))
            switchbotMissing.push("SWITCHBOT_TOKEN");
        if (!getEnv("SWITCHBOT_SECRET"))
            switchbotMissing.push("SWITCHBOT_SECRET");
        if (!getEnv("SWITCHBOT_LOCK_DEVICE_ID"))
            switchbotMissing.push("SWITCHBOT_LOCK_DEVICE_ID");
    }
    categories.push({
        id: "SwitchBot",
        label: "SwitchBot",
        ...
        message: switchbotMissing.length === 0
    ...
    const ready = flatMissing.length === 0 &&
