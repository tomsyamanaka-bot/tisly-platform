...
export function isInsecureAdminPasswordHash(hash, env = process.env) {
...
export function resolveAdminPasswordStatus(hash, env = process.env) {
    const value = normalizeStoredPasswordHash(hash ?? env.ADMIN_PASSWORD_HASH ?? "");
    if (!value) {
        return {
            ok: false,
...
export function buildPhase2381ProductionCheck(env = process.env) {
    ...
    const checks = [
        ...
        },
        {
            ...
            status: notificationsRoute.includes("/test-email") && notificationsRoute.includes("requireAdminAuth")
                ? "GREEN"
                : "RED",
        },
        {
            ...
            status: recoveryDoc.includes("hash:admin-password") && recoveryDoc.includes("systemctl restart")
                ? "GREEN"
                : "RED",
        },
        {
    ...
    const criticalOk = checks
        .filter((c) => ["admin-password-hash-runtime", "hash-admin-password-script", "reject-plaintext-hash", "test-email-admin-auth"].includes(c.id))
        .every((c) => c.ok);
    return {
        phase: "2381-2400",
