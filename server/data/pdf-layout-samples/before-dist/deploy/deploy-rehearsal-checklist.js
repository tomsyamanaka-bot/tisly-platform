...
function isSecretSet(val, minLen = 8) {
    return val.length >= minLen && !INSECURE_VALUES.has(val);
}
function buildEnvChecklist(source = process.env) {
    const get = (key) => envValue(source, key);
...
export const VPS_ENV_PREP_ONE_BLOCK = [
    "cd /opt/tisly/server",
...
export const VPS_PRODUCTION_START_ONE_BLOCK = [
    "cd /opt/tisly",
    "bash scripts/vps-production-start.sh",
];
/** 手動起動（スクリプト不可時の代替 · 秘密値なし） */
export const VPS_PRODUCTION_START_MANUAL_BLOCK = [
    "cd /opt/tisly/server",
...
export const VPS_GIT_PULL_START_ONE_BLOCK = [
...
export const VPS_PRODUCTION_VERIFY_ONE_BLOCK = [
    "systemctl is-active tisly-server",
...
export const VPS_CHECKLIST_STATUS_VERIFY_BLOCK = [
...
export const VPS_BROWSER_SMOKE_ONE_BLOCK = [
    "BASE=https://tisly.jp",
...
/** 失敗時の分岐表 */
export const VPS_FAILURE_BRANCHES = [
    {
        id: "env_missing",
...
export const VPS_DEPLOY_COMMAND_STEPS = [
    {
        id: "ssh",
        title: "SSH 接続",
    ...
    },
    {
        ...
        note: "起動方式は systemd（公式）。PM2 は代替のみ。scripts/vps-production-start.sh が build · release:gate · db:init · systemd · nginx まで実行します。",
    },
    {
...
export function buildProductionStartInfo() {
    return {
        method: "systemd",
        ...
        note: "正式 .env テンプレートは server/.env.production.example。.env 完了後に bash scripts/vps-production-start.sh を 1 回実行。",
    };
}
export const PRODUCTION_BROWSER_TEST_URLS = [
...
export function buildProductionVerificationGuide() {
    const failureTable = VPS_FAILURE_BRANCHES.map((b) => `| ${b.symptom} | ${b.likelyCause} | ${b.checkCommands.join(" · ")} | ${b.fix} |`).join("\n");
    return {
        phase: "1921-1960",
        title: "Production Launch Verification & Browser Test",
        sectionA_urls: PRODUCTION_BROWSER_TEST_URLS.map((u) => `https://tisly.jp${u.path} — ${u.label}`),
        sectionB_success: [
...
export function buildProductionLaunchGuide() {
...
export function buildDeployRehearsalChecklist(source = process.env) {
    ...
    const securityReady = security.verdict === "READY" &&
    ...
    const rehearsalReady = githubReady &&
        buildReady &&
        testReady &&
        gateReady &&
        securityReady;
