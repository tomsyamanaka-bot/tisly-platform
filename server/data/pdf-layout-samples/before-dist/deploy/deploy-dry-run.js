...
import { buildRc2CheckUrls, RC2_PRODUCTION_ROUTES, } from "../config/production-routes.js";
import { buildPwaPublishAudit, NGINX_REQUIRED_ROUTE_PREFIXES, PWA_AUDIT_SPECS, } from "../pwa/pwa-publish-audit.js";
...
const SECRET_DIFF_PATTERNS = SECRET_ENV_KEYS.map((key) => ({
    key,
...
function diffTouchesRealEnvProduction(diffText) {
    return /^\+\+\+ b\/(?:server\/)?\.env\.production(?:\r)?$/m.test(diffText);
}
export function checkSecretLeakInGitDiff(diffText = gitDiffText()) {
    const findings = [];
    ...
    for (const { key, pattern, allowEmpty } of SECRET_DIFF_PATTERNS) {
        ...
        if (allowEmpty && (value === "" || TEMPLATE_VALUES.has(value)))
            continue;
        if (TEMPLATE_VALUES.has(value))
            continue;
        if (value.length > 0) {
...
export function checkUploadsGitignore(gitignore = readGitignore()) {
...
function checkEnvProductionExample() {
    const checks = [];
    ...
    const adminLine = content
        ...
        ?.trim();
    const templateOk = jwtLine === "JWT_SECRET=" && adminLine === "ADMIN_PASSWORD_HASH=";
    checks.push({
...
function checkPwaUrlConsistency(pwaAudit) {
...
function checkProductionUrlClean() {
...
function checkPwaAssets(pwaAudit) {
    const pwaItems = pwaAudit.pwAs.filter((p) => p.isPwa);
    const notReady = pwaItems.filter((p) => p.status === "not_ready");
    const missingSw = pwaItems.filter((p) => p.missingItems.some((m) => m.startsWith("serviceWorker:")));
    const missingManifest = pwaItems.filter((p) => p.missingItems.some((m) => m.startsWith("manifest:")));
    const missingIcons = pwaItems.some((p) => p.missingItems.some((m) => m.startsWith("icons/")));
    if (notReady.length > 0 || missingSw.length > 0 || missingIcons) {
        const parts = [];
        if (missingSw.length)
            parts.push(`SW 不足 ${missingSw.length}`);
        if (missingManifest.length)
            parts.push(`manifest 不足 ${missingManifest.length}`);
        if (missingIcons)
            parts.push("icons 不足");
        return {
            id: "pwa_assets",
...
export function readLastDryRunAt() {
    try {
        if (!fs.existsSync(LAST_DRY_RUN_FILE))
            return null;
        const raw = JSON.parse(fs.readFileSync(LAST_DRY_RUN_FILE, "utf8"));
...
export function writeLastDryRunReport(report) {
...
function dryRunAuditEnv(source) {
    return {
        ...source,
...
export function buildDeployDryRun(source = process.env, options) {
    ...
    const googleTvCaution = googleTv?.recommendedAction ??
...
export function buildReleaseGateInfo(dryRun) {
    const steps = [
        { id: "build", name: "npm run build", status: "pass", message: "API モック — CLI で実行" },
        { id: "tsc", name: "npx tsc --noEmit", status: "pass", message: "API モック — CLI で実行" },
        { id: "test", name: "npm run test", status: "pass", message: "API モック — CLI で実行" },
        {
            id: "dry_run",
