...
function ensureChecklistRows(projectId) {
    const db = getDatabase();
    for (const item of DEPLOYMENT_CHECKLIST_ITEMS) {
        db.prepare(`INSERT OR IGNORE INTO deployment_checklist_rc2 (id, project_id, item_id, completed, created_at, updated_at)
       VALUES (?, ?, ?, 0, datetime('now'), datetime('now'))`).run(`DCL-${projectId}-${item.id}`, projectId, item.id);
    }
}
export function getDeploymentChecklistRC2(projectId) {
    ...
    const rows = getDatabase()
        .prepare(`SELECT * FROM deployment_checklist_rc2 WHERE project_id = ? ORDER BY item_id`)
        .all(bizId);
    const itemMap = new Map(rows.map((r) => [String(r.item_id), r]));
    const items = DEPLOYMENT_CHECKLIST_ITEMS.map((def) => {
...
export function completeDeploymentChecklistItem(projectId, itemId, actor, note) {
    const resolved = resolveProjectId(projectId);
    if (!resolved)
        return null;
    ...
    getDatabase()
        .prepare(`UPDATE deployment_checklist_rc2 SET
