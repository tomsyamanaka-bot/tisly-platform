...
import { customerUrls, getCustomerByCode, upsertCustomer, } from "../customer/customer-store.js";
import { logAudit } from "../provisioning/audit-log.js";
export function generateNextCustomerCode(prefix = "TOMS") {
    const db = getDatabase();
    const rows = db
        .prepare(`SELECT customer_code FROM customers WHERE customer_code LIKE ? COLLATE NOCASE ORDER BY customer_code`)
...
export function createCustomerWizard(input) {
    ...
    const db = getDatabase();
    db.prepare(`INSERT INTO deployment_customer_contacts
     ...
     ON CONFLICT(customer_id) DO UPDATE SET
       ...
       email = excluded.email`).run(customer.customer_id, customerCode, input.siteName, input.address ?? null, input.contactName ?? null, input.phone ?? null, input.email ?? null);
    ...
    db.prepare(`INSERT INTO customer_users (id, customer_id, username, password_hash, role, status)
     VALUES (?, ?, ?, ?, 'owner', 'active')
     ON CONFLICT(customer_id, username) DO UPDATE SET password_hash = excluded.password_hash`).run(`cu-${customerCode}-owner`, customer.customer_id, loginUsername, passwordHash);
    db.prepare(`INSERT INTO deployment_checklist (customer_id, customer_code, items_json, deployment_complete, updated_at)
     VALUES (?, ?, ?, 0, datetime('now'))
     ON CONFLICT(customer_id) DO NOTHING`).run(customer.customer_id, customerCode, JSON.stringify({
        power: false,
        lan: false,
        esp: false,
        shelly: false,
