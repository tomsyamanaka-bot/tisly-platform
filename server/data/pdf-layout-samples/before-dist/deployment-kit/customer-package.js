...
export function buildCustomerPackageData(customerCode) {
    ...
    const ownerUser = getDatabase()
        .prepare(`SELECT username FROM customer_users WHERE customer_id = ? AND role = 'owner' LIMIT 1`)
...
export function buildCustomerPackageHtml(customerCode) {
    const data = buildCustomerPackageData(customerCode);
    if (!data)
        return "<html><body>Customer not found</body></html>";
    const deviceRows = data.devices
        .map((d) => `<tr><td>${escapeHtml(d.deviceId)}</td><td>${escapeHtml(d.label)}</td><td>${escapeHtml(d.kind)}</td></tr>`)
        .join("");
    const qrRows = data.qrList
        .map((q) => `<tr><td>${escapeHtml(q.assetId)}</td><td>${escapeHtml(q.deviceId)}</td><td>${escapeHtml(q.label)}</td><td><a href="${escapeHtml(q.detailUrl)}">QR詳細</a></td></tr>`)
        .join("");
    return `<!DOCTYPE html>
<html lang="ja">
<head>
...
export function buildCustomerPackagePdfBuffer(customerCode) {
