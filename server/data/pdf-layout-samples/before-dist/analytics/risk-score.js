...
export function computeRiskScore(eventType, opts) {
    const classified = classifyEvent(eventType, opts?.createdAt, opts?.concurrentCount ?? 0);
    const factors = [];
    ...
    if (classified.category === "perimeter")
        factors.push("外周センサー");
    if (classified.category === "safety")
        factors.push("安全系");
...
export function mergeClassifiedRisk(c) {
