...
import { isMqttMockMode, mqttBridgeLog, onMqttBridgeAuthError, onMqttBridgeConnect, onMqttBridgeDisconnect, onMqttBridgeInvalidTopic, recordMqttMessageReceived, routeMqttToLivePush, } from "../toms/mqtt-live-push-bridge.js";
let client = null;
let mockTimer = null;
export function startMqttSubscriber() {
    ...
    const subscribeAll = () => {
        ...
        client?.subscribe("tisly/project/#", (err) => {
            if (err)
                onMqttBridgeAuthError(`project subscribe: ${err.message}`);
        });
    };
    client.on("connect", () => {
...
async function handleMessage(topic, raw) {
    ...
    if (routeMqttToLivePush(topic, raw))
        return;
    ...
    if (parsed.channel === "heartbeat") {
        recordHeartbeat(parsed.deviceId, body.platform ?? "mqtt");
        handleEspMqttHeartbeat(parsed.deviceId, {
            platform: body.platform ?? "mqtt",
            rssi: body.rssi,
            uptime: body.uptime,
        });
        ingestDeviceSignal(parsed.deviceId, body.platform ?? "mqtt", body);
        broadcast({
            type: "heartbeat",
...
function startMockSubscriber() {
    ...
    mockTimer = setInterval(() => {
        ...
        void handleMessage(topic, JSON.stringify({ status: "ok", platform: "mock", ts: Date.now() }));
    }, 120_000);
}
