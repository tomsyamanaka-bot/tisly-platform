...
export function deviceLiveMqttTopics(siteId, deviceId) {
    const topic = `tisly/${siteId}/${deviceId}/test/live`;
    return { topic, ackTopic: `${topic}/ack` };
}
function saveDeviceRtt(customerId, deviceId, patch) {
    ...
    db.prepare(`UPDATE devices SET last_test_result = ?, updated_at = datetime('now') WHERE id = ?`).run(JSON.stringify(merged), row.id);
}
function mqttAckRequired() {
    return process.env.MQTT_ACK_REQUIRED === "true";
}
async function waitForDeviceAck(topic, ackTopic, timeoutMs) {
...
export async function runLiveMqttAckTest(customerId, deviceId, timeoutMs = 8000) {
    const db = getDatabase();
    const dev = db
