...
export function isMqttTlsEnvEnabled() {
    return process.env.MQTT_TLS_ENABLED === "true";
}
export function getMqttTlsStatus(mockMode) {
...
export function shouldFallbackMqttTls(mockMode) {
    if (mockMode)
        return false;
...
export function buildMqttConnectOptions(base, mockMode) {
    const opts = {
    ...
    if (!tls.ready || mockMode || !tls.enabled)
        return opts;
...
export function mqttUrlWithTls(url, mockMode) {
    const tls = getMqttTlsStatus(mockMode);
    if (!tls.ready || mockMode || !tls.enabled)
        return url;
