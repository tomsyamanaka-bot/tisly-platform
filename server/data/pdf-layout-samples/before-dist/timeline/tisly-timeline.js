...
function mapEventType(eventType) {
    ...
    if (t.includes("survey") || t === "drawing")
        return "Survey";
    if (t.includes("estimate") || t.includes("invoice") || t.includes("payment"))
        return "Estimate";
    if (t.includes("construction") || t.includes("completion"))
        return "Construction";
    if (t.includes("device") || t.includes("pro_operations"))
        return "Device";
    if (t.includes("alert") || t.includes("alarm"))
        return "Alert";
...
export function buildUnifiedTimeline(filters) {
    ...
    const timelineRows = db.prepare(timelineSql).all(...timelineParams);
    ...
    const deviceParams = filters?.customerCode
        ...
        : [Math.min(limit, 50)];
    try {
        const deviceRows = db.prepare(deviceSql).all(...deviceParams);
    ...
    try {
        const securityRows = db
            .prepare(`SELECT id, event_type, message, source, created_at, metadata_json
    ...
    try {
        const alertRows = db
            .prepare(`SELECT id, event_type, message, severity, created_at, customer_id, site_id
