...
import { listProFloorLayers, ensureProFloorLayersSeed } from "../pro-remote/floor-map-stack.js";
import { listProjectLiveDevices } from "./realtime-devices.js";
function projectCustomerCode(projectId) {
...
export function buildProjectFloorStack(projectId) {
    ...
    const layers = listProFloorLayers(code).map((layer) => {
        ...
        const tierDevices = live.filter((d) => {
            if (!d.floor)
                return tier === "perimeter";
            return d.floor === tier;
        });
        const anomalyCount = tierDevices.filter((d) => d.status === "OFFLINE" || d.status === "WARNING").length;
        const pins = [
            ...layer.pins,
            ...tierDevices
