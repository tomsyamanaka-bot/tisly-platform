...
export function buildProjectDashboard(projectId) {
    ...
    const payments = getDatabase()
        .prepare(`SELECT amount, payment_date, memo FROM business_payments WHERE project_id = ? ORDER BY payment_date DESC`)
        .all(projectId);
    const constructionHistory = getDatabase()
        .prepare(`SELECT to_state, created_at FROM toms_workflow_history
