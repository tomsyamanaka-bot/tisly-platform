...
export async function dispatchTomsPushAlerts() {
    ...
    for (const a of alerts) {
        ...
        db.prepare(`INSERT INTO toms_push_alerts (id, project_id, alert_kind, title, body, href, created_at)
       VALUES (?, NULL, ?, ?, ?, ?, datetime('now'))`).run(id, a.kind, a.title, a.body, a.href);
    }
    ...
    if (first) {
        db.prepare(`UPDATE toms_push_alerts SET sent_at = datetime('now') WHERE alert_kind = ? AND sent_at IS NULL`).run(first.kind);
    }
    return {
