...
export function runSurveyAnalysisV4(projectId) {
    ...
    getDatabase()
        .prepare(`INSERT INTO survey_analysis_v4 (id, project_id, result_json, created_at) VALUES (?, ?, ?, ?)`)
...
export function getLatestSurveyAnalysisV4(projectId) {
    const row = getDatabase()
        .prepare(`SELECT * FROM survey_analysis_v4 WHERE project_id = ? ORDER BY created_at DESC LIMIT 1`)
        .get(projectId);
    if (!row)
        return null;
