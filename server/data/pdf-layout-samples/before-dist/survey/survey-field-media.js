...
export function surveyAudioDir(projectId) {
    const dir = path.join(process.cwd(), "uploads", "survey", projectId, "audio");
    fs.mkdirSync(dir, { recursive: true });
    return dir;
}
export function surveySketchDir(projectId) {
...
export function saveSurveyAudio(params) {
    ...
    const id = uuid();
    getDatabase()
        .prepare(`INSERT INTO survey_audio_memos
       (id, project_id, audio_path, mime_type, duration_sec, transcript, uploaded_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`)
        .run(id, params.projectId, rel, params.mimeType ?? "audio/webm", params.durationSec ?? null, params.transcript ?? null, params.uploadedBy ?? null);
    return { id, url: `/uploads/survey/${rel}`, durationSec: params.durationSec ?? null };
}
export function listSurveyAudio(projectId) {
    const rows = getDatabase()
        .prepare(`SELECT id, audio_path, mime_type, duration_sec, transcript, uploaded_by, created_at
...
export function saveSurveySketch(params) {
    ...
    const id = uuid();
    getDatabase()
        .prepare(`INSERT INTO survey_sketch_memos (id, project_id, image_path, uploaded_by, created_at)
       VALUES (?, ?, ?, ?, datetime('now'))`)
        .run(id, params.projectId, rel, params.uploadedBy ?? null);
    return { id, url: `/uploads/survey/${rel}` };
}
export function listSurveySketches(projectId) {
    const rows = getDatabase()
        .prepare(`SELECT id, image_path, uploaded_by, created_at FROM survey_sketch_memos
