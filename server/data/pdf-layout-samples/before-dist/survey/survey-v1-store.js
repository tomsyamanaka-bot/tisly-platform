...
import { SURVEY_MATERIAL_CATEGORIES, SURVEY_WORKFLOW_STATUSES, } from "./survey-v1-types.js";
import { surveyUploadsDir } from "./survey-store.js";
function isWorkflowStatus(v) {
    return SURVEY_WORKFLOW_STATUSES.includes(v);
}
function isMaterialCategory(v) {
    return SURVEY_MATERIAL_CATEGORIES.includes(v);
}
...
const SURVEY_PHOTO_SELECT = `id, photo_type, photo_path, comment, taken_at, uploaded_by, sort_order, datetime(created_at) as created_at`;
...
function withProjectNotes(project) {
...
export function createSurveyProjectV1(input) {
    ...
    getDatabase()
        .prepare(`INSERT INTO survey_projects (
        ...
        .run(projectId, projectNo, input.customerCode.toUpperCase(), input.customerName.trim(), input.customerAddress?.trim() ?? null, siteName, input.address?.trim() ?? null, input.phone?.trim() ?? null, input.email?.trim() ?? null, input.surveyDate ?? null, input.assignee?.trim() ?? null, now, now);
    if (input.notes !== undefined && input.notes !== null) {
        saveProjectNotes(projectId, String(input.notes).trim());
    }
    return withProjectNotes(getSurveyProjectV1(projectId));
}
export function getSurveyProjectV1(projectId) {
    const row = getDatabase()
        .prepare(`SELECT * FROM survey_projects WHERE project_id = ?`)
        .get(projectId);
    return row ? rowToProject(row) : null;
}
export function getSurveyProjectV1Detail(projectId) {
    ...
    const handoff = handoffRow
        ? {
            ...
            payloadJson: parsePayload(handoffRow.payload_json),
        }
        : null;
    return {
        ...project,
...
export function updateSurveyProjectV1(projectId, patch) {
    ...
    getDatabase()
        ...
        updated_at = ?
       WHERE project_id = ?`)
        .run(patch.customerName ?? existing.customerName, patch.customerAddress !== undefined ? patch.customerAddress : existing.customerAddress, patch.siteName ?? existing.siteName, patch.address !== undefined ? patch.address : existing.address, patch.phone !== undefined ? patch.phone : existing.phone, patch.email !== undefined ? patch.email : existing.email, patch.surveyDate !== undefined ? patch.surveyDate : existing.surveyDate, patch.assignee !== undefined ? patch.assignee : existing.assignee, patch.workflowStatus ?? null, now, projectId);
...
export function listSurveyPhotosV1(projectId) {
    const rows = getDatabase()
        .prepare(`SELECT ${SURVEY_PHOTO_SELECT}
       FROM survey_photos WHERE project_id = ? ORDER BY sort_order ASC, created_at ASC, id ASC`)
        .all(projectId);
    return rows.map(rowToPhoto);
}
const MAX_SURVEY_PHOTOS_V1 = 30;
function nextSurveyPhotoSortOrder(projectId) {
    const sortRow = getDatabase()
        .prepare(`SELECT COALESCE(MAX(sort_order), -1) + 1 as n FROM survey_photos WHERE project_id = ?`)
        .get(projectId);
    return sortRow?.n ?? 0;
}
export function addSurveyPhotoMemoV1(projectId, input) {
    if (!getSurveyProjectV1(projectId))
        throw new Error("project not found");
    if (input.imageBase64) {
        const countRow = getDatabase()
            .prepare(`SELECT COUNT(*) as c FROM survey_photos WHERE project_id = ? AND photo_path NOT LIKE '_memo:%'`)
            .get(projectId);
    ...
    getDatabase()
        .prepare(`INSERT INTO survey_photos (id, project_id, photo_type, photo_path, comment, taken_at, uploaded_by, sort_order, created_at)
       VALUES (?, ?, 'field', ?, ?, ?, ?, ?, datetime('now'))`)
        .run(id, projectId, photoPath, input.comment?.trim() ?? null, takenAt, input.uploadedBy ?? null, sortOrder);
    getDatabase()
...
export function listSurveyMaterialsV1(projectId) {
    const rows = getDatabase()
        .prepare(`SELECT * FROM survey_materials WHERE project_id = ? ORDER BY sort_order ASC, created_at ASC`)
        .all(projectId);
    return rows.map(rowToMaterial);
}
export function addSurveyMaterialV1(projectId, input) {
    if (!getSurveyProjectV1(projectId))
        throw new Error("project not found");
    ...
    const sortRow = getDatabase()
        ...
        .get(projectId);
    getDatabase()
        .prepare(`INSERT INTO survey_materials (id, project_id, category, item_label, quantity, memo, sort_order, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, projectId, input.category, input.itemLabel?.trim() ?? "", qty, input.memo?.trim() ?? "", sortRow.n, now, now);
    getDatabase()
        .prepare(`UPDATE survey_projects SET updated_at = ? WHERE project_id = ?`)
        .run(now, projectId);
...
export function updateWorkflowStatusV1(projectId, workflowStatus) {
    if (!isWorkflowStatus(workflowStatus))
        throw new Error("invalid workflow_status");
    return updateSurveyProjectV1(projectId, { workflowStatus });
}
export function markEstimatePendingV1(projectId, handoffBy) {
    ...
    if (existing) {
        getDatabase()
            .prepare(`UPDATE survey_handoff_log SET
        ...
        getDatabase()
            .prepare(`INSERT INTO survey_handoff_log (id, survey_project_id, business_project_id, handoff_by, handoff_at, payload_json)
         VALUES (?, ?, ?, ?, datetime('now'), ?)`)
...
export function updateSurveyPhotoV1(projectId, photoId, patch) {
    if (!getSurveyProjectV1(projectId))
        return null;
    const row = getDatabase()
    ...
    const title = patch.title !== undefined || patch.comment !== undefined
...
export function moveSurveyPhotoV1(projectId, photoId, direction) {
    if (!getSurveyProjectV1(projectId))
        return null;
    const photos = listSurveyPhotosV1(projectId);
    ...
    db.prepare(`UPDATE survey_photos SET sort_order = ? WHERE id = ? AND project_id = ?`).run(neighbor.sortOrder, current.id, projectId);
    db.prepare(`UPDATE survey_photos SET sort_order = ? WHERE id = ? AND project_id = ?`).run(current.sortOrder, neighbor.id, projectId);
    db.prepare(`UPDATE survey_projects SET updated_at = ? WHERE project_id = ?`).run(new Date().toISOString(), projectId);
    return listSurveyPhotosV1(projectId);
}
export function deleteSurveyPhotoV1(projectId, photoId) {
    const row = getDatabase()
...
export function copySurveyProjectV1(projectId) {
    const detail = getSurveyProjectV1Detail(projectId);
    if (!detail)
        throw new Error("project not found");
...
export function deleteSurveyProjectV1(projectId) {
    if (!getSurveyProjectV1(projectId))
        return false;
