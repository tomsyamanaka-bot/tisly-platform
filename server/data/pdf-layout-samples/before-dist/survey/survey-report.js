...
import { getSurveyProject, listSurveyPhotos, listSurveyDrawings, getSurveyChecklist, getSurveyProjectNotes, } from "./survey-store.js";
...
function photoSection(photos, types, title) {
    const filtered = photos.filter((p) => types.includes(p.photoType));
    if (!filtered.length)
        return `<section><h2>${title}</h2><p class="muted">なし</p></section>`;
    const imgs = filtered
...
export function buildSurveyReportHtml(projectId) {
    ...
    const drawingHtml = drawings.length
        ? drawings
            .map((d) => `<figure><img src="${escapeHtml(d.url)}" alt="図面" /><figcaption>${escapeHtml(d.fileName ?? d.id)}</figcaption></figure>`)
            .join("")
        : "<p class=\"muted\">なし</p>";
