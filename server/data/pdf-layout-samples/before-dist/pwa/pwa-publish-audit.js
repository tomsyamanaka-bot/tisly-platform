...
import { MOCK_REAL_GUARDS, checkProductionEnv, } from "../config/production-env-checker.js";
...
function auditPwaItem(spec, prodBase, locBase, isProdUrl) {
    ...
    if (spec.isPwa) {
        ...
        if (!fileExists("icons/icon-192.png"))
            missingItems.push("icons/icon-192.png");
        if (!fileExists("icons/icon-512.png"))
            missingItems.push("icons/icon-512.png");
        if (!isProdUrl)
            missingItems.push("TISLY_PUBLIC_URL が本番 URL ではない");
        if (!prodBase.startsWith("https://"))
            missingItems.push("HTTPS 公開 URL が未設定");
    }
    let status = "ok";
    ...
    if (!spec.isPwa) {
        ...
        const blocking = missingItems.some((m) => m.startsWith("icons/") ||
...
export function buildPwaPublishAudit(source = process.env) {
    const prodBase = (source.TISLY_PUBLIC_URL ?? source.PUBLIC_BASE_URL ?? "https://tisly.jp").replace(/\/$/, "");
    const port = (source.TISLY_PORT ?? source.PORT ?? "3080");
    const locBase = `http://localhost:${port}`;
    const publicUrl = (source.TISLY_PUBLIC_URL ?? source.PUBLIC_BASE_URL ?? "");
    const isProdUrl = publicUrl.length > 0 &&
        !publicUrl.includes("localhost") &&
        !publicUrl.includes("127.0.0.1") &&
        publicUrl.startsWith("https://tisly.jp");
    const mockReal = MOCK_REAL_GUARDS.map((g) => ({
        service: g.service,
    ...
    const pwAs = PWA_AUDIT_SPECS.map((spec) => auditPwaItem(spec, prodBase, locBase, isProdUrl));
    const summary = {
