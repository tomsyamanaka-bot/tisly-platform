...
function normalizePwaRole(role) {
    if (role === "super_admin")
        return "admin";
    return normalizeRole(role);
}
export const PWA_APP_CATALOG = {
    installer: {
        id: "installer",
        label: "施工",
...
export function getPwaAppsForRole(role, opts) {
    const r = normalizePwaRole(role);
    switch (r) {
        case "installer":
            if (opts?.installerSurveyOptional)
                return ["installer", "survey"];
            return ["installer"];
        case "surveyor":
...
export function buildPracticalHubCards(role) {
    const r = normalizePwaRole(role);
...
export function showOpsPanelsForRole(role) {
    const r = normalizePwaRole(role);
