...
function htmlHas(rel, pattern) {
    const html = readText(rel);
    if (!html)
        return false;
    return pattern.test(html);
}
const HTML_SCRIPT_MODULES = {
...
function swRegisteredInHtml(htmlFile, swPath) {
    const html = readText(htmlFile);
    if (!html)
        return false;
    const esc = swPath.replace(/\//g, "\\/");
    const direct = new RegExp(`register\\(["']${esc}["']`).test(html) ||
...
function auditOne(spec) {
    ...
    if (!htmlExists)
        missing.push(`html:${spec.htmlFile}`);
    ...
    if (!swExists)
        missing.push(`service-worker file:${swFile}`);
    if (!swReg)
        missing.push("service worker registration");
    ...
    if (!icon192)
        missing.push("icons/icon-192.png");
    if (!icon512)
        missing.push("icons/icon-512.png");
    let manifest = null;
    ...
    if (manifest) {
        ...
        if (!startOk)
            missing.push(`start_url expected ${spec.expectedStartUrl}`);
        if (spec.expectedScope) {
            ...
            if (!scopeOk)
                missing.push(`scope expected ${spec.expectedScope}`);
        }
        const icons = manifest.icons;
        const manifestIcons = Array.isArray(icons) &&
            icons.some((i) => String(i?.src ?? "").includes("icon-192"));
        checks.push({
            id: "manifest_icons",
...
export function buildPwaInstallAudit() {
