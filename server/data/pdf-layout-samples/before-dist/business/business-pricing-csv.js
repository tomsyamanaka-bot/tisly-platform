...
export function exportPricingRulesCsv(opts) {
    ...
    if (opts?.customerCode) {
        rules = rules.filter((r) => r.scopeType === "customer" && r.scopeRef === opts.customerCode);
    }
    if (opts?.contractorCode) {
        rules = rules.filter((r) => r.scopeType === "contractor" && r.scopeRef === opts.contractorCode);
    }
...
export function previewPricingRulesCsv(csvText) {
    const lines = csvText
        .trim()
        .split(/\r?\n/)
        .filter((l) => l.trim());
    ...
    const rows = [];
    if (!lines.length)
        return { rows, errors: ["empty csv"], validCount: 0, invalidCount: 0 };
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const row = {
            ...
            scope: cells[col("customer_code")]?.trim()
                ? "customer"
...
export function importPricingRulesCsv(csvText, opts) {
