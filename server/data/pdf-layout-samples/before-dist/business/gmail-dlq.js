...
export function enqueueGmailDeadLetter(input) {
    ...
    getDatabase()
        .prepare(`INSERT INTO gmail_send_dlq
       (id, project_id, queue_id, to_address, subject, status, attempt_count, last_error, payload_json, created_at)
       VALUES (?, ?, ?, ?, ?, 'dead_letter', ?, ?, ?, ?)`)
        .run(id, input.projectId ?? null, input.queueId ?? null, input.toAddress, input.subject, input.attemptCount ?? 0, input.lastError ?? null, input.payload != null ? JSON.stringify(input.payload) : null, now);
    ...
    return rowToDlq(getDatabase()
        .prepare(`SELECT * FROM gmail_send_dlq WHERE id = ?`)
        .get(id));
}
export function listGmailDlq(opts) {
    const clauses = [];
    const params = [];
    if (opts?.projectId) {
