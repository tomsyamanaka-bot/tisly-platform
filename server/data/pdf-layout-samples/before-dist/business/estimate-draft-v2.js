...
import { getLatestSurveyAnalysisV2, runSurveyAnalysisV2, } from "../survey/ai-survey-analysis-v2.js";
...
function candidatesToLines(candidates) {
...
function rowToDraft(row) {
    const lines = JSON.parse(String(row.lines_json ?? "[]"));
...
export function createEstimateDraftV2(projectId, opts) {
    const project = getBusinessProject(projectId);
    if (!project)
        throw new Error("project not found");
    const surveyProjectId = project.surveyProjectId;
    if (!surveyProjectId)
        throw new Error("surveyProjectId required");
    const analysis = opts?.runAnalysis !== false
        ? runSurveyAnalysisV2(surveyProjectId)
        : getLatestSurveyAnalysisV2(surveyProjectId);
    if (!analysis)
        throw new Error("survey analysis v2 not found");
    const lines = candidatesToLines(analysis.estimateCandidates);
    const sums = summarize(lines);
    const id = `ED2-${uuid().slice(0, 8).toUpperCase()}`;
    const now = new Date().toISOString();
    getDatabase()
        .prepare(`INSERT INTO business_estimate_drafts_v2 (
        id, project_id, version, lines_json, subtotal, total_cost, gross_profit, gross_profit_rate,
        status, created_at, updated_at
      ) VALUES (?, ?, 'v2', ?, ?, ?, ?, ?, 'draft', ?, ?)`)
        .run(id, projectId, JSON.stringify(lines), sums.subtotal, sums.totalCost, sums.grossProfit, sums.grossProfitRate, now, now);
...
export function getEstimateDraftV2(id) {
    const row = getDatabase()
        .prepare(`SELECT * FROM business_estimate_drafts_v2 WHERE id = ?`)
        .get(id);
    return row ? rowToDraft(row) : null;
}
export function getLatestEstimateDraftV2(projectId) {
    const row = getDatabase()
        .prepare(`SELECT * FROM business_estimate_drafts_v2 WHERE project_id = ? ORDER BY created_at DESC LIMIT 1`)
        .get(projectId);
    return row ? rowToDraft(row) : null;
}
export function patchEstimateDraftV2(id, patch) {
    ...
    const now = new Date().toISOString();
    getDatabase()
        .prepare(`UPDATE business_estimate_drafts_v2 SET
        lines_json = ?, subtotal = ?, total_cost = ?, gross_profit = ?, gross_profit_rate = ?,
        status = ?, updated_at = ?
       WHERE id = ?`)
        .run(JSON.stringify(lines), sums.subtotal, sums.totalCost, sums.grossProfit, sums.grossProfitRate, status, now, id);
    return getEstimateDraftV2(id);
}
