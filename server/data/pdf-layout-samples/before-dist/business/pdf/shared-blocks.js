...
export function renderPdfHeader(docTitle, docNo) {
...
export function renderTomsDocLayoutHeader(input) {
    const co = getTomsCompanyInfo();
    const addressee = formatTomsAddressee(input.addressee);
    const regRow = input.includeRegistrationNo !== false
...
export function renderAmountBanner(total) {
...
export function renderTomsEstimateStandardHeader(header) {
...
export function renderTomsCompanyFooter() {
    const co = getTomsCompanyInfo();
...
export function renderCustomerBlock(customerName, title, address, projectNo) {
...
export function renderTomsCustomerSiteBlock(input) {
    const rows = [];
    ...
    if (input.siteAddress)
        rows.push(`<p>工事場所: ${escapeHtml(input.siteAddress)}</p>`);
    if (input.contactName)
        rows.push(`<p>ご担当: ${escapeHtml(input.contactName)}</p>`);
    if (input.phone)
        rows.push(`<p>TEL: ${escapeHtml(input.phone)}</p>`);
    if (input.email)
        rows.push(`<p>Email: ${escapeHtml(input.email)}</p>`);
    rows.push("</div>");
    const meta = [
...
export function renderTomsEstimateHeaderTable(header) {
    const rows = [
    ...
    const body = rows
        .map(([label, value]) => `<tr><th class="hdr-label">${escapeHtml(label)}</th><td class="hdr-value">${escapeHtmlMultiline(value)}</td></tr>`)
        .join("");
    return `<table class="header-table"><tbody>${body}</tbody></table>`;
}
export function renderTomsInvoiceHeaderTable(header) {
    const rows = [
        ["宛名", formatTomsAddressee(header.addressee)],
        ["件名", header.subject],
...
export function renderTomsLineItemsTable(items) {
    const rows = items
        .map((i, idx) => `<tr><td class="num col-no">${i.lineNo ?? idx + 1}</td><td class="col-desc">${escapeHtmlMultiline(i.description)}</td><td class="num col-qty">${i.quantity}</td><td class="num col-price">¥${i.unitPrice.toLocaleString("ja-JP")}</td><td class="num col-amount">¥${i.amount.toLocaleString("ja-JP")}</td></tr>`)
        .join("");
...
export function renderLineItemsTable(items) {
    return renderTomsLineItemsTable(items.map((i, idx) => ({
        lineNo: idx + 1,
...
export function renderTotals(subtotal, tax, total) {
...
export function renderPhotoGrid(photos, includeImages = false) {
    const slots = photos
        .slice(0, 20)
        .map((p) => {
