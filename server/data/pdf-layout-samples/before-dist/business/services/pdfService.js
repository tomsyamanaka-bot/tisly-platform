...
import { getPdfTemplateMeta, renderPdfPlaceholderHtml, } from "./pdf-templates.js";
...
function writePdf(projectId, folder, fileName, buf) {
    const dir = businessUploadsDir(projectId, folder);
    const full = path.join(dir, fileName);
    fs.writeFileSync(full, buf);
    return `/uploads/business/${projectId}/${folder}/${fileName}`;
}
function writeHtml(projectId, folder, fileName, html) {
    const dir = businessUploadsDir(projectId, folder);
    const full = path.join(dir, fileName);
    fs.writeFileSync(full, html, "utf8");
    return `/uploads/business/${projectId}/${folder}/${fileName}`;
}
function renderWithTemplate(kind, project, doc, pdfLines) {
    ...
    const fileName = path.basename(generateQnapFilePath(project, kind === "completion_report" ? "completion_report" : kind, kind === "estimate"
        ? doc.estimateNo
        : kind === "invoice"
            ? doc.invoiceNo
            : undefined));
    const pdfPath = writePdf(project.id, "pdfs", fileName, minimalPdfBuffer(meta.description, [...pdfLines, `template: ${meta.provider}/${meta.version}`]));
    return { pdfPath, htmlPath, template: meta };
}
export function generateEstimatePdf(project, estimate, ctx) {
...
export function generateInvoicePdf(project, invoice, estimate, ctx) {
...
export function generateCompletionReportPdf(project, report) {
...
export function getEstimatePdfOrPlaceholder(project, estimate, ctx) {
    const html = renderEstimateHtml(project, estimate, {
        siteName: ctx?.siteName,
...
export function getInvoicePdfOrPlaceholder(project, invoice, estimate, ctx) {
    const html = renderInvoiceHtml(project, invoice, estimate, {
        estimateRefNo: ctx?.estimateRefNo ?? invoice.estimateRefNo ?? estimate.estimateNo,
        notes: ctx?.notes,
        includePhotos: ctx?.includePhotos,
...
export function getCompletionReportPdfOrPlaceholder(project, report) {
    if (report.pdfPath) {
        const local = path.join(process.cwd(), report.pdfPath.replace(/^\//, ""));
        if (fs.existsSync(local))
            return { contentType: "application/pdf", path: local };
    }
