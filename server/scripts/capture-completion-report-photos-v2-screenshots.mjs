/**
 * Completion Report Photos v2 提出用スクショ
 * Usage: npm run build && node scripts/capture-completion-report-photos-v2-screenshots.mjs
 */
import fs from "fs";
...
const outDir = path.join(root, "data/completion-report-photos-v2-screenshots");
...
async function ensureCameraProject(token) {
  ...
  const hit =
    (list.projects || []).find((p) => p.automation?.photos?.length >= 6) ||
  null;
...
async function main() {
  ...
  try {
    ...
    await sleep(800);
    await page.click('.detail-tab[data-tab="automation-photos"]').catch(() => {});
    await sleep(700);
    screens.push(await capture(page, "01-project-detail-photo-slots.png"));

    await page.goto(`${baseUrl}/estimate-v1?project=${encodeURIComponent(projectId)}`, {
      waitUntil: "networkidle0",
      timeout: 30000,
    });
    await sleep(1200);
    await page.click("#btn-create-completion-report").catch(() => {});
    await sleep(900);
    screens.push(await capture(page, "02-missing-photo-warning.png"));
    await page.click('.cr-photo-check-actions [data-action="proceed"]').catch(() => {});
    await sleep(400);

    screens.push(await capture(page, "03-completion-report-before-pdf.png"));
    await page.click("#btn-pdf-completion").catch(() => {});
    await sleep(2000);
    screens.push(await capture(page, "04-completion-report-pdf-preview.png"));
    ...
    await sleep(500);
    screens.push(await capture(page, "05-document-center-photo-slot.png"));
    ...
    await page.click('.detail-tab[data-tab="automation-photos"]').catch(() => {});
    await sleep(600);
    screens.push(await capture(page, "06-android10-photo-slots.png", android10));
