...
describe("security-floor-v1", () => {
  ...
  it("serves pages CSS keyframes and APIs", async () => {
    ...
    assert.match(css, /sf-iso3d-room-label/);
    assert.match(css, /rgba\(255,\s*255,\s*255,\s*0\.82\)/);
    assert.match(css, /backdrop-filter:\s*blur\(3px\)/);
    assert.match(css, /font-size:\s*9px/);
    assert.match(css, /padding:\s*2px\s+6px/);
    assert.match(css, /\.sf-iso3d-mount[\s\S]*touch-action:\s*none/);
    ...
    assert.match(html, /security-floor-remote-config-v1\.js\?v=2507/);
    assert.match(html, /security-floor-push-v1\.js\?v=2507/);
    assert.match(html, /security-floor-light-v1\.js\?v=2507/);
    assert.match(html, /security-floor-operator-v1\.js\?v=2507/);
    assert.match(html, /security-floor-iso3d-v1\.js\?v=2507/);
    assert.match(html, /security-floor-v1\.css\?v=2507/);
    assert.match(html, /sf-brand-logo/);
    assert.match(html, /tisly-shield-logo-128\.png/);
    assert.match(html, /icons\/icon-128\.png\?v=2507/);
    ...
    assert.match(iso3dJs, /herringbone|ヘリンボーン/);
    assert.match(iso3dJs, /sf-iso3d-room-ico/);
    assert.match(iso3dJs, /WALL_TOP|WALL_THICK/);
    assert.match(iso3dJs, /0x4f5e38|4f5e38/);
    assert.match(iso3dJs, /0x6e5d4f|6e5d4f/);
    ...
    assert.match(iso3dJs, /bindPinchZoom|pinchZoom/);
    assert.match(iso3dJs, /touches\.length\s*!==\s*2|touches\.length\s*===\s*2/);
    assert.match(iso3dJs, /new OrbitControls\(\s*camera,\s*mountEl/);
