...
export function collectBusinessAlerts(): BusinessAlert[] {
  const alerts: BusinessAlert[] = [];
  const paymentDue = countProjectsByStatus(
    expandStatusAliases(["invoice_sent"]) as BusinessProjectStatus[]
  );
  if (paymentDue > 0) {
    alerts.push({
      id: "payment_due",
  ...
  const estimateUnsent = countProjectsByStatus(
  ...
  const recentErrors = listBusinessIntegrationLogs({ limit: 20 }).filter(
    (l) => l.status === "error"
  );
...
export async function sendBusinessMockNotifications(): Promise<{
