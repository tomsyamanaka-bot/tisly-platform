...
export interface OfflineSyncResult {
  synced: Array<{ type: OfflineSyncOpType; projectId?: string; clientId?: string }>;
  failed: Array<{ type: OfflineSyncOpType; error: string; clientId?: string }>;
  skipped: Array<{ type: OfflineSyncOpType; reason: string; clientId?: string }>;
}

export function processBusinessOfflineSync(items: OfflineSyncItem[]): OfflineSyncResult {
  const result: OfflineSyncResult = { synced: [], failed: [], skipped: [] };
    ...
    try {
      switch (item.type) {
        ...
        case "photo_memo": {
          ...
          saveBusinessPhoto(
            pid,
            (item.payload.kind as "survey" | "construction") ?? "survey",
            String(item.payload.imageBase64),
            String(item.payload.fileName ?? "photo.jpg")
          );
        ...
        case "estimate_item": {
          const pid = item.projectId ?? String(item.payload?.projectId ?? "");
          const items = (item.payload?.items ?? []) as EstimateLineItem[];
          if (!pid) {
        ...
        case "invoice_memo": {
          ...
          createInvoiceFromEstimate(
            pid,
            item.payload?.paymentDueDate != null ? String(item.payload.paymentDueDate) : undefined
          );
          result.synced.push({ type: item.type, projectId: pid, clientId: item.clientId });
          break;
        }
      ...
      result.failed.push({ type: item.type, error: (e as Error).message, clientId: item.clientId });
    }
  }
