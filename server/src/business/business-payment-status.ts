...
export function computePaymentStatus(projectId: string): BusinessProjectStatus | null {
  const project = getBusinessProject(projectId);
  if (!project?.invoiceId) return null;
  const inv = getInvoice(project.invoiceId);
  if (!inv) return null;
  const payments = listBusinessPayments({ projectId });
  const paidAmount = payments.reduce((s, p) => s + p.amount, 0);
  const due = inv.total;
  if (paidAmount <= 0) return "invoice_sent";
  if (paidAmount >= due) return "paid";
  return "partial_paid";
}

export function applyPaymentStatusAfterRecord(projectId: string): {
  ...
  const updated = updateBusinessProject(
    projectId,
    {
      status: target,
