...
const MEMORY = new Map<string, { eventId?: string; timestamp: string; seenAt: number }>();
const MAX_ENTRIES = 10_000;
const TTL_SEC = 3600;

export async function replayStoreHas(
  signature: string,
  eventId?: string
): Promise<boolean> {
  ...
  if (MEMORY.has(signature)) return true;
  if (eventId) {
...
export async function replayStoreAdd(
  signature: string,
  eventId?: string,
  timestamp?: string
): Promise<void> {
  ...
  if (config.rateLimitProvider === "redis" && (await pingRedis())) {
    ...
    if (r) {
      ...
      if (eventId) await r.setex(`replay:eid:${eventId}`, TTL_SEC, signature);
      return;
    }
  }
