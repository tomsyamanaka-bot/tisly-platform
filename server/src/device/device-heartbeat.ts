...
import {
  statusFromHeartbeatAge,
  updateDeviceStatusFields,
  type DeviceStatus,
...
const lastNotified = new Map<string, DeviceStatus>();

export function getHeartbeatThresholds(): { warnSec: number; offlineSec: number } {
  return {
...
export function recordDeviceHeartbeat(deviceId: string, platform?: string): DeviceStatus {
  const db = getDatabase();
  const now = new Date().toISOString();
  const existing = db
    .prepare("SELECT id, first_seen, device_status FROM devices WHERE device_id = ?")
    .get(deviceId) as
    | { id: string; first_seen: string | null; device_status: string | null }
    | undefined;

  if (existing) {
    db.prepare(
      `UPDATE devices SET last_heartbeat_at = ?, last_seen = ?, heartbeat_status = 'ok',
        device_status = 'ONLINE', platform = COALESCE(?, platform),
        first_seen = COALESCE(first_seen, ?), updated_at = datetime('now')
       WHERE device_id = ?`
    ).run(now, now, platform ?? null, now, deviceId);
  } else {
    const id = uuid();
    db.prepare(
      `INSERT INTO devices (id, device_id, device_type, platform, label, last_heartbeat_at, last_seen,
        first_seen, heartbeat_status, device_status)
       VALUES (?, ?, 'gateway', ?, ?, ?, ?, ?, 'ok', 'ONLINE')`
    ).run(id, deviceId, platform ?? "unknown", deviceId, now, now, now);
  }

  db.prepare(
    `INSERT INTO device_heartbeats (id, device_id, received_at, payload_json)
     VALUES (?, ?, ?, ?)`
  ).run(uuid(), deviceId, now, JSON.stringify({ platform: platform ?? null }));
...
export function evaluateDeviceHeartbeatStatuses(): Array<{
  deviceId: string;
  status: DeviceStatus;
  elapsedSec: number;
}> {
  ...
  const rows = db
    .prepare(
      `SELECT device_id, last_heartbeat_at, device_status, commissioning_status
       FROM devices
  ...
  const changes: Array<{ deviceId: string; status: DeviceStatus; elapsedSec: number }> = [];
  ...
  for (const d of rows) {
    ...
    if (!d.last_heartbeat_at) continue;
...
export function startDeviceHeartbeatMonitor(
  onStatusChange?: (change: { deviceId: string; status: DeviceStatus }) => void
): void {
