...
import {
  deviceModeUsesEsp,
  deviceModeUsesMock,
  deviceModeUsesShelly,
  getDeviceMode,
  type DeviceMode,
} from "./device-mode-store.js";
...
function inferKind(deviceType: string, deviceId: string): DeviceKind {
  const t = deviceType.toLowerCase();
  const id = deviceId.toUpperCase();
  if (t.includes("shelly") || id.includes("SHELLY")) return "Shelly";
  if (t.includes("camera") || t.includes("cam") || id.includes("CAM")) return "Camera";
  if (t.includes("plc") || id.includes("PLC")) return "PLC";
  if (t.includes("esp") || t.includes("gateway") || id.includes("ESP")) return "ESP";
  return "Other";
}

export function listUnifiedDevices(customerCode?: string): UnifiedDeviceView[] {
  ...
  const out: UnifiedDeviceView[] = [];
  for (const r of rows) {
    const status = normalizeDeviceStatus(r.device_status) as UnifiedDeviceView["status"];
    const kind = inferKind(r.device_type, r.device_id);
    let source: UnifiedDeviceView["source"] = "db";
    if (kind === "ESP" && deviceModeUsesEsp()) source = "esp";
    else if (kind === "Shelly" && deviceModeUsesShelly()) source = "shelly";
    else if (deviceModeUsesMock()) source = "mock";

    const view: UnifiedDeviceView = {
...
function getMockFallbackDevices(): UnifiedDeviceView[] {
...
export function ingestDeviceSignal(
  deviceId: string,
  platform?: string,
  payload?: Record<string, unknown>
): { status: string; mode: DeviceMode } {
  const mode = getDeviceMode();
  ...
  if (payload && Object.keys(payload).length > 0) {
    const db = getDatabase();
    const row = db.prepare(`SELECT metadata_json FROM devices WHERE device_id = ?`).get(deviceId) as
      | { metadata_json: string | null }
      | undefined;
