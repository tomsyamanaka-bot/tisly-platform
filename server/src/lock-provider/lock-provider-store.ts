...
import type {
  ...
  LockUserRole,
  PresenceUser,
} from "../providers/lock/types.js";

const DEFAULT_LOCK_USERS: Omit<LockUser, "id" | "createdAt">[] = [
...
const DEFAULT_PRESENCE_USERS: Omit<PresenceUser, "id">[] = [
...
function rowToLockUser(row: Record<string, unknown>): LockUser {
  return {
    id: String(row.id),
    name: String(row.name),
    role: row.role as LockUserRole,
...
function rowToLockEvent(row: Record<string, unknown>): LockEvent {
  return {
    id: String(row.id),
    provider: row.provider as LockProviderId,
    deviceId: String(row.device_id),
    eventType: row.event_type as LockEventType,
...
function rowToPresenceUser(row: Record<string, unknown>): PresenceUser {
  let deviceIds: string[] = [];
  try {
    deviceIds = JSON.parse(String(row.device_ids ?? "[]")) as string[];
  } catch {
    deviceIds = [];
  }
  return {
...
export function ensureLockProviderSeed(): void {
  const db = getDatabase();
  const userCount = (db.prepare("SELECT COUNT(*) as c FROM lock_users").get() as { c: number }).c;
  if (userCount === 0) {
    const now = new Date().toISOString();
    const stmt = db.prepare(
      `INSERT INTO lock_users (id, name, role, enabled, notification_enabled, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    );
  ...
  const presenceCount = (
    db.prepare("SELECT COUNT(*) as c FROM presence_users").get() as { c: number }
  ).c;
  if (presenceCount === 0) {
    const stmt = db.prepare(
      `INSERT INTO presence_users (id, name, device_ids, role, notification_enabled)
       VALUES (?, ?, ?, ?, ?)`
    );
...
export function listLockUsers(): LockUser[] {
  const db = getDatabase();
  const rows = db.prepare("SELECT * FROM lock_users ORDER BY created_at").all() as Record<
    string,
    unknown
  >[];
...
export function getLockUserByName(name: string): LockUser | null {
  const db = getDatabase();
  const row = db
...
export function listPresenceUsers(): PresenceUser[] {
  const db = getDatabase();
  const rows = db.prepare("SELECT * FROM presence_users ORDER BY name").all() as Record<
    string,
    unknown
  >[];
...
export function createLockEvent(input: {
  ...
  const now = new Date().toISOString();
  db.prepare(
    `INSERT INTO lock_events (id, provider, device_id, event_type, user_id, user_name, success, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
    input.provider,
...
export function listLockEvents(limit = 50): LockEvent[] {
  const db = getDatabase();
  const rows = db
...
export function listFaceLockEvents(limit = 50): LockEvent[] {
...
export function createFamilyNotification(input: {
  ...
  const now = new Date().toISOString();
  db.prepare(
    `INSERT INTO family_notifications (id, kind, user_name, provider, method, message, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(id, input.kind, input.userName, input.provider, input.method, input.message, now);
  return { id, ...input, createdAt: now };
}

export function listFamilyNotifications(limit = 50): FamilyNotificationRecord[] {
  const db = getDatabase();
  const rows = db
  ...
  return rows.map((row) => ({
    id: String(row.id),
    kind: row.kind as FamilyNotificationKind,
...
export function listChildArrivalNotifications(limit = 50): FamilyNotificationRecord[] {
  return listFamilyNotifications(limit * 2)
    .filter((n) => n.kind === "child_arrived_home")
    .slice(0, limit);
}

export function resetLockProviderStoreForTests(): void {
  const db = getDatabase();
