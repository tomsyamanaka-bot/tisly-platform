...
const SCENARIO_META: Record<
  MockLockScenario,
  { userName: string; eventType: LockEventType; method: string }
> = {
...
export function generateMockLockEvent(scenario: MockLockScenario): LockEvent {
  const meta = SCENARIO_META[scenario];
  const user = getLockUserByName(meta.userName);
  const provider: LockProviderId =
    meta.eventType === "face_unlock" || meta.eventType === "nfc_unlock"
      ? "sesame"
