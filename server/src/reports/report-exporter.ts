...
function archiveReportToQnap(report: BuiltReport): string | null {
  ...
  if (config.qnap.mode === "mock") {
    try {
      getDatabase()
        .prepare(
          `INSERT INTO qnap_archives (id, archive_type, format, file_path, record_count)
...
export function getReportExport(exportId: string): ExportRecord | null {
  const row = getDatabase()
    .prepare(
      `SELECT export_id, customer_id, site_id, generated_by, generated_at,
              format, status, report_type, archive_path
       FROM customer_report_exports WHERE export_id = ?`
    )
    .get(exportId) as ExportRecord | undefined;
  return row ?? null;
}
