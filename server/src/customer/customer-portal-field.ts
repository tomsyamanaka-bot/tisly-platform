...
export interface CustomerPortalFieldView {
  customerCode: string;
  ownerOnly: true;
  devices: Array<{ deviceId: string; label: string; deviceType: string; status: string }>;
  cameras: Array<{ deviceId: string; label: string; status: string }>;
  qrAssets: ReturnType<typeof listAssetQrHistory>;
  completionReports: Array<{ projectId: string; title: string; pdfPath: string | null; createdAt: string }>;
  maintenanceHistory: ReturnType<typeof listMaintenanceReports>;
  maintenanceCases: ReturnType<typeof listMaintenanceCases>;
  notificationHistory: Array<{ id: string; message: string; severity: string; createdAt: string }>;
}

export function buildCustomerPortalFieldView(customerCode: string): CustomerPortalFieldView | null {
  ...
  const completionRows = getDatabase()
    .prepare(
      `SELECT bp.id, bp.title, bcr.pdf_path, bcr.created_at
