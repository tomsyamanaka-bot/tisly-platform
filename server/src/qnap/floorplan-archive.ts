...
export function archiveFloorplanToQnap(input: {
  ...
  const site = db.prepare(`SELECT customer_id FROM sites WHERE id = ?`).get(floor.site_id) as
    | { customer_id: string }
    | undefined;
  ...
  const mapRow = db
    ...
    .get(input.floorId) as { image_path: string } | undefined;
  ...
  db.prepare(
    `INSERT INTO floor_maps (id, floor_id, image_path, updated_at)
     VALUES (?, ?, ?, datetime('now'))
     ON CONFLICT(floor_id) DO UPDATE SET image_path = image_path, updated_at = excluded.updated_at`
  ).run(uuid(), input.floorId, rel);
