...
export function archiveEventsToFile(format: ExportFormat = "json", days = 1): string {
  ...
  db.prepare(
    `INSERT INTO qnap_archives (id, archive_type, format, file_path, record_count, created_at)
     VALUES (?, 'events', ?, ?, ?, datetime('now'))`
  ).run(id, format, filepath, rows.length);
...
export function listArchives(): Array<{
  ...
  return db
    .prepare(
      `SELECT id, archive_type as archiveType, format, file_path as filePath,
