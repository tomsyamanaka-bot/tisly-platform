...
export function switchBotModeLabel(mode: string): SwitchBotGateLabel {
  if (mode === "real") return "本番注意";
  if (mode === "dryrun" || mode === "dryRun") return "実機前チェック";
  return "初回公開OK";
}

export function buildSwitchBotReleaseGateChecks(
  source: NodeJS.ProcessEnv = process.env
): DryRunCheckItem[] {
  const get = (k: string, fallback = ""): string => (source[k] ?? fallback).trim();
...
export function buildSwitchBotDeploymentChecklist(): Array<{
  id: string;
  label: string;
  ok: boolean;
  detail: string;
}> {
  const get = (k: string, fallback = ""): string => (process.env[k] ?? fallback).trim();
  const mode = get("SWITCHBOT_MODE", "mock").toLowerCase();
  const autoArm = get("SWITCHBOT_AUTO_ARM_ENABLED", "false") === "true";
  const autoDisarm = get("SWITCHBOT_AUTO_DISARM_ENABLED", "false") === "true";
