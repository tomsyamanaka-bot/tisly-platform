...
import type {
  ...
  SecurityAutomationSettings,
  SecurityEventLog,
  SecurityMode,
  SecuritySource,
  SecurityState,
  UnknownDevicePolicy,
} from "./security-automation-types.js";

const DEFAULT_SETTINGS: SecurityAutomationSettings = {
...
export function ensureSecurityAutomationSeed(): void {
  const db = getDatabase();
  const stateRow = db.prepare("SELECT id FROM security_state LIMIT 1").get();
  if (!stateRow) {
    const now = new Date().toISOString();
    db.prepare(
      `INSERT INTO security_state (id, mode, reason, source, last_changed_at, last_changed_by)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).run(uuid(), "disarmed", "Initial state", "system", now, "system");
  }

  const ruleCount = (
    db.prepare("SELECT COUNT(*) as c FROM security_automation_rules").get() as { c: number }
  ).c;
  if (ruleCount === 0) {
    for (const rule of DEFAULT_RULES) {
      db.prepare(
        `INSERT INTO security_automation_rules
        ...
        rule.enabled ? 1 : 0,
        rule.triggerType,
        rule.requiredPresence,
        rule.action,
        rule.delaySeconds,
        rule.unknownDevicePolicy,
        rule.requireConfirmation ? 1 : 0
      );
    }
  }
  ...
  const settingsRow = db
    ...
    .get("security_automation_settings") as { value_json: string } | undefined;
  if (!settingsRow) {
    const initial = {
      ...DEFAULT_SETTINGS,
      autoArmEnabled: config.switchbot.autoArmEnabled,
      autoDisarmEnabled: config.switchbot.autoDisarmEnabled,
      unknownDevicePolicy: (process.env.SECURITY_UNKNOWN_DEVICE_POLICY as
        | "block_auto_arm"
        | "unknown_as_away"
        | "unknown_as_home") || DEFAULT_SETTINGS.unknownDevicePolicy,
    };
    db.prepare(
      `INSERT INTO platform_settings (key, value_json, updated_at) VALUES (?, ?, datetime('now'))`
    ).run("security_automation_settings", JSON.stringify(initial));
  }
}
...
export function getSecurityState(): SecurityState {
  ...
  const row = getDatabase()
    .prepare(
      `SELECT id, mode, reason, source, last_changed_at, last_changed_by FROM security_state ORDER BY last_changed_at DESC LIMIT 1`
    )
    .get() as {
    id: string;
    mode: SecurityMode;
...
export function saveSecurityState(
  mode: SecurityMode,
  reason: string,
  source: SecuritySource,
  changedBy: string
...
export function listSecurityEventLogs(limit = 50): SecurityEventLog[] {
  const rows = getDatabase()
    .prepare(
      `SELECT id, event_type, source, message, before_mode, after_mode, metadata_json, created_at
    ...
    source: SecuritySource;
    message: string;
    before_mode: SecurityMode | null;
    after_mode: SecurityMode | null;
...
export function getRegisteredDevices(): RegisteredPresenceDevice[] {
  ensureSecurityAutomationSeed();
  const rows = getDatabase()
    .prepare(
      `SELECT id, name, type, owner_name, mac_address, ip_address, enabled, last_seen_at, presence_status
    ...
    presence_status: RegisteredPresenceDevice["presenceStatus"];
  }>;
  return rows.map((r) => ({
    id: r.id,
    name: r.name,
...
export function upsertPresenceDevice(
  input: Partial<RegisteredPresenceDevice> & { name: string; type: RegisteredPresenceDevice["type"] }
): RegisteredPresenceDevice {
  const id = input.id ?? uuid();
  const now = new Date().toISOString();
  getDatabase()
    .prepare(
      `INSERT INTO registered_presence_devices
...
export function updateDevicePresenceInStore(
  deviceId: string,
  status: RegisteredPresenceDevice["presenceStatus"]
): RegisteredPresenceDevice | null {
  const now = new Date().toISOString();
  const result = getDatabase()
    .prepare(
      `UPDATE registered_presence_devices SET presence_status = ?, last_seen_at = ?, updated_at = ? WHERE id = ?`
    )
...
export function getAutomationRules(): SecurityAutomationRule[] {
  ensureSecurityAutomationSeed();
  const rows = getDatabase()
    .prepare(
      `SELECT id, name, enabled, trigger_type, required_presence, action, delay_seconds, unknown_device_policy, require_confirmation
    ...
    name: string;
    enabled: number;
    trigger_type: SecurityAutomationRule["triggerType"];
    required_presence: SecurityAutomationRule["requiredPresence"];
    action: SecurityAutomationRule["action"];
...
export function updateAutomationRule(
  id: string,
  patch: Partial<SecurityAutomationRule>
): SecurityAutomationRule | null {
  const existing = getAutomationRules().find((r) => r.id === id);
  if (!existing) return null;
  const merged = { ...existing, ...patch };
  getDatabase()
    .prepare(
      `UPDATE security_automation_rules SET
    ...
    .run(
      ...
      merged.enabled ? 1 : 0,
      merged.triggerType,
      merged.requiredPresence,
      merged.action,
      merged.delaySeconds,
      merged.unknownDevicePolicy,
      merged.requireConfirmation ? 1 : 0,
      id
    );
  return merged;
}

export function getAutomationSettings(): SecurityAutomationSettings {
  ensureSecurityAutomationSeed();
  const row = getDatabase()
    .prepare("SELECT value_json FROM platform_settings WHERE key = ?")
...
export function saveAutomationSettings(settings: Partial<SecurityAutomationSettings>): SecurityAutomationSettings {
...
export function resetSecurityAutomationForTests(): void {
  const db = getDatabase();
