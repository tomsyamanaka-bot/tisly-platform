...
export interface SurveyAnalysisV2Result {
  id: string;
  projectId: string;
  perimeterCameras: Array<{ label: string; reason: string; confidence: number }>;
  entranceCameras: Array<{ label: string; reason: string; confidence: number }>;
  indoorSensors: Array<{ label: string; type: string; confidence: number }>;
  lanDistanceEstimateM: number;
  powerOutlets: Array<{ label: string; location: string }>;
  shellyPlacements: Array<{ label: string; purpose: string }>;
...
function checklistChecked(checklist: Record<string, unknown>, key: string): boolean {
  const item = checklist[key];
  if (!item || typeof item !== "object") return false;
  return Boolean((item as { checked?: boolean }).checked);
}

export function runSurveyAnalysisV2(projectId: string): SurveyAnalysisV2Result {
  ...
  const estimateCandidates: EstimateCandidateV2[] = [
    {
      id: "EC-PERIM",
      category: "device",
      name: "外周防犯カメラ（PoE）",
  ...
  getDatabase()
    .prepare(
      `INSERT INTO survey_analysis_v2 (id, project_id, result_json, created_at) VALUES (?, ?, ?, ?)`
    )
...
export function getLatestSurveyAnalysisV2(projectId: string): SurveyAnalysisV2Result | null {
  const row = getDatabase()
    .prepare(
      `SELECT * FROM survey_analysis_v2 WHERE project_id = ? ORDER BY created_at DESC LIMIT 1`
    )
    .get(projectId) as Record<string, unknown> | undefined;
  if (!row) return null;
  const result = JSON.parse(String(row.result_json)) as Omit<
    SurveyAnalysisV2Result,
