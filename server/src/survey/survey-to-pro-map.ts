...
export function generateFloorMapFromSurvey(projectId: string): {
  customerCode: string;
  tiers: string[];
  layers: Array<{ layerId: string; tier: string; displayName: string; imageUrl: string | null }>;
  roofCreated: false;
} {
  ...
  const layersOut: Array<{ layerId: string; tier: string; displayName: string; imageUrl: string | null }> = [];
  ...
  for (const tier of PRO_FLOOR_TIERS) {
    const layer = database
      ...
      .get(customer.customer_id, tier) as
      | { id: string; display_name: string; image_path: string | null }
      | undefined;
    if (!layer) continue;
    ...
    if (tierDrawing && !layer.image_path) {
      ...
      if (fs.existsSync(srcFull)) {
        ...
        const floorId = database
          ...
          .get(layer.id) as { floor_id: string | null } | undefined;
        if (floorId?.floor_id) {
          database
    ...
    const refreshed = database
      ...
      .get(layer.id) as { id: string; display_name: string; image_path: string | null; tier: string };
    layersOut.push({
  ...
  database
    .prepare(
      `INSERT INTO survey_floor_map_links (project_id, customer_code, linked_at) VALUES (?, ?, datetime('now'))
  ...
  return {
    ...
    layers: layersOut.sort((a, b) => PRO_FLOOR_TIERS.indexOf(a.tier as (typeof PRO_FLOOR_TIERS)[number]) - PRO_FLOOR_TIERS.indexOf(b.tier as (typeof PRO_FLOOR_TIERS)[number])),
    roofCreated: false,
  };
}

export function getSurveyProMapLink(projectId: string): { linked: boolean; customerCode: string | null } {
  const row = getDatabase()
    .prepare(`SELECT customer_code FROM survey_floor_map_links WHERE project_id = ?`)
    .get(projectId) as { customer_code: string } | undefined;
