...
export interface AiIntakeResult {
  ...
  rooms: Array<{ name: string; floor: string }>;
  exterior_points: Array<{ label: string; posHint: string }>;
  entry_points: Array<{ label: string }>;
  windows: number;
  doors: number;
  stairs: number;
  electrical_panel: { count: number; notes: string };
  network_point: { count: number; notes: string };
  risk_points: string[];
  recommended_devices: Array<{ type: string; qty: number; reason: string }>;
}

export function runSurveyAiIntake(projectId: string, input?: AiIntakeInput): AiIntakeResult {
  ...
  const checked = Object.entries(checklist).filter(
  ...
  const hazard =
    (checklist.hazard_zones as { note?: string } | undefined)?.note?.trim() || "";
  ...
  const result: AiIntakeResult = {
    ...
    recommended_devices: [
      ...
      {
        type: "camera",
        qty: Math.max(1, cameras + ((checklist.camera as { checked?: boolean } | undefined)?.checked ? 1 : 0)),
        reason: "外周・玄関",
      },
  ...
  getDatabase()
    .prepare(
      `INSERT INTO survey_ai_intakes (id, project_id, input_json, result_json, created_at) VALUES (?, ?, ?, ?, datetime('now'))`
    )
    .run(
      id,
      projectId,
      JSON.stringify({ notes: input?.notes, gps: input?.gps ?? { lat: project.gpsLat, lng: project.gpsLng } }),
...
export function getLatestAiIntake(projectId: string): AiIntakeResult | null {
  const row = getDatabase()
    .prepare(
      `SELECT result_json FROM survey_ai_intakes WHERE project_id = ? ORDER BY created_at DESC LIMIT 1`
    )
    .get(projectId) as { result_json: string } | undefined;
  if (!row) return null;
  try {
    return JSON.parse(row.result_json) as AiIntakeResult;
  } catch {
    return null;
  }
}
