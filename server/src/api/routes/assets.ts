...
import {
  ...
  getAssetQrByToken,
  listAssetQrHistory,
} from "../../assets/asset-qr.js";

export const assetsRouter = Router();

const assetAuth = [requireAuth("installer")] as const;

function assertAssetRole(req: AuthedRequest, res: Response): boolean {
...
assetsRouter.post("/qr/create", ...assetAuth, (req: AuthedRequest, res) => {
  if (!assertAssetRole(req, res)) return;
...
assetsRouter.get("/qr/history", ...assetAuth, (req: AuthedRequest, res) => {
  if (!assertAssetRole(req, res)) return;
...
assetsRouter.get("/qr/:assetId", ...assetAuth, (req: AuthedRequest, res) => {
  if (!assertAssetRole(req, res)) return;
