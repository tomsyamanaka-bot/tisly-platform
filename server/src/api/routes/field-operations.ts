...
import { roleMeetsRequirement, type AppRole } from "../../auth/roles.js";
import { buildTomsKpi } from "../../toms/toms-kpi.js";
import {
  buildFieldOperationsAudit,
  findBusinessProjectBySurvey,
  generateProjectEstimateV4,
  syncProRemoteFromBusinessProject,
  listFieldAssets,
  summarizeFieldAssets,
  addMaintenanceReplacementParts,
  listMaintenanceReplacementParts,
  FIELD_ASSET_KINDS,
} from "../../field-operations/index.js";

export const fieldOperationsRouter = Router();

const opsAuth = [requireAuth("surveyor")] as const;

function assertOpsRole(req: AuthedRequest, res: Response, minRole: AppRole = "surveyor"): boolean {
...
fieldOperationsRouter.get("/assets", ...opsAuth, (req: AuthedRequest, res) => {
  if (!assertOpsRole(req, res, "installer")) return;
...
fieldOperationsRouter.get("/kpi", ...opsAuth, (req: AuthedRequest, res) => {
  if (!assertOpsRole(req, res, "manager")) return;
...
fieldOperationsRouter.post(
  "/projects/:projectId/estimate-v4",
  ...opsAuth,
...
fieldOperationsRouter.post(
  "/projects/:projectId/pro-remote-sync",
  ...opsAuth,
  (req: AuthedRequest, res) => {
    if (!assertOpsRole(req, res, "installer")) return;
    try {
...
fieldOperationsRouter.get(
  "/survey/:surveyProjectId/business-link",
  ...opsAuth,
  (req: AuthedRequest, res) => {
    if (!assertOpsRole(req, res)) return;
...
fieldOperationsRouter.post(
  "/maintenance/reports/:reportId/parts",
  ...opsAuth,
  (req: AuthedRequest, res) => {
    if (!assertOpsRole(req, res, "maintenance")) return;
