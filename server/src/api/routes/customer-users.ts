...
customerUsersRouter.get("/:customerCode/users", ...auth, (req: AuthedRequest, res) => {
...
customerUsersRouter.post(
  ...
  (req: AuthedRequest, res) => {
    ...
    const { username, role } = req.body as { username?: string; role?: CustomerRole };
    if (!username?.trim()) {
    ...
    const result = inviteCustomerUser({
      ...
      role: (role ?? "viewer") as CustomerRole,
