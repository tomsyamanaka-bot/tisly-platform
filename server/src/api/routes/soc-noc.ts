...
function securityEvents(limit = 30, customerId?: string, tenantId?: string) {
...
function networkHealth(customerId?: string, tenantId?: string) {
  const db = getDatabase();
  if (customerId) {
    const devices = db
...
socNocRouter.get("/summary", opsCustomerScopeMiddleware, (req: AuthedRequest, res) => {
  ...
  try {
    if (req.opsScope?.customerId) {
      ...
      tvOffline = (
        db
          .prepare(
            `SELECT COUNT(*) as c FROM tv_devices WHERE status != 'active' OR last_seen_at IS NULL`
          )
          .get() as { c: number }
      ).c;
    }
    qnapWarning = (
      db.prepare(`SELECT COUNT(*) as c FROM qnap_archives WHERE status = 'warning'`).get() as
        | { c: number }
        | undefined
    )?.c ?? 0;
  } catch {
  ...
  if (!recoveryPending) recoveryPending = openIncidents;
...
socNocRouter.get("/soc", opsCustomerScopeMiddleware, (req: AuthedRequest, res) => {
  ...
  res.json({
    ...
    recovery: {
      openIncidents: countOpenIncidents(
        cid ? { customerId: cid, tenantId: tid } : null
      ),
    },
  });
});

socNocRouter.get("/noc", opsCustomerScopeMiddleware, (req: AuthedRequest, res) => {
  const db = getDatabase();
  const cid = req.opsScope?.customerId;
  const tid = req.opsScope?.tenantId;
  let offline = db
  ...
  if (cid) {
    offline = db
      .prepare(
        `SELECT * FROM devices WHERE heartbeat_status != 'ok'
