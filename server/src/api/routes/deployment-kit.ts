...
import {
  ...
  getCustomerContact,
  createSiteWizard,
  listDeploymentSiteTypes,
  listSitesForCustomerCode,
  provisionDeploymentDevice,
  ...
  getCustomerMaintenanceSummary,
  listMaintenanceCases,
  buildCustomerPackageData,
  buildCustomerPackageHtml,
  buildCustomerPackagePdfBuffer,
  buildDeploymentChecklist,
  updateChecklistItem,
  markDeploymentComplete,
  buildDeploymentKpi,
  type ChecklistItemId,
  type DeploymentDeviceKind,
  type InstallStepType,
} from "../../deployment-kit/index.js";
...
deploymentKitRouter.get("/customers/next-code", requireAdminAuth, (req, res) => {
  const prefix = (req.query.prefix as string) ?? "TOMS";
...
deploymentKitRouter.post("/customers/wizard", requireAdminAuth, (req, res) => {
  try {
    ...
    const result = createCustomerWizard({
      ...
      plan: body.plan as "Lite" | "Standard" | "PRO" | "PRO_REMOTE" | undefined,
    });
...
deploymentKitRouter.post("/sites/wizard", requireAdminAuth, (req, res) => {
  try {
    ...
    const result = createSiteWizard({
      ...
      siteType: body.siteType as import("../../deployment-kit/site-wizard.js").DeploymentSiteType,
      name: body.name,
      address: body.address,
    });
    res.status(201).json(result);
...
deploymentKitRouter.post("/maintenance/request", requireAdminAuth, (req, res) => {
  try {
    ...
    const result = createCustomerMaintenanceRequest(body as { customerCode: string });
    res.status(201).json(result);
  } catch (e) {
    res.status(400).json({ error: String(e) });
  }
...
deploymentKitRouter.put("/checklist/:customerCode/:itemId", requireAdminAuth, (req, res) => {
  try {
    const ok = Boolean((req.body as { ok?: boolean }).ok);
    const result = updateChecklistItem(
      String(req.params.customerCode),
      String(req.params.itemId) as ChecklistItemId,
      ok
    );
    res.json(result);
  } catch (e) {
