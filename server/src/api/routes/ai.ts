...
const aiAuth = [requireAuth("surveyor")] as const;

function assertAiRole(req: AuthedRequest, res: Response): boolean {
...
aiRouter.post("/survey-analysis", ...aiAuth, (req: AuthedRequest, res) => {
  if (!assertAiRole(req, res)) return;
  const body = req.body as { projectId?: string; surveyProjectId?: string };
...
aiRouter.post("/survey-analysis-v2", ...aiAuth, (req: AuthedRequest, res) => {
  if (!assertAiRole(req, res)) return;
  const body = req.body as { projectId?: string; surveyProjectId?: string };
  const projectId = body.surveyProjectId ?? body.projectId;
  if (!projectId) {
...
aiRouter.get("/survey-analysis-v2/:projectId", ...aiAuth, (req: AuthedRequest, res) => {
  if (!assertAiRole(req, res)) return;
...
aiRouter.get("/survey-analysis/:projectId", ...aiAuth, (req: AuthedRequest, res) => {
  if (!assertAiRole(req, res)) return;
