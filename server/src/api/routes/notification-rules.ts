...
notificationRulesRouter.post("/", (req, res) => {
  const { name, sensorType, timeWindow, severity, channels, siteId, enabled, priority } =
    req.body;
  if (!name) {
  ...
  getDatabase()
    .prepare(
      `INSERT INTO notification_rule_conditions
    ...
    .run(
      ...
      timeWindow ?? "night",
      severity ?? "critical",
      JSON.stringify(channels ?? ["push", "discord"]),
      siteId ?? null,
      enabled !== false ? 1 : 0,
      priority ?? 0
    );
...
notificationRulesRouter.patch("/:id", (req, res) => {
  ...
  db.prepare(
    `UPDATE notification_rule_conditions SET
       name = COALESCE(?, name),
