...
import {
  ...
  deleteCustomerNotificationRule,
  notificationRulesPortalPayload,
  updateCustomerNotificationRule,
} from "../../customer/customer-notification-rules.js";
...
customerNotificationRulesRouter.get("/:customerCode/notification-rules", ...auth, (req, res) => {
  const customer = resolve(req as AuthedRequest, String(req.params.customerCode));
  if (!customer) {
...
customerNotificationRulesRouter.post(
  "/:customerCode/notification-rules",
  requireAuth("manager"),
...
customerNotificationRulesRouter.patch(
  "/:customerCode/notification-rules/:id",
  requireAuth("manager"),
  requireTenantMatch("customerCode"),
  (req: AuthedRequest, res) => {
    const customer = resolve(req, String(req.params.customerCode));
    ...
    const result = updateCustomerNotificationRule(
...
customerNotificationRulesRouter.delete(
  "/:customerCode/notification-rules/:id",
  requireAuth("admin"),
  requireTenantMatch("customerCode"),
  (req: AuthedRequest, res) => {
