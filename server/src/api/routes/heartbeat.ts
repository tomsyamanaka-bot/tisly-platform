...
heartbeatRouter.get("/status", (_req, res) => {
  ...
  const settings = getPlatformSetting<{ warnSec: number; alarmSec: number }>("heartbeat");
  const devices = db
    .prepare(
      `SELECT device_id, label, platform, last_heartbeat_at, heartbeat_status FROM devices ORDER BY device_id`
    )
    .all();
  res.json({
