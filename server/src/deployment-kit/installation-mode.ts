...
export function recordInstallStep(input: InstallStepInput) {
  ...
  db.prepare(
    `INSERT INTO deployment_install_records
    ...
    input.gpsLng ?? null,
    input.notes ?? null,
    input.installerUserId ?? null
  );

  if (input.step === "placement" && input.deviceId) {
    db.prepare(
      `UPDATE devices SET commissioning_status = 'placed', commissioned_at = datetime('now') WHERE device_id = ?`
    ).run(input.deviceId);
  }
  if (input.step === "test" && input.deviceId) {
    db.prepare(
      `UPDATE devices SET commissioning_status = 'tested', last_test_result = ? WHERE device_id = ?`
    ).run(input.notes ?? "ok", input.deviceId);
  }
  if (input.step === "sign" && input.deviceId) {
    db.prepare(
      `UPDATE devices SET commissioning_status = 'completed', commissioned_at = datetime('now') WHERE device_id = ?`
    ).run(input.deviceId);
  }
...
export function getInstallRecords(customerCode: string, deviceId?: string) {
  const sql = deviceId
...
export function getInstallationDashboard(customerCode: string) {
  ...
  const devices = db
    .prepare(
      `SELECT device_id, label, commissioning_status, site_id FROM devices WHERE customer_id = ?`
    )
  ...
  const records = getInstallRecords(customerCode) as Array<{ step: string; device_id: string }>;
  const stepsByDevice = new Map<string, Set<string>>();
  for (const r of records) {
    if (!r.device_id) continue;
    if (!stepsByDevice.has(r.device_id)) stepsByDevice.set(r.device_id, new Set());
