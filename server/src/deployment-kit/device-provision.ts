...
const KIND_MAP: Record<DeploymentDeviceKind, { deviceType: string; platform: string }> = {
...
export function provisionDeploymentDevice(input: DeploymentDeviceInput) {
  ...
  const site = db
    ...
    .get(input.siteId, customer.customer_id, customer.tenant_id ?? customer.customer_id) as
    | { id: string; tenant_id: string }
    | undefined;
  if (!site) throw new Error("site not found for customer");
  ...
  if (existing) throw new Error(`deviceId ${deviceId} already exists`);
...
export function listDeploymentDevices(customerCode: string) {
  const customer = getCustomerByCode(customerCode);
  if (!customer) return [];
  return getDatabase()
    .prepare(
      `SELECT d.id, d.device_id, d.device_type, d.label, d.site_id, d.heartbeat_status,
