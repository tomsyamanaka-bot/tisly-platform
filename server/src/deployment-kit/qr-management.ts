...
export function createDeploymentAsset(input: DeploymentAssetInput): DeploymentAsset {
  const db = getDatabase();
  const existing = db
    .prepare(`SELECT asset_id FROM deployment_assets WHERE device_id = ?`)
    .get(input.deviceId) as { asset_id: string } | undefined;
  if (existing) {
...
export function getDeploymentAsset(assetId: string): DeploymentAsset | null {
...
export function recordAssetScan(assetId: string): void {
  getDatabase()
    .prepare(
      `UPDATE deployment_assets SET scan_count = scan_count + 1, last_scan_at = datetime('now') WHERE asset_id = ?`
    )
    .run(assetId);
}
...
export function getAssetDetail(assetId: string) {
  ...
  const device = db
    .prepare(
      `SELECT id, device_id, device_type, label, site_id, heartbeat_status, commissioning_status,
              last_seen, metadata_json, install_note
       FROM devices WHERE device_id = ?`
    )
    .get(asset.deviceId) as Record<string, unknown> | undefined;

  const site = db
    .prepare(`SELECT id, name, address, site_type FROM sites WHERE id = ?`)
    .get(asset.siteId) as Record<string, unknown> | undefined;

  let floors: Array<Record<string, unknown>> = [];
  try {
    floors = db
      .prepare(
        `SELECT f.id, f.name, f.floor_plan_path, fm.image_path
         FROM floors f
         LEFT JOIN floor_maps fm ON fm.floor_id = f.id
         WHERE f.site_id = ?
         ORDER BY f.order_no`
      )
      .all(asset.siteId) as Array<Record<string, unknown>>;
  } catch {
    floors = [];
  }
  ...
  try {
    photos = db
      .prepare(
        `SELECT id, photo_type, photo_path, uploaded_at
         FROM install_photos
         WHERE customer_id = (SELECT customer_id FROM customers WHERE customer_code = ? COLLATE NOCASE)
           AND (device_id = ? OR site_id = ?)
         ORDER BY uploaded_at DESC LIMIT 20`
      )
      .all(asset.customerCode, asset.deviceId, asset.siteId) as Array<Record<string, unknown>>;
  } catch {
    photos = [];
  }

  const maintenanceHistory = listMaintenanceCases(asset.customerCode).filter(
    (c) => c.deviceIds.includes(asset.deviceId) || c.siteId === asset.siteId
  );
...
export function buildAssetQr(assetId: string) {
  const asset = getDeploymentAsset(assetId);
...
export function listDeploymentAssets(customerCode: string): DeploymentAsset[] {
  const rows = getDatabase()
    .prepare(`SELECT * FROM deployment_assets WHERE customer_code = ? ORDER BY created_at DESC`)
    .all(customerCode.toUpperCase()) as Array<Record<string, unknown>>;
  return rows.map((row) => ({
    assetId: String(row.asset_id),
    customerCode: String(row.customer_code),
    siteId: String(row.site_id),
