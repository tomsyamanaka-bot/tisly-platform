...
export function claimNfcProvisioning(input: NfcClaimInput): { deviceRowId: string; deviceId: string } {
  ...
  const existing = db
    ...
    .get(deviceId, input.customerId) as { id: string } | undefined;

  let deviceRowId: string;
  if (existing) {
    deviceRowId = existing.id;
    db.prepare(
      `UPDATE devices SET commissioning_status = 'claimed', commissioned_at = ?, commissioned_by = ?,
