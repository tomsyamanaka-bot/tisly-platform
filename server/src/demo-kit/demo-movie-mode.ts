 ...
 */
import { triggerDemoNotification, type DemoNotificationKind } from "./demo-notifications.js";
import { runDemoShellyReboot } from "./demo-shelly-reboot.js";

export type DemoMovieScene = "notify" | "intrusion" | "recovery" | "maintenance";

const SCENE_SEQUENCE: DemoMovieScene[] = ["notify", "intrusion", "recovery", "maintenance"];

const SCENE_KIND: Record<DemoMovieScene, DemoNotificationKind | null> = {
...
let movieTimer: ReturnType<typeof setTimeout> | null = null;
...
export function stopDemoMovie(): void {
  if (movieTimer) clearTimeout(movieTimer);
  movieTimer = null;
  movieRunning = false;
  movieStep = 0;
}

async function runScene(scene: DemoMovieScene, customerCode: string): Promise<void> {
  if (scene === "recovery") {
  ...
  if (kind) await triggerDemoNotification(kind, customerCode);
}
