...
} from "../customer/retire-obsolete-demo-customers-v1.js";
...
/** 正規デモ顧客コード（2件のみ） */
export const DEMO_PACK_CODES = ["TOMS001", "TOYOSHIMA001"] as const;
...
function seedDevicesForCustomer(c: DemoPackCustomerDef): void {
  ...
  for (const d of extra) {
    db.prepare(
      `INSERT INTO devices
         (id, device_type, platform, device_id, label, customer_id, site_id,
          serial_number, firmware_version, last_seen, last_heartbeat_at,
          heartbeat_status, device_status, metadata_json)
       VALUES (?, ?, 'demo-kit', ?, ?, ?, ?, ?, '1.0.0', ?, ?, 'ok', 'ONLINE', ?)
       ON CONFLICT(id) DO UPDATE SET
         customer_id=excluded.customer_id, site_id=excluded.site_id,
         device_status='ONLINE', last_seen=excluded.last_seen,
  ...
  for (const deviceId of [
    ...
    `${c.customerCode}-ESP-02`,
    `${c.customerCode}-GW-01`,
  ]) {
    db.prepare(
      `UPDATE devices SET device_status = 'ONLINE', metadata_json = ?
       WHERE device_id = ?`
    ).run(
      JSON.stringify({
        demo_kit: true,
  ...
  db.prepare(
    `INSERT OR IGNORE INTO camera_devices
       (id, customer_id, site_id, device_id, channel, camera_name, camera_group)
     VALUES (?, ?, ?, ?, 1, ?, 'demo')`
  ).run(
    camId,
    c.customerId,
    c.siteId,
    `${c.customerCode}-CAM-01`,
    `${c.customerName} カメラ`
  );
}
  ...
  const row = db
    .prepare(
      `SELECT project_id FROM survey_projects
       WHERE customer_code = ? ORDER BY created_at LIMIT 1`
    )
...
function seedSurveyAssets(c: DemoPackCustomerDef): void {
  ...
  const photoCount = (
    getDatabase()
      .prepare(
        `SELECT COUNT(*) as c FROM survey_photos WHERE project_id = ?`
      )
  ...
  const drawingCount = (
    getDatabase()
      .prepare(
        `SELECT COUNT(*) as c FROM survey_drawings WHERE project_id = ?`
      )
      .get(projectId) as { c: number }
  ).c;
  if (drawingCount < 1) {
  ...
  const samples = [
    {
      type: "intrusion",
      title: "侵入検知",
      body: `${c.siteName} — 外周センサー`,
    },
    {
      type: "heartbeat_ok",
      title: "通信復旧",
      body: `${c.customerCode}-ESP-01`,
    },
    {
  ...
  for (const s of samples) {
    db.prepare(
      `INSERT INTO notification_logs
         (id, device_id, event_type, channel, title, body, payload_json, status, sent_at)
       VALUES (?, ?, ?, 'web_push', ?, ?, ?, 'sent',
...
/** Idempotent — 正規2顧客＋現場・機器・現調・通知履歴 */
...
export function getDemoPackStatus(): Array<{
  ...
  return DEMO_PACK_CUSTOMERS.map((c) => {
    ...
    const siteCount = (
      db
        .prepare(`SELECT COUNT(*) as c FROM sites WHERE customer_id = ?`)
    ...
    const svy = db
      .prepare(
        `SELECT project_id FROM survey_projects WHERE customer_code = ? LIMIT 1`
      )
      .get(c.customerCode) as { project_id: string } | undefined;
    const photoCount = svy
      ? (
          db
            ...
            .get(svy.project_id) as { c: number }
        ).c
      : 0;
...
export function clearDemoPackSurveyUploads(): void {
  for (const c of DEMO_PACK_CUSTOMERS) {
    const dir = path.join(
      ...
      "uploads",
      "survey",
      surveyProjectId(c.customerCode)
    );
