...
function runDemoKitSeed(): DemoResetResult {
...
export function clearDemoKitData(): void {
  ...
  for (const c of DEMO_PACK_CUSTOMERS) {
    ...
    db.prepare(
      `DELETE FROM device_timeline WHERE customer_id = ? AND (actor = 'demo-kit' OR title LIKE '%デモ%')`
    ).run(c.customerId);
  }
}
...
export function ensureDemoKit(): DemoResetResult {
