...
function resolveCronExpr(): string {
  ...
  if (sched.mode === "morning") return config.demoReset.cronExpr || "0 6 * * *";
  if (sched.mode === "before_sales") return "0 8 * * 1-5";
...
export function startDemoResetCron(): void {
  ...
  cronTask = cron.schedule(
    ...
    () => {
      ...
      if (!config.demoReset.enabled && !current.enabled) return;
      if (current.mode === "manual" && !config.demoReset.enabled) return;
...
export function refreshDemoResetCron(): void {
  startDemoResetCron();
}

/** env から初回スケジュールを同期 */
export function syncDemoResetFromEnv(): void {
