...
const URL_CHECK_SPECS: UrlCheckSpec[] = [
  {
    path: "/app",
    label: "App Hub",
...
function nginxHasRoutes(): boolean {
  const confPath = path.join(serverRoot, "deploy/nginx/tisly.jp.conf");
  if (!fs.existsSync(confPath)) return false;
  const conf = fs.readFileSync(confPath, "utf8");
  return ["/app", "/survey", "/business", "/sales", "/customer/", "/tv/", "/api/"].every(
    (p) => conf.includes(p)
  );
}

function swCachesRoute(swContent: string, routePath: string): boolean {
...
export function buildUrlCheck(): UrlCheckReport {
  ...
  const entries: UrlCheckEntry[] = URL_CHECK_SPECS.map((spec) => {
    ...
    let serviceWorker: RehearsalCheck;
...
export function buildPwaRehearsalAudit(
  ...
  const entries: PwaRehearsalEntry[] = pwaSpecs.map((spec) => {
    const routePath = spec.pathTemplate.replace(":code", DEMO_CUSTOMER);
    const installEntry = installAudit.entries.find(
      (e) => e.route === spec.pathTemplate || e.route.replace(":code", DEMO_CUSTOMER) === routePath
    );
    const publishEntry = publishAudit.pwAs.find((p) => p.id === spec.id);

    const installReady: RehearsalCheck = {
      id: "install_ready",
      label: "installReady",
    ...
    const manifestOk =
      spec.manifestIsDynamic || (manifestFile ? fileExists(manifestFile) : false);
    const manifest: RehearsalCheck = {
      id: "manifest",
      label: "manifest",
    ...
    const swFile = spec.serviceWorker.replace(/^\//, "");
    const swOk = fileExists(swFile);
    const serviceWorker: RehearsalCheck = {
      id: "service_worker",
      label: "sw",
      status: swOk ? "pass" : "fail",
      message: swOk ? spec.serviceWorker : `SW 不足: ${swFile}`,
    ...
    if (manifestFile) {
      try {
        const mf = JSON.parse(readText(manifestFile) ?? "{}") as { display?: string };
...
export function buildTvRehearsalAudit(): TvRehearsalReport {
  ...
  const focusApi =
    fs.existsSync(path.join(serverRoot, "src", "api", "routes", "tv.ts")) &&
    fs.readFileSync(path.join(serverRoot, "src", "api", "routes", "tv.ts"), "utf8").includes(
      '"/focus-camera"'
    );
  const cameraFocus =
    tvJs.includes("camera_focus") || tvJs.includes("focusCamera");
  const wsClient =
    tvJs.includes("WebSocket") || tvJs.includes("new WebSocket");
...
export function buildSecurityRehearsalAudit(
  source: NodeJS.ProcessEnv = process.env
): SecurityRehearsalReport {
  ...
  const jwt: RehearsalCheck = {
    id: "jwt",
    label: "jwt",
    status:
      jwtVal && !INSECURE_VALUES.has(jwtVal) && jwtVal.length >= 32 ? "pass" : "fail",
    message:
      jwtVal && !INSECURE_VALUES.has(jwtVal) && jwtVal.length >= 32
        ? "JWT_SECRET 設定済み（32文字以上）"
        : "JWT_SECRET 未設定・デフォルト・短すぎ",
  };

  const ingest = (source.INGEST_SECRET ?? "").trim();
  const secret: RehearsalCheck = {
    id: "secret",
    label: "secret",
  ...
  const adminHashCheck: RehearsalCheck = {
    id: "admin_hash",
    label: "admin hash",
  ...
  const debugFlag: RehearsalCheck = {
    id: "debug_flag",
    label: "debug flag",
    status:
      nodeEnv === "production" && !demoReset
        ? demoMode
          ? "warn"
          : "pass"
        : nodeEnv === "test"
  ...
  for (const guard of MOCK_REAL_GUARDS) {
    ...
    if (isReal) realServices.push(guard.service);
  }
  const mockFlag: RehearsalCheck = {
    id: "mock_flag",
    label: "mock flag",
  ...
  const verdict: ReadyVerdict =
    checks.every((c) => c.status !== "fail") && !hasBlockingEnvErrors(source)
      ? "READY"
      : "NOT READY";
...
export function calculateReadyScore(input: {
  ...
  if (!input.buildOk) ngItems.push("Build: dist/index.js 未ビルド");

  const testScore = input.testOk ? 15 : 0;
  if (!input.testOk) ngItems.push("Test: npm run test / release:gate 未合格");
  ...
  if (pwaScore < 20) ngItems.push(`PWA: ${input.pwaAudit.readyCount}/${input.pwaAudit.totalPwa} READY`);
  ...
  if (tvScore < 10) ngItems.push(`TV: ${tvPass}/${tvTotal} チェック合格`);
...
export function buildProductionSimulation(
  ...
  const sections = {
    releaseGate: {
      id: "release_gate",
      label: "Release Gate",
      status: releaseGatePass ? ("pass" as RehearsalStatus) : ("fail" as RehearsalStatus),
      message: releaseGate.message,
    },
    health: {
      id: "health",
      label: "Health",
      status: healthProbe.ok ? ("pass" as RehearsalStatus) : ("fail" as RehearsalStatus),
    ...
    build: {
      id: "build",
      label: "Build",
      status: buildOk ? ("pass" as RehearsalStatus) : ("warn" as RehearsalStatus),
      message: buildOk ? "dist/index.js 存在" : "npm run build 未実行",
    },
    nginx: {
      id: "nginx",
      label: "nginx",
      status: nginxHasRoutes() ? ("pass" as RehearsalStatus) : ("fail" as RehearsalStatus),
    ...
    ws: {
      ...
      label: "ws",
      status:
        nginxHasWebSocket() && urlAudit.publicFacingClean
          ? ("pass" as RehearsalStatus)
          : ("fail" as RehearsalStatus),
    ...
    pwa: {
      id: "pwa",
      label: "pwa",
      status: pwaAudit.verdict === "READY" ? ("pass" as RehearsalStatus) : ("fail" as RehearsalStatus),
      message: `${pwaAudit.readyCount}/${pwaAudit.totalPwa} PWA installReady`,
    },
    env: {
      id: "env",
      label: "env",
      status: securityAudit.verdict === "READY" ? ("pass" as RehearsalStatus) : ("fail" as RehearsalStatus),
  ...
  return {
    ...
    summary: {
      ...
      readyRate: Math.round(
        ((sectionPass + urlCheck.readyCount + pwaAudit.readyCount) /
