...
const DEMO_USER_ROLES = ["owner", "admin", "manager", "installer", "surveyor", "maintenance", "viewer"] as const;
...
export function buildVpsCustomerLoginDeployBlock(): string[] {
  return [
    "cd /opt/tisly",
...
export function buildCustomerLoginCheck(): CustomerLoginCheckReport {
  ...
  const loginFormExists =
    customerPortalHtmlOk && /id="login-form"/.test(portalHtml) && /id="login-username"/.test(portalHtml);
  const submitButtonExists =
    customerPortalHtmlOk && /type="submit"[^>]*id="btn-login"/.test(portalHtml);
  const portalNavHiddenBeforeLogin =
    customerPortalHtmlOk && /id="portal-nav"[^>]*hidden/.test(portalHtml);
  ...
  const postLoginRedirectOk =
    customerJsOk && /location\.replace/.test(portalJs) && /setCustomerToken/.test(portalJs);

  const demoUsers = DEMO_USER_ROLES.map((role) => `${DEMO_CUSTOMER_CODE.toLowerCase()}.${role}`);

  const demoPasswordConfigured =
    Boolean(process.env.CUSTOMER_DEMO_PASSWORD?.trim()) || process.env.NODE_ENV !== "production";

  const demoAccountOk =
    customerPortalHtmlOk && /demo-user-hint/.test(portalHtml) && demoPasswordConfigured;
  ...
  const customerPortalPrecached =
    swJs.includes("customer-portal.html") &&
  ...
  const checks: CustomerLoginCheckItem[] = [
    {
      id: "customer-route",
