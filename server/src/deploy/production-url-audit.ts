...
const PATTERNS: { kind: UrlViolationKind; re: RegExp }[] = [
...
/** 監査対象ルートと関連 HTML */
export const PRODUCTION_ROUTE_FILES: Record<string, string[]> = {
...
function isAcceptablePath(relPath: string): boolean {
  const norm = relPath.replace(/\\/g, "/");
  return ACCEPTABLE_PATH_SEGMENTS.some((s) => norm.includes(s.replace(/\\/g, "/")));
}

function isAcceptableSnippet(snippet: string): boolean {
...
function scanFile(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const { kind, re } of PATTERNS) {
      ...
      if (isAcceptableSnippet(snippet)) continue;
...
function collectRouteFiles(): { route: string; files: string[] }[] {
  const items: { route: string; files: string[] }[] = [];
...
export function buildProductionUrlAudit(): ProductionUrlAuditReport {
  const violations: UrlViolation[] = [];
