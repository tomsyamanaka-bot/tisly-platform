...
/** 30秒 → 5分 → 30分 で通知先を変更 */
export const DEFAULT_ESCALATION: EscalationSchedule[] = [
...
export function tierForElapsed(elapsedSec: number): EscalationSchedule {
  let current = DEFAULT_ESCALATION[0]!;
  for (const s of DEFAULT_ESCALATION) {
    if (elapsedSec >= s.afterSec) current = s;
  }
  return current;
}
