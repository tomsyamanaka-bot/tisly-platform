...
export const DEFAULT_HEARTBEAT_RULE: RecoveryRule = {
...
export const DEVICE_RECOVERY_RULES: RecoveryRule[] = [
...
export function findRuleForDevice(
  deviceKind: string,
  trigger: RecoveryRule["trigger"] = "heartbeat_lost"
): RecoveryRule | undefined {
  const kind = deviceKind as DeviceKind;
  return DEVICE_RECOVERY_RULES.find(
    (r) => r.trigger === trigger && r.deviceKinds.includes(kind)
  );
}
