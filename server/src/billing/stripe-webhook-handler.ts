...
import {
  ...
  syncPlanFromSubscriptionObject,
} from "./plan-sync.js";
import type { StripeSubscriptionEvent } from "./stripe-client.js";
import { getCustomerByCode } from "../customer/customer-store.js";
import { logAudit } from "../provisioning/audit-log.js";

function resolveCustomerFromSubscription(sub: Record<string, unknown>) {
  ...
  const stripeCustomerId =
    typeof customerRef === "string"
      ? customerRef
      : (customerRef as { id?: string } | undefined)?.id;
  if (stripeCustomerId) {
    return getBillingByStripeCustomerId(stripeCustomerId);
  }
  const meta = sub.metadata as { customer_code?: string } | undefined;
  if (meta?.customer_code) {
    const c = getCustomerByCode(meta.customer_code);
    if (c) return getBillingByCustomerId(c.customer_id);
  }
  return null;
}

export function handleStripeWebhookEvent(event: StripeSubscriptionEvent): {
  handled: boolean;
  action: string;
  customerCode?: string;
} {
  ...
  switch (event.type) {
    ...
    case "invoice.payment_succeeded": {
      ...
      const customer =
        (subId ? getBillingByStripeSubscriptionId(subId) : null) ??
