...
export function practicalSearchV1(query: string, limit = 40): PracticalSearchHitV1[] {
  const q = query.trim();
  if (!q) return [];
  const like = `%${q}%`;
  const hits: PracticalSearchHitV1[] = [];

  const estimates = getDatabase()
    .prepare(
      `SELECT e.id, e.estimate_no, e.project_id, p.title, p.customer_name, p.address
    ...
    .all(like, like, like, like) as Array<Record<string, string>>;
  ...
  const invoices = getDatabase()
    .prepare(
      `SELECT i.id, i.invoice_no, i.project_id, p.title, p.customer_name
  ...
  const customers = getDatabase()
    .prepare(
      `SELECT id, name, company, phone, address FROM toms_customer_master
  ...
  const projects = getDatabase()
    .prepare(
      `SELECT id, project_no, title, customer_name, address, phone, status
  ...
  const surveys = getDatabase()
    .prepare(
      `SELECT project_id, project_no, site_name, customer_name, address, phone, assignee
       FROM survey_projects
       WHERE project_no LIKE ? OR site_name LIKE ? OR customer_name LIKE ? OR address LIKE ? OR phone LIKE ? OR assignee LIKE ?
       ORDER BY updated_at DESC LIMIT 15`
    )
    .all(like, like, like, like, like, like) as Array<Record<string, string>>;
