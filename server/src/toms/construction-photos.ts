...
const KEYWORDS: Array<{ category: ConstructionPhotoCategory; patterns: RegExp[] }> = [
...
export function classifyPhotoCategory(filename: string, caption = ""): ConstructionPhotoCategory {
...
export function saveConstructionPhoto(input: {
  ...
  getDatabase()
    .prepare(
      `INSERT INTO business_construction_photos
...
export function listConstructionPhotos(projectId: string): ConstructionPhoto[] {
  const rows = getDatabase()
    .prepare(
      `SELECT * FROM business_construction_photos WHERE project_id = ? ORDER BY created_at ASC`
    )
