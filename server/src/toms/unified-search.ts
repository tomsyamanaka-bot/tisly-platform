...
export function unifiedSearch(query: string, limit = 40): UnifiedSearchHit[] {
  const q = query.trim();
  if (!q) return [];
  const like = `%${q}%`;
  ...
  const customers = getDatabase()
    .prepare(
      `SELECT id, name, company, address, phone, email FROM toms_customer_master
       WHERE name LIKE ? OR company LIKE ? OR phone LIKE ? OR address LIKE ? OR email LIKE ?
       LIMIT 15`
    )
    .all(like, like, like, like, like) as Array<Record<string, string>>;
  ...
  const projects = getDatabase()
    .prepare(
      `SELECT id, project_no, customer_name, title, address, phone, status
  ...
  const invoices = getDatabase()
    .prepare(
      `SELECT i.id, i.invoice_no, i.project_id, p.title
  ...
  const assets = getDatabase()
    .prepare(
      `SELECT id, label, asset_type, serial_number, project_id FROM toms_assets
       WHERE label LIKE ? OR serial_number LIKE ? OR asset_type LIKE ?
       LIMIT 10`
    )
    .all(like, like, like) as Array<Record<string, string>>;
  ...
  const maint = getDatabase()
    .prepare(
      `SELECT case_id, site_name, status, customer_code FROM maintenance_cases
  ...
  const construction = getDatabase()
    .prepare(
      `SELECT id, title, construction_schedule_json FROM business_projects
       WHERE construction_schedule_json LIKE ? LIMIT 5`
    )
    .all(like) as Array<Record<string, string>>;
