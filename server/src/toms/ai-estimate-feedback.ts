...
export function saveAiEstimateFeedback(input: {
  ...
  getDatabase()
    .prepare(
      `INSERT INTO ai_estimate_feedback
    ...
    .run(
      id,
      input.projectId,
      input.estimateV3Id ?? null,
      input.action,
      input.notes ?? "",
      JSON.stringify(input.candidate ?? {}),
      now
    );
  const labels: Record<AiFeedbackAction, string> = {
    adopted: "採用",
    revised: "修正",
    rejected: "却下",
  };
...
export function listAiEstimateFeedback(projectId: string): AiEstimateFeedbackRecord[] {
  const rows = getDatabase()
    .prepare(
      `SELECT * FROM ai_estimate_feedback WHERE project_id = ? ORDER BY created_at DESC`
    )
  ...
  return rows.map((r) => ({
    ...
    action: String(r.action) as AiFeedbackAction,
    notes: String(r.notes ?? ""),
    candidate: JSON.parse(String(r.candidate_json ?? "{}")) as Record<string, unknown>,
