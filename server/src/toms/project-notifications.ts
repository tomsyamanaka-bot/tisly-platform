...
export type ProjectNotificationKind =
  | "unacked_alert"
  | "estimate_unsent"
  | "invoice_unsent"
  | "payment_pending"
...
function rowToNotification(r: Record<string, unknown>): ProjectNotification {
  return {
    id: String(r.id),
    projectId: String(r.project_id),
    kind: String(r.kind) as ProjectNotificationKind,
    title: String(r.title),
    body: String(r.body),
    severity: String(r.severity) as ProjectNotification["severity"],
    href: String(r.href ?? ""),
...
function upsertDerivedNotification(input: {
  ...
  const existing = getDatabase()
    .prepare(
      `SELECT id FROM toms_project_notifications
       WHERE project_id = ? AND kind = ? AND acknowledged = 0`
    )
    .get(input.projectId, input.kind) as { id: string } | undefined;
  if (existing) {
    getDatabase()
      .prepare(
        `UPDATE toms_project_notifications SET title = ?, body = ?, severity = ?, href = ?
         WHERE id = ?`
      )
  ...
  getDatabase()
    .prepare(
      `INSERT INTO toms_project_notifications
    ...
    .run(
      ...
      input.body,
      input.severity,
      input.href ?? `/project/${input.projectId}`
    );
}

export function refreshProjectNotifications(projectId: string): ProjectNotification[] {
  ...
  for (const d of listProjectLiveDevices(projectId)) {
    if (d.status === "OFFLINE" || d.status === "WARNING") {
      ...
      let kind: ProjectNotificationKind | null = null;
      if (t.includes("esp") || t.includes("controller")) kind = "esp_anomaly";
      else if (t.includes("shelly")) kind = "shelly_anomaly";
      else if (t.includes("camera")) kind = "camera_anomaly";
      if (kind) {
        upsertDerivedNotification({
          projectId,
          kind,
  ...
  const rows = getDatabase()
    .prepare(
      `SELECT * FROM toms_project_notifications WHERE project_id = ?
...
export function acknowledgeProjectNotification(
  projectId: string,
  ...
  getDatabase()
    .prepare(
      `UPDATE toms_project_notifications
