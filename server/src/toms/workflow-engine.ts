...
import { TOMS_WORKFLOW_STATES, type TomsWorkflowState } from "./toms-types.js";
...
const ALLOWED: Partial<Record<TomsWorkflowState, TomsWorkflowState[]>> = {
  draft: ["survey", "closed"],
...
const STATUS_TO_TOMS: Record<BusinessProjectStatus, TomsWorkflowState> = {
  new: "draft",
...
const TOMS_TO_STATUS: Record<TomsWorkflowState, BusinessProjectStatus> = {
  draft: "new",
...
const TOMS_TIMELINE: Partial<Record<TomsWorkflowState, string>> = {
  survey: "survey",
...
export function businessStatusToToms(status: string): TomsWorkflowState {
  const n = normalizeProjectStatus(status);
  return STATUS_TO_TOMS[n] ?? "draft";
}

export function getTomsWorkflowState(projectId: string): TomsWorkflowState | null {
  const p = getBusinessProject(projectId);
  if (!p) return null;
  return businessStatusToToms(p.status);
}

export function listWorkflowHistory(projectId: string, limit = 100): WorkflowHistoryEntry[] {
  const rows = getDatabase()
    .prepare(
      `SELECT * FROM toms_workflow_history WHERE project_id = ? ORDER BY created_at DESC LIMIT ?`
    )
    .all(projectId, limit) as Array<Record<string, unknown>>;
  return rows.map((r) => ({
    id: String(r.id),
    projectId: String(r.project_id),
    fromState: String(r.from_state) as TomsWorkflowState,
    toState: String(r.to_state) as TomsWorkflowState,
    note: String(r.note ?? ""),
...
export function transitionTomsWorkflow(
  projectId: string,
  to: TomsWorkflowState,
  opts?: { actor?: string; note?: string }
): { state: TomsWorkflowState; projectStatus: BusinessProjectStatus } {
  ...
  getDatabase()
    .prepare(
      `INSERT INTO toms_workflow_history (id, project_id, from_state, to_state, note, actor, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
...
export function recordWorkflowFromBusinessStatus(
  projectId: string,
  fromStatus: string,
  toStatus: string,
  actor = "business"
