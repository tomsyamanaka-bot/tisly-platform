...
type AiV3Recommended = Record<string, unknown>;

function countSymbols(projectId: string): {
  esp: number;
  light: number;
  camera: number;
  lanM: number;
  ...
  for (const plan of listDrawingPlans(projectId)) {
    for (const s of plan.symbols) {
      ...
      if (t.includes("esp") || t.includes("controller")) esp += 1;
      else if (t.includes("light") || t.includes("照明")) light += 1;
      else if (t.includes("camera") || t.includes("カメラ")) camera += 1;
    }
...
export function generateAiEstimateV3(projectId: string): AiEstimateV3Result {
  ...
  getDatabase()
    .prepare(
      `INSERT INTO toms_ai_estimate_v3
...
export function getLatestAiEstimateV3(projectId: string): AiEstimateV3Result | null {
  const row = getDatabase()
    .prepare(
      `SELECT * FROM toms_ai_estimate_v3 WHERE project_id = ? ORDER BY created_at DESC LIMIT 1`
    )
  ...
  return {
    ...
    checklist: JSON.parse(String(row.checklist_json ?? "[]")) as string[],
    candidate: {
      id: String(row.id),
      projectId: String(row.project_id),
      source: "manual" as const,
      recommended: JSON.parse(String(row.candidate_json ?? "{}")) as Record<string, unknown>,
      applied: false,
