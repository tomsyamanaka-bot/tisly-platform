...
export function buildFirmwareConfig(customerId: string, deviceId: string): FirmwareConfigExport {
  const db = getDatabase();
  const dev = db
    .prepare(
      `SELECT device_id, site_id, device_type FROM devices WHERE device_id = ? AND customer_id = ?`
    )
    .get(deviceId, customerId) as { device_id: string; site_id: string | null; device_type: string | null } | undefined;
  ...
  try {
    ...
    if (st.certIssued) {
      ...
      if (meta?.metadata_json) {
        const parsed = JSON.parse(meta.metadata_json) as { cert?: { certPem?: string } };
        if (parsed.cert?.certPem) certPlaceholder = String(parsed.cert.certPem);
      }
    }
  } catch {
    /* placeholder PEM */
