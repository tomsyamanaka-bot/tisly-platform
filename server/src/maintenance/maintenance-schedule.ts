...
function maintReportDir(customerCode: string): string {
  const dir = path.join(process.cwd(), "uploads", "maintenance", customerCode);
  fs.mkdirSync(dir, { recursive: true });
  return dir;
}

export function listMaintenanceSchedules(customerCode?: string): MaintenanceSchedule[] {
  const rows = customerCode
    ? (getDatabase()
        .prepare(
          `SELECT * FROM maintenance_schedules WHERE customer_code = ? ORDER BY due_date ASC`
        )
...
function rowToSchedule(r: Record<string, unknown>): MaintenanceSchedule {
  return {
...
export function listMaintenanceReports(customerCode: string, limit = 50): MaintenanceReport[] {
  const rows = getDatabase()
    .prepare(
      `SELECT * FROM maintenance_reports WHERE customer_code = ? ORDER BY completed_at DESC LIMIT ?`
    )
    .all(customerCode.toUpperCase(), limit) as Array<Record<string, unknown>>;
  return rows.map((r) => {
    let photos: string[] = [];
    try {
      photos = JSON.parse(String(r.photo_paths_json ?? "[]")) as string[];
    } catch {
      photos = [];
    }
    return {
