...
import { computeRiskScore, type AiAlertPriority } from "./risk-score.js";
import { analyzeTrends, type TrendReport } from "./trend-analyzer.js";
...
export function processEventAnalytics(event: TislyEvent): {
  riskScore: number;
  priority: AiAlertPriority;
  factors: string[];
} {
  const db = getDatabase();
  const siteId = event.siteId ?? (event.payload?.site_id as string | undefined);
  let concurrent = 0;
  if (siteId) {
    const row = db
      .prepare(
  ...
  db.prepare(
    `INSERT INTO analytics_snapshots (id, event_id, device_id, site_id, event_type, risk_score, priority, factors_json, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`
  ).run(
    uuid(),
    event.id ?? uuid(),
...
export function generateAiSummary(period: "today" | "week" | "month"): AiSummaryReport {
...
export function generateNaturalLanguageReport(
  period: "today" | "week" | "month"
  ...
  paragraphs.push(
    `${periodLabel}の分析では、全${trend.totalEvents}件のイベントのうち、異常・警告は${trend.anomalyCount}件でした。`
  );
  ...
  if (topSite && trend.anomalyCount > 0) {
    ...
    if (trend.peakHour) {
      paragraphs.push(
        `${name}では${trend.peakHour}前後の時間帯に${catJa}イベントが集中しています。`
      );
    } else {
