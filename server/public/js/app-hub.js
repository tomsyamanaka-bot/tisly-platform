...
function toggleOpsPanels(_show) {
  // /app 現場ハブでは Deploy / 監査を常時非表示
...
async function loadHubApps() {
  ...
  const moreApps = document.getElementById("hub-more-apps");
  const grid = document.getElementById("hub-app-grid");
  const appList = data.apps || [];
  if (grid) {
    grid.innerHTML = appList
      .map(
        (a) =>
  ...
  if (moreApps) moreApps.hidden = appList.length === 0;
  // 通知・ワークフロー・オペレーションは現場ハブから除外
